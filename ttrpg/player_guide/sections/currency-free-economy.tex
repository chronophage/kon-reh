...
\begin{tcolorbox}[colback=yellow!5!white,colframe=yellow!70!black,title=Abstracted Treasure: The Leverage Standard]
  ...
  \quad --- \text{``Give +1 die or skip a minor obstacle''} $\rightarrow$ Minor \\
  \quad --- \text{``Resolve a significant problem or open a major opportunity''} $\rightarrow$ Standard \\
  \quad --- \text{``End a war, topple a dynasty, or redirect destiny''} $\rightarrow$ Major
