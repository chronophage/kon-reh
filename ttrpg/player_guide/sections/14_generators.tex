...
\index{generators!deck}\index{cards|see{generators, deck}}\index{procedures!random}\index{clocks}\index{Story Beats (SB)}\index{Boons}
...
Draw \(\mathbf{1\times♠}\) + \(\mathbf{1\times♡}\).\; The Spade sets the where; the Heart sets the who. Highest rank sets the main Clock. \emph{Optional:} If both are the same color, GM gains \textbf{+1 SB} for that opening beat. \index{SB economy}
