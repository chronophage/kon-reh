Based on the audit findings, here's the corrected LaTeX chapter for Magic and Special Abilities:
...
\subsection*{Ritual Invocation (Invoker)}
\begin{itemize}
\item \textbf{Time}\index{rituals!time}: Requires Significant Time (typically 10-30 minutes)
\item \textbf{Obligation}\index{Obligation}: Completing a ritual always marks +1 Obligation
\item \textbf{No Push}\index{Rites!push}: Invoker Rites cannot use Push It benefits
\item \textbf{Symbol Display}\index{symbols}: The Symbol must be visible throughout the ritual
\end{itemize}
