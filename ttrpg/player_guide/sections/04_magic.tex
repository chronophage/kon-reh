...
\item Each 1 rolled generates a \textbf{Story Beat}\index{Story Beats} immediately
...
\item Additional \textbf{Story Beats}\index{Story Beats} may be generated
\end{itemize}

\subsection*{Phase 3: Backlash} \index{magic!backlash}
Story Beats manifest as magical consequences:
\begin{itemize}
\item The Game Master spends \textbf{SB}\index{Story Beats} thematically
...
\subsection*{Spellcasting with Tags}
Characters with the \textbf{Caster's Gift} talent (2 XP) can create magical effects using \textbf{[TAGS]} as building blocks. Rites have predefined tags, but  casters construct effects by combining tags narratively.

\paragraph{Common Tag Examples:}
\begin{itemize}
\item \textbf{[AREA]}: Affect multiple targets or a zone
\item \textbf{[BARRIER]}: Create an obstruction or wall
\item \textbf{[BIND]}: Restrict movement or actions
\item \textbf{[CLEANSE]}: Remove conditions or negative effects
\item \textbf{[DISPEL]}: Counter or end magical effects
\item \textbf{[FORTIFY]}: Grant protection or resistance
\item \textbf{[HEAL]}: Restore health or remove harm
\item \textbf{[ILLUSION]}: Create false perceptions
\item \textbf{[MARK]}: Tag a target for tracking or effects
\item \textbf{[REVEAL]}: Expose hidden things or truths
\item \textbf{[WARD]}: Create protective barriers
\end{itemize}

\paragraph{ Casting Process:}
\begin{enumerate}
\item \textbf{Describe Effect}: Narrate the desired magical outcome using tag concepts
\item \textbf{Assign Tags}: Identify which tags best represent the effect
\item \textbf{Set Difficulty}: GM determines DV based on complexity and tags used
\item \textbf{Channel \& Weave}: Use the standard casting loop
\item \textbf{Apply Tags}: Successful cast applies the chosen tag effects
\item \textbf{Backlash}: Thematically appropriate to elements/tags used
\end{enumerate}

\paragraph{Example  Cast:}
A caster wants to create a wall of fire to block pursuers:
\begin{itemize}
\item \textbf{Tags}: [BARRIER] + [FIRE] (implied damage element)
\item \textbf{DV}: 3 (moderate complexity)
\item \textbf{Effect}: Creates a blazing wall that blocks movement and deals fire damage
\item \textbf{Backlash}: Fire-related consequences (burns, smoke, ignition of nearby materials)
\end{itemize}
...
\subsection*{The Warlock (Rites Path)}
\begin{itemize}
\item Binds to a single \textbf{Patron} with Thiasos (Familiar) and Codex (4 XP)
\item Accesses that Patron's full Rite list
\item Structured, powerful, but accrues \textbf{Obligation}
\item Can Push Rites for +1 Obligation
\end{itemize}

\subsection*{The Invoker (Symbol Path)}
\begin{itemize}
\item Acquires \textbf{Patron's Symbols} (4 XP each)
\item Accesses ritual invocation of Patron's Rites
\item Safe but slow—ritual time = DV + 1
\item Can Crack the Seal for instant cast at steep Obligation cost
\end{itemize}

\subsection*{Becoming an Invoker}
\begin{itemize}
\item Choose a \textbf{Patron}\index{Patrons} entity
...
\item \textbf{Set Difficulty}\index{Difficulty Value (DV)}: DV = max(Obligation cost - Spirit, Tier)
\item \textbf{Roll}\index{dice pools}: Wits + Arcana (or appropriate magical skill)
...
\subsection*{Invoker Ritual Timing}
\begin{itemize}
\item \textbf{Standard Ritual}: Time required = DV + 1 (in Player Turns)
\item \textbf{Crack the Seal}: Instant cast by compromising Symbol (+2/+3 Obligation)
\item \textbf{Symbol Requirements}: Must be openly displayed during ritual
\item \textbf{Environment}: Requires suitable ritual space and conditions
\end{itemize}
...
\item \textbf{Backlash}\index{magic!backlash}: SB spent thematically
\end{itemize}
...
\item Ritual time = DV + 1 Player Turns
\item Crack the Seal for instant cast (+2/+3 Obligation)
\end{itemize}
...
\begin{itemize}
\item \textbf{Channel}\index{magic!channel}: Wits 4 + Arcana 3 $=$ 7d10, gets 4 successes (\textbf{Potential 4}\index{magic!Potential})
\item \textbf{Weave}\index{magic!weave}: Wits 4 + Fire Magic 3 $=$ 7d10, gets 3 successes
\item \textbf{Effect}\index{Rites!effect}: Creates a fire wall that blocks enemies (DV met)
\item \textbf{Backlash}\index{magic!backlash}: 2 SB spent—nearby foliage smolders, creating smoke
\end{itemize}
...
\subsection*{Invoker Ritual Example}
An invoker performs a ritual:
\begin{itemize}
\item \textbf{Preparation}\index{rituals!preparation}: Sets up ritual space, displays Symbol openly
\item \textbf{Ritual Time}\index{rituals!time}: DV 3 Rite requires 4 Player Turns (DV + 1)
\item \textbf{Completion}\index{rituals!completion}: Marks +1 Obligation segment
\item \textbf{Effect}\index{rituals!effect}: Creates protective ward around safehouse
\end{itemize}
...
\textbf{Magic as Character Development:} Use magical experiences as opportunities for character growth and backstory development, allowing players to narrate how their characters learned new abilities through significant story moments.\end{chapter}