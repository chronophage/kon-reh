...
\subsection*{Free Casting Cheat Sheet}
...
\index{cantrips}\index{magic!cantrips}

Not every spell is a ritual.

\textbf{Cantrips} are simple, practiced bits of magic—effects you’ve cast so many times they’ve become second nature. If a spell uses \textbf{only one [TAG]}, you may resolve it with a \textbf{single roll} instead of the usual two-step casting process.

\paragraph{How It Works}
\begin{itemize}
  \item Declare your intent and the single [TAG] you’re using
  \item Roll the appropriate Attribute + Casting Skill
  \item Resolve the result normally using the Outcome Matrix
\end{itemize}

\paragraph{What Cantrips Are Good For}
\begin{itemize}
  \item Small, immediate effects
  \item Everyday magic and utility
  \item Flavorful problem-solving that keeps the story moving
\end{itemize}

\paragraph{What Cantrips Cannot Do}
Cantrips cannot:
\begin{itemize}
  \item Combine multiple Tags
  \item Create lasting, large-scale, or complex effects
  \item Bypass wards, sanctums, or major magical defenses
\end{itemize}

If a spell feels important, dangerous, or transformative, it’s no longer a cantrip—and must be cast using the full rules.

\paragraph{In Short}
Cantrips let you \emph{do magic freely}, while heavier spellwork still carries weight, risk, and consequence.
...
\newpage
\section{Optional Magic Path: Psionics}
\index{Psionics}

Psionics represents disciplined mental power drawn entirely from within.  
Unlike spellcasting or Patron rites, psionic abilities require no symbols, rituals, or bargains—but exact a toll on the mind itself.

Psionics is an optional system. If used, it follows the same resolution rules as other actions, but introduces a unique resource: \textbf{Mental Fatigue}.

\subsection{\textit{\textbf{Optional}} Psionic Arts Overview}

\begin{center}
\small
\begin{longtable}{p{3cm} p{3.2cm} p{2.2cm} p{3.8cm}}
\toprule
\textbf{Art} & \textbf{What It Does} & \textbf{Key Attribute} & \textbf{Cost \& Risk} \\
\midrule
Telekinesis &
Move, lift, throw, or shape objects at range &
Spirit (force) / Wits (precision) &
1 Mental Fatigue to activate; heavier effects generate SB \\
\addlinespace

Telepathy &
Read thoughts, emotions, or surface intent &
Wits (analysis) / Presence (emotive) &
Depth increases DV and SB; unwilling minds resist \\
\addlinespace

Clairvoyance &
See distant places or hidden truths &
Wits (focus) / Spirit (trance) &
Mental Fatigue scales with distance; spying creates SB \\
\addlinespace

Biofeedback &
Heal, enhance, or regulate the body &
Spirit (control) / Wits (technique) &
Healing capped per day; major effects generate SB \\
\addlinespace

Astral Projection &
Leave the body to scout or travel astrally &
Spirit &
1 Mental Fatigue per scene; body is Helpless \\
\addlinespace

Psychic Assault &
Inflict mental harm or disrupt cognition &
Spirit (overwhelm) / Wits (precision) &
Always generates SB; bypasses armor \\
\addlinespace

Mind Shield &
Block or dampen psychic effects &
Wits (filter) / Spirit (soak) &
Protecting others costs double Mental Fatigue \\
\addlinespace

Empathic Manipulation &
Influence emotions and group mood &
Presence &
Strong effects create SB and emotional backlash \\
\addlinespace

Precognition &
Glimpse possible futures or imminent danger &
Spirit &
High SB generation; risks fixation on fate \\
\bottomrule
\end{longtable}
\end{center}

\subsection{Using Psionics}

To use psionics, roll \textbf{Attribute + Psionics} against a DV set by scope and risk.

\begin{itemize}
\item Each use costs \textbf{Mental Fatigue} (tracked separately from normal Fatigue).
\item Partial successes and misses generate Story Beats as normal.
\item Overflowing Mental Fatigue converts into standard Fatigue or Harm.
\item Psionics never replaces skills—social, physical, and investigative rolls still matter.
\end{itemize}

\subsection{Why Use Psionics?}

Psionics offers:
\begin{itemize}
\item Immediate, flexible power
\item No ritual or material requirements
\item High narrative impact
\end{itemize}

At the cost of:
\begin{itemize}
\item Rapid Story Beat generation
\item Mental exhaustion
\item Legal, social, or psychological fallout
\end{itemize}

Psionics rewards restraint, foresight, and consequence management.

\begin{tcolorbox}[
  colback=blue!5!white,
  colframe=blue!75!black,
  title=Witchcraft Rules (Compressed),
  fonttitle=\bfseries,
  boxrule=0.5pt
]
\small

\textbf{Core Concept:} Witchcraft is magic practiced through patron-bound Orders. Each Order grants access to unique rites, costs, and identity-altering consequences.

\medskip
\noindent
\textbf{The Curse of the Unanointed:}
Witches are never fully welcome in sanctified spaces.
\begin{itemize}[leftmargin=1.25em]
  \item Suffer \textbf{–1 die} to social actions in consecrated places.
  \item Wards, blessings, and holy rites react violently, inflicting \textbf{1 Fatigue}.
\end{itemize}

\medskip
\noindent
\textbf{Ritual Magic (Weave \& Cast):}
Witchcraft is resolved through a two-step ritual process.
\begin{itemize}[leftmargin=1.25em]
  \item \textit{Weave:} Shape the magical intent (Attribute + Skill).
  \item \textit{Cast:} Channel the effect into reality (Attribute + Skill).
  \item Each roll generates \textbf{Story Beats (SB)} on \textbf{1s}.
\end{itemize}

\medskip
\noindent
\textbf{Orders (Examples):}
\begin{itemize}[leftmargin=1.25em]
  \item \textbf{Mab, Queen of Courts:} Glamour, bargains, fae oaths.  
  \textit{Risk — Glamour-Bloom:} Lose a memory to gain \textbf{+1 Effect}.
  \item \textbf{Aveh, The Rider:} Identity shedding and anonymity.  
  \textit{Risk — Unface:} Your features blur or vanish from memory.
  \item \textbf{Morag, The Hag:} Hidden costs and prophetic insight.  
  \textit{Risk — Mirecloth:} Sacrifice identity to uncover a truth.
\end{itemize}

\medskip
\noindent
\textbf{Identity Cost:}
Every use of witchcraft leaves a mark on who you are.  
The cost of magic is never just coin or time—it is memory, identity, or reputation.

\medskip
\noindent
\textbf{Example:}
A witch of Mab invokes \emph{Revealing Glamour} to pierce an illusion.  
They may either gain insight, or erase a memory that both they and the target once shared.
