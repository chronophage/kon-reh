...
\item Explain how you gained skills, languages (see the full rules for language mechanics), and connections
...
\item \textbf{A Signature Contact} (named NPC; usually a Cap~1 follower you \emph{do not} pay XP for; they cannot take independent actions (see \ref{sec:follower-independent}), but can grant +1d \emph{once per scene} when their help is plausible)
...
\item \textbf{An Obligation Clock} (4 segments; when filled, it creates a \emph{call-in} from that world---a favor owed, a duty recalled, a price to pay; see \ref{sec:bg-obligation})
\end{itemize}
...
\item \textbf{Challenges}: Intrigue timers, reputation stakes
\end{itemize}
...
\item \textbf{Resource Access}: Borrowed tools, rooms, shrines, or records (scene-time instead of XP; see Assets, \ref{ch:assets-followers})
...
\item \textbf{Contact Use}: +1d as assist; cannot take independent actions unless purchased as a \emph{Follower} (Cap$^2$ XP; \ref{ch:assets-followers})
...
\item \textbf{Languages}: Choose one tongue linked to the background (see the full rules for languages); at least \emph{Conversational}
\end{itemize}
...
\item Spend Story Beats (see \ref{ch:world-interaction}) to \emph{complicate the old ties}
\end{itemize}
...
\item \textbf{Transform}: Rewrite the background after a major arc; keep one tag, swap the rest, reset the timer
...
\item Pick 1 language tied to the background at \emph{Conversational} or better (see the full rules for languages).
\end{itemize}
...
\item \textbf{Hook}: Duel timer from an old slight; an audit of campaign spoils
\end{itemize}
...
A bond is a \textbf{player-to-player agreement} about how your characters relate. It must be:
\begin{itemize}
...
Once per bond \textbf{per session}, when you take a significant action to aid the other character and explicitly reference your bond in an \textbf{intricate description} (see Description Ladder, \ref{ch:core-mechanics}), you both gain \textbf{1 Boon} after the action resolves.
...
When you assist a bonded ally on a roll, the first Boon you spend to assist (see Assistance, \ref{ch:core-mechanics}) is \textbf{refunded} if the roll succeeds. (You still mark any Fatigue or other costs normally.)

\subsection{Creating Bonds at Character Creation}

At campaign start, each player should establish \textbf{two bonds} with other player characters. Use the tables below for inspiration, or invent your own.
...
\textbf{Per Session:}
\begin{itemize}
\item Once per bond per session, invoke with intricate description $\rightarrow$ both gain 1 Boon.
\end{itemize}
