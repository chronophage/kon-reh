...
\paragraph{Rush Rule:} If you want to complete training faster, you may attempt a \textbf{Rush}. The GM starts a \textbf{Risk Clock [4]}. Each downtime segment you roll \textbf{Spirit + Resolve} (DV 3) to push yourself. For each failure (or 1 on the die), the clock ticks. If the clock fills before training is complete, the new ability has a flaw (temporary –1 die, or a narrative complication). The GM decides the flaw.
...
\begin{itemize}
  ...
  \item \textbf{The Traveler:} Guide a lost traveller to safety.
...
See \ref{ch:gear} for full rules. In brief:
\begin{itemize}
  ...
  \item \textbf{Permanent magic items:} Requires a Downtime Project Clock (6–12 segments), rare components (obtained through quests), and the XP cost of the item (see \ref{sec:gear-magic-items}). You roll each downtime to advance the clock.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Extended research:} Use a Progress Clock [4–8] for complex topics (e.g., uncovering a noble’s secret). Each downtime you can roll to advance the clock.
\end{itemize}
...
\begin{itemize}
  \item \textbf{Rumours:} Roll \textbf{Presence + Sway} (DV 3). On success, learn one rumour (true or false – the GM decides). On partial, the rumour is vague or costs you a drink (1 Fatigue).
...
Your character may have a unique long‑term goal (e.g., building a guild, writing a grimoire, taming a monster). Work with your GM to create a \textbf{Downtime Project Clock} (6–12 segments). Each downtime you can describe your progress and roll an appropriate skill to advance the clock. When it fills, the project is complete.
...
Your GM determines how much downtime you have between adventures (usually a few days to several weeks). They also set DVs for downtime rolls and adjudicate any complications. Downtime is not a “free” rest – the world may advance clocks (faction agendas, rival plans) while you rest. Ask your GM what has changed when you return.
