...
\begin{itemize}
  ...
  \item \textbf{2 SB:} Moderate setback -- alarm raised, lose cover, lesser foe appears, or advance a faction or front clock by 1 segment.
...
\end{tikzpicture}
\caption{Taking Harm and Fatigue – The Procedure (see \ref{sec:take-harm-fatigue})}
\label{fig:harm-fatigue-procedure}
\end{figure}
...
\paragraph{Examples}
\begin{itemize}
  \item \textbf{Light Armor, Harm 2:} Result is Harm 1, Fatigue 1. Apply Harm (clears existing Fatigue, Harm 0$\rightarrow$1), then add Fatigue 1. (If you had Fatigue before, it is cleared first.)
  ...
  \item \textbf{Overflow example:} Body 2, currently 1 Fatigue, Heavy Armor hit with Harm 3 $\rightarrow$ result Fatigue 2. Total Fatigue becomes 1+2=3, which fills the track (max 2). Overflow triggers: Harm 0$\rightarrow$1, clear all Fatigue. Final: Harm 1, Fatigue 0.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Example (Spear Fighter):} Kael is at Close range with a spear (medium weapon, +1d at Close, +2d at Near). He steps back to Near as part of his attack, worsening his Position from Controlled to Desperate, but gains +2d instead of +1d.
\end{itemize}
