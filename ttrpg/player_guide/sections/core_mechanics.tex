...
\textbf{When you are hit, the GM states the Harm value. You calculate the conversion using your armor type, then apply the result in this order:}

\begin{enumerate}
  \item \textbf{GM States the Harm:} The GM narrates the attack and tells you the incoming Harm value (e.g., ``You take Harm 3'').
        \begin{itemize}
          \item Your job is to know your armor type and apply the conversion yourself.
          \item The GM does \textit{not} calculate the conversion for you.
        \end{itemize}

  \item \textbf{Calculate the Conversion:} Using the \textbf{Armor Conversion Table} (see \ref{sec:armor}), determine:
        \begin{itemize}
          \item \textbf{Fatigue gained:} The Fatigue cost of the hit (minimum 1).
          \item \textbf{Harm remaining:} Any Harm that bypasses your armor.
        \end{itemize}
        \begin{itemize}
          \item \textbf{Example:}
                \begin{itemize}
                  \item \textit{GM:} ``You take Harm 2.''
                  \item \textit{You (Light Armor):} ``Harm 2 in Light Armor $\to$ Harm 1 remaining, Fatigue 1 gained.''
                \end{itemize}
        \end{itemize}

  \item \textbf{Apply Remaining Harm First:}
        \begin{itemize}
          \item Clear \textbf{all} existing Fatigue (the shock of injury resets exhaustion).
          \item Increase your Harm level by the Harm remaining value you calculated.
        \end{itemize}
        \begin{itemize}
          \item \textbf{Example:} You calculated Harm remaining = 1 $\to$ Clear Fatigue, Harm +=1.
        \end{itemize}

  \item \textbf{Add the Fatigue:}
        \begin{itemize}
          \item Add the Fatigue gained from your calculation to your Fatigue track.
          \item If this fills or exceeds your track (Body value), resolve \textbf{Overflow} (see \ref{sec:fatigue}).
        \end{itemize}
        \begin{itemize}
          \item \textbf{Example:} You calculated Fatigue gained = 2 $\to$ Add 2 Fatigue, then check if Fatigue $\geq$ Body.
        \end{itemize}

  \item \textbf{Narrate the Result:}
        \begin{itemize}
          \item Describe what it looks like when your character takes the hit.
          \item ``The blade slides under my shield -- I feel the sting (1 Harm) as exhaustion fades (1 Fatigue added).''
          \item Or: ``My breastplate absorbs the impact -- the blow rattles my teeth (2 Fatigue) but draws no blood.''
        \end{itemize}
\end{enumerate}

\begin{tcolorbox}[colback=yellow!5!white,colframe=yellow!75!black,title=\textbf{Player Tip: Own Your Conversion},breakable]
\textbf{You are responsible for calculating your own armor conversion.} Your job is to:
\begin{itemize}
  \item Know your armor type (Light/Medium/Heavy/Superior/Mythic).
  \item Consult the conversion table when the GM states incoming Harm.
  \item Apply the result in the correct order: \textbf{Harm first} (clear Fatigue), then \textbf{Fatigue} (check overflow).
  \item Narrate the result to keep the fiction alive.
\end{itemize}
\textit{This keeps the pressure engine running: the GM narrates the attack, you manage your resources, and the story stays vivid.}
\end{tcolorbox}

\paragraph{Armor Conversion Table (Player Reference)}
\label{sec:armor}
\begin{center}
\begin{tabular}{l|ccccc}
\toprule
\textbf{Armor Type} & \multicolumn{5}{c}{\textbf{Incoming Harm}} \\
\cmidrule(lr){2-6}
& \textbf{Harm 1} & \textbf{Harm 2} & \textbf{Harm 3} & \textbf{Harm 4} & \textbf{Harm 5} \\
\midrule
Light  & F1 & H1 + F1 & H2 + F1 & H3 + F1 & H4 + F1 \\
Medium & F1 & F1 & H2 + F1 & H3 + F1 & H4 + F1 \\
Heavy  & F1 & F1 & F2 & H1 + F2 & H2 + F2 \\
Superior & F1 & F1 & F1 & F3 & H1 + F3 \\
Mythic   & F1 & F1 & F1 & F1 & F4 \\
\bottomrule
\end{tabular}
\end{center}
\textit{Minimum 1 Fatigue per hit, even if conversion would give 0.}
\textit{``F'' = Fatigue gained; ``H'' = Harm remaining.}

\paragraph{Understanding the Pattern}
The armor conversion table follows a simple pattern. Each armor type has a \textbf{Step} (Light = 1, Medium = 2, Heavy = 3, Superior = 4, Mythic = 5). For any incoming Harm:

\begin{itemize}
  \item \textbf{If Harm $\leq$ Step:} All Harm is converted to Fatigue. The Fatigue gained is 1 less than the Harm value (minimum 1).
  \item \textbf{If Harm $>$ Step:} The first Step points of Harm are converted to Fatigue (Step $-$ 1 Fatigue), and the remaining Harm points are applied directly.
\end{itemize}

\begin{center}
\begin{tabular}{l|c|c}
\toprule
\textbf{Armor Step} & \textbf{Converts} & \textbf{Fatigue per Step} \\
\midrule
1 (Light)  & 1 Harm & 1 Fatigue \\
2 (Medium) & 2 Harm & 1 Fatigue \\
3 (Heavy)  & 3 Harm & 2 Fatigue \\
4 (Superior) & 4 Harm & 3 Fatigue \\
5 (Mythic)   & 5 Harm & 4 Fatigue \\
\bottomrule
\end{tabular}
\end{center}

\paragraph{Player Examples (Full Conversion Process)}
\begin{itemize}
  \item \textbf{Example 1 -- Light Armor:}
        \begin{itemize}
          \item \textit{GM:} ``You take Harm 2.''
          \item \textit{You (Light Armor):} ``Harm 2 in Light Armor $\to$ Harm 1 remaining, Fatigue 1 gained.''
          \item \textit{Apply:} Clear Fatigue, Harm +=1, then add 1 Fatigue.
          \item \textit{Narrative:} ``The blade slides under my shield -- I feel the sting (Harm 1) as exhaustion fades (Fatigue 1 added).''
        \end{itemize}

  \item \textbf{Example 2 -- Heavy Armor (Full Conversion):}
        \begin{itemize}
          \item \textit{GM:} ``You take Harm 3.''
          \item \textit{You (Heavy Armor):} ``Harm 3 in Heavy Armor $\to$ Harm 0 remaining, Fatigue 2 gained.''
          \item \textit{Apply:} No Harm to apply (Fatigue does not clear), then add 2 Fatigue.
          \item \textit{Narrative:} ``My breastplate absorbs the full impact -- the blow rattles my teeth (2 Fatigue) but draws no blood.''
        \end{itemize}

  \item \textbf{Example 3 -- Heavy Armor (Excess Harm):}
        \begin{itemize}
          \item \textit{GM:} ``You take Harm 4.''
          \item \textit{You (Heavy Armor):} ``Harm 4 in Heavy Armor $\to$ Harm 1 remaining, Fatigue 2 gained.''
          \item \textit{Apply:} Clear Fatigue, Harm +=1, then add 2 Fatigue.
          \item \textit{Narrative:} ``The blow slips past my guard -- I feel the bite of the blade (Harm 1) as the impact rattles my bones (Fatigue 2 added).''
        \end{itemize}

  \item \textbf{Example 4 -- Superior Armor:}
        \begin{itemize}
          \item \textit{GM:} ``You take Harm 4.''
          \item \textit{You (Superior Armor):} ``Harm 4 in Superior Armor $\to$ Harm 0 remaining, Fatigue 3 gained.''
          \item \textit{Apply:} No Harm to apply (Fatigue does not clear), then add 3 Fatigue.
          \item \textit{Narrative:} ``The blow lands square on my chest -- the armor holds, but the force drives the air from my lungs (3 Fatigue).''
        \end{itemize}

  \item \textbf{Example 5 -- Mythic Armor:}
        \begin{itemize}
          \item \textit{GM:} ``You take Harm 5.''
          \item \textit{You (Mythic Armor):} ``Harm 5 in Mythic Armor $\to$ Harm 0 remaining, Fatigue 4 gained.''
          \item \textit{Apply:} No Harm to apply (Fatigue does not clear), then add 4 Fatigue.
          \item \textit{Narrative:} ``The hammer strikes my pauldron -- the mythic alloy sings, absorbing the blow, but my arms tremble with the effort (4 Fatigue).''
        \end{itemize}

  \item \textbf{Example 6 -- Mythic Armor (Excess Harm):}
        \begin{itemize}
          \item \textit{GM:} ``You take Harm 6.''
          \item \textit{You (Mythic Armor):} ``Harm 6 in Mythic Armor $\to$ Harm 1 remaining, Fatigue 4 gained.''
          \item \textit{Apply:} Clear Fatigue, Harm +=1, then add 4 Fatigue.
          \item \textit{Narrative:} ``The blade finds a gap in the mythic plating -- I feel the cold bite of steel (Harm 1) as the impact drives me to my knees (Fatigue 4 added).''
        \end{itemize}

  \item \textbf{Example 7 -- Overflow (Player Perspective):}
        \begin{itemize}
          \item \textit{GM:} ``You take Harm 3.''
          \item \textit{You (Heavy Armor, Body 2, Fatigue 1):} ``Harm 3 in Heavy Armor $\to$ Harm 0 remaining, Fatigue 2 gained.''
          \item \textit{Apply:} No Harm to apply (Fatigue does not clear), then add 2 Fatigue.
          \item \textit{Check Overflow:} Fatigue was 1, now 3 ($\geq$ Body 2) $\to$ Overflow! Harm +=1, clear all Fatigue.
          \item \textit{Narrative:} ``The crushing blow shatters my guard -- pain flares (Harm 1) as exhaustion vanishes (Fatigue cleared).''
        \end{itemize}
\end{itemize}

\paragraph{Key Reminders}
\begin{itemize}
  \item \textbf{Always apply Harm first} (which clears Fatigue), then add Fatigue.
  \item \textbf{Minimum Fatigue per hit is 1}, even if the table says 0.
  \item \textbf{Overflow occurs when Fatigue $\geq$ Body}, not when it exceeds.
  \item \textbf{The table is the rule.} Do not calculate beyond what it shows.
  \item \textbf{Narrate the result} -- it makes the fiction richer and keeps everyone engaged.
  \item \textbf{Higher-tier armor} (Superior, Mythic) is rare. Your GM will tell you if it is available.
\end{itemize}
