...
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,title=\textbf{Helping Others -- Assistance}]
  ...
  \begin{itemize}
    \item \textbf{Assist (Spend 1 Boon):} Give the ally +1 die on their roll.  
          ...
          Only one character may provide an Assist on a given action.
    \item \textbf{Setup (Take an Action):} Roll an appropriate skill (DV set by GM).  
          ...
          Multiple characters may use Setup on the same ally, each contributing separately.
  \end{itemize}
  \medskip
  \textbf{Total bonus limit:} The total number of extra dice granted to a single roll from all sources (Assist, Setup, talents, etc.) cannot exceed \textbf{+3}.
\end{tcolorbox}
...
\end{itemize}
A \textbf{Charge} or \textbf{Skirmish} that takes you out of an enemy's Close range provokes an opportunity attack as usual (unless you used Disengage beforehand).
