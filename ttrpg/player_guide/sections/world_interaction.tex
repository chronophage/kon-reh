...
\begin{enumerate}
\item \textbf{Set the Leg:} Name origin and destination; start a Travel Timer (4--10 segments based on difficulty).
...
\begin{description}
\item[Guide] \index{Travel!Guide} Sets course, reads signs. Rolls navigation (typically Wits + Survival); on success, tick the Travel Timer +1.
...
\begin{itemize}
\item \textbf{Assets} (see Chapter \ref{ch:assets-followers}): Spend 1 Boon to activate an asset dramatically during travel – reveal a hidden path, call for emergency aid, or secure safe lodging.
\item \textbf{Followers}: Assign followers to travel roles for bonuses. A Cap 3 Scout follower adds +3 to navigation rolls, for example. They can also take independent actions (scout ahead, secure supplies) at the cost of Exposure or Harm (see Chapter \ref{ch:assets-followers}).
...
\begin{itemize}
\item \textbf{Hazards}: Weather, terrain challenges, wildlife encounters, environmental spirits.
\item \textbf{Social}: Border checks, local politics, cultural misunderstandings, hospitality demands.
\item \textbf{Supplies}: Food shortages, equipment failure, resource management, contaminated water.
\item \textbf{Pursuit}: Being followed, hunted, or racing against time.
\item \textbf{Thin Places}: Unplanned crossings into the Ways Between (see \ref{sec:ways-between}).
\end{itemize}
