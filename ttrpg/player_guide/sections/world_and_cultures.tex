...
A smuggler’s favour or a criminal contact’s secret can be represented as a \textbf{String} (see \ref{sec:strings}) – spendable for Advantage on a relevant roll. Accepting a favour from a fence or a syndicate typically starts a \textbf{Debt Clock [4]} (see \ref{sec:terrestrial-patrons}). When the clock fills, the contact calls in the debt: a dangerous delivery, false testimony, or an assassination. Refusal turns the contact into an enemy and may advance the \textbf{Crisis} clock. GMs can also use the networks above as \textbf{Complication} sources when players roll 1s during travel: a customs officer has a tip from a rival smuggler, or the party’s cargo is “confused” with a shipment of cursed goods.
...
\begin{itemize}
  ...
  \item \textbf{Copper over Iron (2 XP):} Passive. When you visibly favour copper or brass tools over iron in fae‑facing scenes, you gain +1 die on Courtesy or Bargain rolls.
\end{itemize}
...
\paragraph{Sample Bonds:}
\begin{itemize}
  \item “A Thorn Courtier saved me from a geas – now I owe them a favour.”
