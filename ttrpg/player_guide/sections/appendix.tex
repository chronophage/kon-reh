...
\textbf{Declare Action $\rightarrow$ Build Dice Pool (Attribute + Skill) $\rightarrow$ Roll $\rightarrow$ Count Successes}
...
When tracks fill:
\begin{itemize}
    \item Fatigue overflow $\rightarrow$ increase Harm by 1, clear Fatigue.
    \item Mental Strain $\rightarrow$ causes instability or disadvantage.
    \item Harm 3 $\rightarrow$ Last Stand or Patron's Claim (see ``Optional Player-Character Death'' in \S\texttt{[MISSING REF: sec:pc-death-optional]}).
\end{itemize}
\end{tcolorbox}
...
%Carrion King & Decay Druid, Gentle Necromancer & Decay $\rightarrow$ renewal, rot as resource, harvest endings \\
