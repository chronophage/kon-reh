...
\usepackage{geometry}
≥ometry{margin=1in, columnsep=0.5in}
\usepackage{graphicx}
\usepackage{parskip}
\usepackage{tabularx}
\usepackage{booktabs}
...
≠wenvironment{example}{\color{black!70!green}\small\textbf{Example: }}{\par}

\begin{document}

\twocolumn[
¢ering
{ℎuge\bfseries Fate's Edge Quickstart}\\[0.5cm]
{łarge Your first session in 15 minutes}\\[0.3cm]
\rule{łinewidth}{0.5pt}\\[0.5cm]
]
...
\begin{itemize}
ℹtem Reference to the \textbf{2−page Cyclopedia} (core rules)
ℹtem 4 pre‑generated characters
ℹtem A complete starter adventure: \textit{The Lantern at Dusk}
ℹtem Game Master tips
ℹtem Optional simplified magic rules
\end{itemize}
No other books required.

\section*{Core Rules Reference}
The complete core rules (dice resolution, Position, Difficulty Values, Story Beats, Boons, combat, etc.) are summarised on the \textbf{Fate's Edge Player's Cyclopedia} – a free 2−page reference sheet.  
\textbf{Print it or keep it handy at the table.}  
This Quickstart assumes you have the Cyclopedia nearby; it focuses on the adventure and pre‑generated characters.

\section{Pre‑Generated Characters}
Choose one. All are balanced for the starter adventure.

\subsection*{1. Levi −− Soldier}
...
ℹtem The barrow is guarded by restless spirits (the \textbf{Duskwights}).
...
\subsection*{Timers}
\begin{itemize}
ℹtem \textbf{Barrow Collapse [6]} −− ticks when the party fails a roll inside (GM advances 1 segment per Miss). At 6, the entrance seals −− escape becomes a Desperate group action.
ℹtem \textbf{Lena's Agenda [4]} −− ticks at the end of each scene inside the barrow. When it fills, Lena has already taken the lantern; the party must chase her.
\end{itemize}
...
\textit{Miss}: They attack as a single mob (Cap 2, Harm 1 each, 2 Fatigue each on hit). Combat follows normal rules (see Cyclopedia).
...
Each PC earns \textbf{6−−10 XP} at the end. XP can be spent (see Cyclopedia for full costs):
\begin{itemize}
ℹtem Raise an Attribute: new rating $×$ 3 XP (downtime = new rating days)
ℹtem Raise a Skill: new level $×$ 2 XP (downtime = new level days)
ℹtem Learn a Minor Talent (2−−4 XP) −− examples: \textit{Keen Senses}, \textit{Second Wind}, \textit{Combat Reflexes}
ℹtem Acquire a Minor Asset (4 XP) −− safehouse, contact, or special tool.
\end{itemize}
...
\begin{itemize}
ℹtem \textbf{Start strong}: Use the cold open −− "The barrow's green light flickers. You smell rot. What do you do?"
ℹtem \textbf{Set DV to 3} by default. Only raise to 4 or 5 if the task is truly legendary.
ℹtem \textbf{Spend SB early and often}. A single SB can turn a quiet scene into a tense one.
ℹtem \textbf{Fail forward}: On a Miss, never say "nothing happens." Always introduce a new complication or advance a timer.
ℹtem \textbf{Use the pre‑gens} −− they are designed to showcase different play styles.
ℹtem \textbf{Keep the spotlight moving}. Ask quiet players directly: "Sera, what are you doing while this happens?"
ℹtem \textbf{End on a cliffhanger}. When the lantern is lifted, describe the ceiling cracking −− then say "to be continued."
\end{itemize}
...
\begin{itemize}
ℹtem 1−−2 SB: minor flare, Fatigue 1, eerie sound.
ℹtem 3+ SB: Harm 1, the spell affects an ally, or a Patron omen appears.
\end{itemize}

\subsection*{Sample Spells (DV 2−−4)}
\begin{itemize}
ℹtem \textit{Light} [Illuminate] −− DV 2. Create a small, sustained light.
ℹtem \textit{HEAL} [HEAL] −− DV 2. Heal 1 Fatigue from yourself or a touched ally.
ℹtem \textit{Shield of Air} [Ward] [Defend] −− DV 3. Gain Position +1 on next defense.
ℹtem \textit{Flame Cloak} [Burning] [Area] −− DV 3. Enemies suffer −−1 die to approach.
\end{itemize}
...
\begin{itemize}
ℹtem \textbf{Player's Guide}: Attributes, Skills, Talents, magic systems, character advancement, and the world.
ℹtem \textbf{GM Guide}: Timers, fronts, advanced Story Beat management, faction rules, and high‑tier play.
ℹtem \textbf{Adventures}: \textit{Blood and Silk} (village defense), \textit{Beyond Millhaven} (mediation), \textit{The Brass Gate} (heist).
\end{itemize}
