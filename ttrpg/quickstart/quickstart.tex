...
Success with Cost & successes $\ge$ DV, SB$\ge$1 & You do it, GM spends SB for a twist. \\
...
\begin{itemize}
\item 1 SB – minor: noise, dropped item, tick a clock.
...
\textit{Partial}: She agrees to split any reward but demands a favor later (start a Debt Clock [4]).  
...
Each failure ticks Collapse +1. If Collapse reaches 6 before you escape, the entrance seals – you must find another way (start a second Clock [4] to dig out, each success removes 1 segment; on failure, mark Fatigue).  
...
\end{itemize}
\textbf{Rush rule}: Skip downtime by accepting a \textit{Haste clock [4]}. If it fills before you finish, the new ability has a flaw (GM’s choice).
...
\item \textbf{Fail forward}: On a Miss, never say “nothing happens.” Always introduce a new complication or advance a clock.
...
\item \textit{Mend} [Mend] – DV 2. Heal 1 Fatigue from yourself or a touched ally.
...
\item \textbf{GM Guide}: Clocks, fronts, advanced Story Beat management, faction rules, and high‑tier play.
