% Fate's Edge Essential Guide
% A complete free version of the game --- 20-30 pages
% Compile with pdflatex

\documentclass[11pt]{book}
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage{geometry}
\geometry{margin=1in, headheight=14pt, includehead}
\usepackage{graphicx}
\usepackage{parskip}
\setlength{\parskip}{0.8ex plus 0.3ex minus 0.2ex}
\usepackage{booktabs}
\usepackage{longtable}
\usepackage{array}
\usepackage{xcolor}
\usepackage{tcolorbox}
\tcbuselibrary{breakable, skins}
\usepackage{titlesec}
\titleformat{\chapter}{\Huge\bfseries}{\thechapter.}{1em}{}
\titleformat{\section}{\Large\bfseries}{\thesection}{1em}{}
\titleformat{\subsection}{\large\bfseries}{\thesubsection}{1em}{}
\usepackage{fancyhdr}
\pagestyle{fancy}
\fancyhf{}
\fancyhead[LE,RO]{\thepage}
\fancyhead[RE]{\textit{\leftmark}}
\fancyhead[LO]{\textit{\rightmark}}
\renewcommand{\headrulewidth}{0.4pt}
\usepackage{makeidx}
\usepackage[colorlinks=true, linkcolor=blue!60!black, urlcolor=blue!60!black]{hyperref}

% Custom boxes
\newtcolorbox{tipbox}{colback=gray!5, colframe=gray!50, coltitle=black, title=Tip, fonttitle=\bfseries, boxrule=0.5pt, leftrule=3pt, before skip=8pt, after skip=8pt, breakable}
\newtcolorbox{examplebox}{colback=blue!5, colframe=blue!75!black, title=Example, fonttitle=\bfseries, boxrule=0.5pt, leftrule=3pt, before skip=8pt, after skip=8pt, breakable}
\newtcolorbox{warningbox}{colback=yellow!5, colframe=yellow!75!black, title=Important, fonttitle=\bfseries, boxrule=0.5pt, leftrule=3pt, before skip=8pt, after skip=8pt, breakable}
\newtcolorbox{rulesummary}{colback=green!5, colframe=green!75!black, title=Core Rule Summary, fonttitle=\bfseries, boxrule=0.5pt, leftrule=3pt, before skip=8pt, after skip=8pt, breakable}
\newtcolorbox{graybox}{colback=gray!5, colframe=gray!50, coltitle=black, fonttitle=\bfseries, boxrule=0.5pt, leftrule=3pt, before skip=8pt, after skip=8pt, breakable}

\setcounter{secnumdepth}{3}
\setcounter{tocdepth}{2}

\begin{document}
\input{titlepage}

\frontmatter
\thispagestyle{empty}
\begin{center}
  \vspace*{1in}
  {\Huge \textbf{Fate's Edge}}\\[0.5in]
  {\LARGE Essential Guide}\\[0.5in]
  {\Large A Complete Free Version of the Game}\\[0.2in]
  {\large Nicholas A. Gasper}\\[0.2in]
  {\large \today}\\[1in]
  \begin{quote}
    \itshape
    ``The coin that never spends is the one you don't remember taking.''\\
    --- Serafine of the Velvet Touch
  \end{quote}
  \vfill
\end{center}
\clearpage

\thispagestyle{empty}
\begin{center}
  \vspace*{2in}
  \textbf{Fate's Edge: Essential Guide}\\[0.2in]
  Copyright \copyright\ Nicholas A. Gasper\\[0.2in]
  This work is licensed under the Creative Commons Attribution 4.0 International License.\\
  To view a copy of this license, visit:\\
  \url{http://creativecommons.org/licenses/by/4.0/}
\end{center}
\clearpage

\tableofcontents
\clearpage

\mainmatter

% ============================================================
% CHAPTER 1: WELCOME TO FATE'S EDGE
% ============================================================
\chapter{Welcome to Fate's Edge}
\label{ch:welcome}

\section{What Is This Game?}

\textbf{Fate's Edge} is a tabletop roleplaying game where \emph{every choice carries weight}. You are not playing a board game or a tactical simulator. You are telling a story together --- a story of risk, consequence, and legacy.

In this game, you play exiles, rogues, survivors, and heroes in a world of ancient magic, fallen empires, and vibrant cultures. The world remembers what you do. Every oath, every betrayal, every desperate gamble echoes forward.

\subsection*{Core Philosophy: Narrative First}
The story is the reason the rules exist, not the other way around. When you sit down to play, you are not rolling dice to see what happens --- you are telling a story, with dice as your co-authors.

\textbf{Mechanics serve the story.} A dice roll is never just pass/fail. It changes the fictional landscape.

\begin{itemize}
  \item \textbf{Clean Success} --- The plan works as intended.
  \item \textbf{Success \& SB} --- You get what you want, but the world pushes back.
  \item \textbf{Partial} --- You make progress, but the situation remains unresolved.
  \item \textbf{Miss} --- You fail, and the situation escalates.
\end{itemize}

Every roll matters. The story never stalls; it evolves.

\subsection*{Risk as the Engine of Drama}

Fate's Edge is built on meaningful risk. Safety is boring. When your character has something to lose --- reputation, allies, ideals, life --- their actions become heroic or tragically memorable.

Your primary tool for managing risk is the \textbf{Story Beat (SB)} economy. When the dice show a 1, the world reacts. The GM gains SB to introduce complications, escalate threats, or reveal hidden dangers.

\begin{warningbox}
\textbf{Important:} SB are generated on \emph{any} roll that produces a 1, whether the overall roll succeeds or fails. They are not punishment; they are fuel for an unpredictable, responsive narrative.
\end{warningbox}

\subsection*{Characters Who Change the World}

Character growth in Fate's Edge is not about accumulating abstract power. It is meaningful growth rooted in the story. You earn \textbf{Experience Points (XP)} by engaging with challenges and complexities. You spend XP to improve capabilities, acquire assets, or unlock cultural talents.

Advancement ties directly to narrative. A character becomes a legendary commander by leading armies, not by grinding monsters. A wizard masters forbidden lore by surviving backlash.

\subsection*{Key Concepts at a Glance}

\begin{itemize}
  \item \textbf{Dice Pool:} Attribute + Skill d10s. 6+ = Success. 1 = Story Beat (SB) for the GM.
  \item \textbf{Position \& Effect:} Position (Dominant/Controlled/Desperate) defines stakes of failure. Effect (Limited/Standard/Great) describes what a clean success achieves.
  \item \textbf{Timers:} Visual trackers for ongoing challenges (e.g., a 4-segment lock, an 8-segment faction rise).
  \item \textbf{Boons:} Narrative tokens you earn by embracing risk. Spend them to re-roll dice, improve Position, or activate Assets.
\end{itemize}

\section{Safety Tools: Lines, Veils \& X-Card}

Fate's Edge explores mature themes --- conflict, loss, horror, betrayal, and difficult choices. The table must always feel safe. Use these three simple tools.

\subsection*{X-Card}
Any player (including the GM) can draw an X on a card or simply say ``X'' when a moment makes them uncomfortable. The described content is removed from the fiction --- no questions, no explanations. The scene is retconned or redirected immediately.

\subsection*{Lines}
A Line is a hard boundary: content that will not appear in the game at all. Examples: ``No sexual violence,'' ``No harm to children,'' ``No body horror.'' Players name their Lines before the campaign; the group honors them.

\subsection*{Veils}
A Veil is a soft boundary: content that can happen ``off screen'' or fade to black. The event is acknowledged but not described. Example: ``A romantic scene can happen, but fade to black before it becomes explicit.''

\subsection*{How to Use These Tools}
\begin{itemize}
  \item Establish Lines and Veils in Session Zero. Write them down where everyone can see them.
  \item Keep an X-Card visible on the table.
  \item Remind players they can use the X-Card at any time.
  \item When the X-Card is used, pause, rewind if needed, and move on. Do not interrogate.
  \item The GM is also a player; they may use the X-Card for their own comfort.
\end{itemize}

These tools are not about censorship --- they are about consent. They allow the table to tell darker, more intense stories because everyone knows they can tap out.

\section{Session Zero: The Foundation of Play}

Before your first dice roll, gather for a Session Zero. This is where you build the shared understanding that makes Fate's Edge sing.

\subsection*{Session Zero Checklist}
\begin{enumerate}
  \item \textbf{Lines \& Veils:} What content is off-limits (Lines) or only implied (Veils)?
  \item \textbf{Tone:} Cinematic action? Grim survival? Political intrigue? Dark horror?
  \item \textbf{Campaign length:} One-shot, short arc (3--6 sessions), or long campaign?
  \item \textbf{Character hooks:} Each player shares one goal and one fear for their character.
  \item \textbf{Starting situation:} Where does the story begin?
  \item \textbf{Scheduling:} How often do you meet? What happens if a player misses?
\end{enumerate}

Taking thirty minutes for Session Zero will save hours of mid-campaign friction.

\clearpage

% ============================================================
% CHAPTER 2: CORE MECHANICS
% ============================================================
\chapter{Core Mechanics}
\label{ch:core-mechanics}

\begin{rulesummary}
\textbf{The Core Loop in 90 Seconds}
\begin{enumerate}
  \item Player declares intent and approach (Attribute + Skill).
  \item GM sets Position (Dominant/Controlled/Desperate) and Difficulty (DV 2-5+).
  \item Player rolls dice pool (Attribute + Skill d10s).
  \item Count Successes (6+ = success, 10 = 2 successes) and Story Beats (1s).
  \item Compare successes to DV using the Outcome Matrix.
  \item GM spends Story Beats (SB) for complications.
  \item Player marks resources (Boons, Fatigue, Harm).
\end{enumerate}
\end{rulesummary}

\section{The Resolution Loop}

Every meaningful action follows this simple cycle:

\subsection*{1. Declare Intent and Approach}
The player states what they want to achieve and how (Attribute + Skill). Example: \textit{``I want to sneak past the guard (Wits + Stealth).''}

\subsection*{2. Set Difficulty Value (DV)}
The GM sets the target number of successes needed.

\begin{center}
\begin{tabular}{ll}
\toprule
\textbf{DV} & \textbf{Situation} \\
\midrule
2 & Routine, no pressure, almost guaranteed. \\
3 & Default --- mild opposition, some risk. \\
4 & Hard --- serious opposition or bad angle. \\
5+ & Extreme --- major boss or dramatic gamble. \\
\bottomrule
\end{tabular}
\end{center}

\subsection*{3. Set Position}
Position tells you how risky the action is and changes how dice behave.

\begin{center}
\begin{tabular}{lll}
\toprule
\textbf{Position} & \textbf{Narrative Frame} & \textbf{Mechanical Effect} \\
\midrule
Dominant & You press advantage & Re-roll one failure. \\
Controlled & Balanced, normal & No re-rolls. \\
Desperate & You act under duress & Re-roll one success. \\
\bottomrule
\end{tabular}
\end{center}

You can spend 1 Boon to improve Position by one step before rolling.

\subsection*{4. Roll the Dice}
Roll Attribute + Skill d10s.

\begin{itemize}
  \item Each die showing 6-9 = 1 success.
  \item Each die showing 10 = 2 successes (critical).
  \item Each die showing 1 = 1 Story Beat (SB) for the GM.
\end{itemize}

\subsection*{5. Apply the Outcome}
Compare successes to DV:

\begin{center}
\begin{tabular}{lll}
\toprule
\textbf{Roll Result} & \textbf{Outcome} & \textbf{What Happens} \\
\midrule
S $\ge$ DV, SB = 0 & Clean Success & Intent achieved; no complication. \\
S $\ge$ DV, SB > 0 & Success with SB & Intent achieved; GM spends SB for complication. \\
0 < S < DV & Partial & Progress made; player gains 1 Boon. \\
S = 0 & Miss & No progress; player gains 2 Boons; GM escalates. \\
\bottomrule
\end{tabular}
\end{center}

\section{Boons (Player Resource)}

Boons represent momentum and narrative leverage. You may hold up to 5 Boons.

\subsection*{Earning Boons}
\begin{itemize}
  \item \textbf{Partial:} Gain 1 Boon.
  \item \textbf{Miss:} Gain 2 Boons.
  \item \textbf{Bond-driven action:} Once per bond per session, gain 1 Boon.
  \item \textbf{GM award:} For creativity, sacrifice, or strong narrative play.
\end{itemize}

\subsection*{Spending Boons}
\begin{itemize}
  \item Re-roll a single die (after seeing the result).
  \item Activate an on-screen Asset (1 Boon).
  \item Improve Position by one step before a roll.
  \item Power certain Rites or abilities.
  \item Convert 2 Boons $\rightarrow$ 1 XP (once per session, max 2 XP via conversion).
\end{itemize}

\subsection*{Limits}
\begin{itemize}
  \item Maximum 5 Boons held.
  \item At end of each scene, reduce held Boons to maximum 2 (excess lost).
  \item At most 2 Boons from failures per character per scene.
\end{itemize}

\section{Story Beats (GM Resource)}

Every time a player rolls a 1, the GM gains a Story Beat (SB). Spend SB to introduce complications or escalate pressure.

\begin{center}
\begin{tabular}{ll}
\toprule
\textbf{SB Cost} & \textbf{Effect} \\
\midrule
1 SB & Minor: noise, trace, tick a timer +1. \\
2 SB & Moderate: alarm raised, lose Position, lesser foe appears. \\
3+ SB & Major: reinforcements, scene shifts, break an asset. \\
\bottomrule
\end{tabular}
\end{center}

\begin{tipbox}
\textbf{Be aggressive with Story Beats.} Unused pressure is wasted momentum. One strong beat is better than many small penalties but you have to build towards them.
\end{tipbox}

\section{Fatigue and Harm}

\subsection*{Fatigue}
Each character has a Fatigue track equal to their Body attribute.

\begin{itemize}
  \item Each point of Fatigue worsens your Position by one step (Dominant $\rightarrow$ Controlled $\rightarrow$ Desperate).
  \item If already Desperate, each additional Fatigue imposes --1 die.
  \item When Fatigue track fills, increase Harm by 1 and clear all Fatigue.
\end{itemize}

\subsection*{Harm}
Harm represents serious injury.

\begin{center}
\begin{tabular}{ll}
\toprule
\textbf{Harm Level} & \textbf{Effect} \\
\midrule
Harm 1 & --1 die on related actions. \\
Harm 2 & --1 die on most actions. \\
Harm 3 & Incapacitated or dying. \\
\bottomrule
\end{tabular}
\end{center}

\subsection*{Armor Conversion}
Armor does not reduce Harm directly. Instead, it converts Harm to Fatigue.

\begin{center}
\begin{tabular}{llll}
\toprule
\textbf{Armor Type} & \textbf{Harm 1} & \textbf{Harm 2} & \textbf{Harm 3} \\
\midrule
Light & Fatigue 1 & Harm 1 + Fatigue 1 & Harm 2 + Fatigue 1 \\
Medium & Fatigue 1 & Fatigue 1 & Harm 2 + Fatigue 1 \\
Heavy & Fatigue 1 & Fatigue 1 & Fatigue 2 \\
\bottomrule
\end{tabular}
\end{center}

\textit{Minimum 1 Fatigue per hit, even if conversion would give 0.}

\subsection*{Recovery}
\begin{itemize}
  \item \textbf{Fatigue:} Short rest (1 hour) removes 2 Fatigue. Full night's rest removes all.
  \item \textbf{Harm:} Requires medical care (Medicine roll) or magical healing. Natural recovery takes one week per level.
\end{itemize}

\subsection*{Timers}

Timers are visual trackers for ongoing challenges and accumulating pressure.

\subsubsection*{Timer Sizes}
\begin{itemize}
  \item \textbf{4 segments:} Short-term pressure (a guard patrol arrives, a door breaks).
  \item \textbf{6 segments:} Standard scene timer (a ritual completes, a rival escapes).
  \item \textbf{8+ segments:} Multi-scene or campaign timer (a faction consolidates power).
\end{itemize}

\subsubsection*{When to Tick a Timer}
\begin{itemize}
  \item On a Partial or Miss.
  \item When you spend Story Beats (1 SB = 1 tick typically).
  \item When the fiction demands it: \textit{``While you argue, the fire spreads.''}
\end{itemize}

\subsubsection*{When a Timer Fills}
The defined event happens. The event should be visible and change the situation.

\textbf{Examples:}
\begin{itemize}
  \item A \textbf{Guard Patrol [4]} timer fills: guards arrive.
  \item A \textbf{Ritual Completion [6]} timer fills: the ritual finishes.
  \item An \textbf{Alliance Forming [8]} timer fills: a faction commits to action.
\end{itemize}

\begin{tipbox}
\textbf{Timers are visible.} Show them to the players. Write \textit{Cult Influence [6]} on an index card and place it on the table. Tick it aloud: \textit{``The cult's chant grows louder --- the ritual advances.''}
\end{tipbox}

\clearpage

% ============================================================
% CHAPTER 3: CHARACTER CREATION
% ============================================================
\chapter{Character Creation}
\label{ch:character-creation}

\section{Overview}

Characters begin at Tier I (Novice) with \textbf{32 Experience Points (XP)}. You may gain up to +4 XP by taking meaningful Bonds (with other player characters) and Complications.

\section{Step 1: Concept}

Write one sentence that describes your character's origin, profession, and one defining trait.

Examples:
\begin{itemize}
  \item \textit{``An Aeler tunnel-wright who lost her breath-count and now hears stone sing.''}
  \item \textit{``A Vhasian duelist with a scarred face and a vow to never lose again.''}
\end{itemize}

\section{Step 2: Attributes}

You have four Attributes (1-5). Spend XP to raise them from a base of 1 each.

\begin{center}
\begin{tabular}{lll}
\toprule
\textbf{Attribute} & \textbf{Description} & \textbf{Cost} \\
\midrule
Body & Strength, endurance, physical resilience. & new rating $\times$ 3 XP \\
Wits & Perception, speed, quick thinking. & new rating $\times$ 3 XP \\
Spirit & Willpower, focus, connection to the intangible. & new rating $\times$ 3 XP \\
Presence & Charisma, command, force of personality. & new rating $\times$ 3 XP \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Cost is incremental:}
\begin{itemize}
  \item 1 $\to$ 2 costs 6 XP (2 $\times$ 3)
  \item 2 $\to$ 3 costs 9 XP (3 $\times$ 3)
  \item 3 $\to$ 4 costs 12 XP (4 $\times$ 3)
\end{itemize}

\textbf{Recommended starting ranges:}
\begin{itemize}
  \item Primary attribute (used most): \textbf{3} (costs 15 XP total: 1$\to$2 = 6, 2$\to$3 = 9)
  \item Secondary attributes: \textbf{2} each (costs 6 XP each)
\end{itemize}

\section{Step 3: Skills}

Choose Skills (0-5) that match your concept. Key skills should be 2-3; others 0-1.

\begin{itemize}
  \item Cost: \textbf{new level $\times$ 2 XP} (incremental)
  \item Recommended starting total: Spend 8-12 XP on skills.
\end{itemize}

\textbf{Cost is incremental:}
\begin{itemize}
  \item 0 $\to$ 1 costs 2 XP (1 $\times$ 2)
  \item 1 $\to$ 2 costs 4 XP (2 $\times$ 2)
  \item 2 $\to$ 3 costs 6 XP (3 $\times$ 2)
\end{itemize}

\subsection*{Core Skill List}
\begin{multicols}{2}
  \textbf{Combat:}
  \begin{itemize}
    \item Melee
    \item Ranged
    \item Brawl
    \item Tactics
  \end{itemize}
  \textbf{Physical:}
  \begin{itemize}
    \item Athletics
    \item Stealth
    \item Endurance
    \item Craft
    \item Survival
  \end{itemize}
  \textbf{Social:}
  \begin{itemize}
    \item Sway
    \item Command
    \item Deception
    \item Performance
    \item Insight
  \end{itemize}
  \textbf{Knowledge:}
  \begin{itemize}
    \item Lore
    \item Investigation
    \item Medicine
  \end{itemize}
  \textbf{Magic:}
  \begin{itemize}
    \item Arcana
    \item Ritual
  \end{itemize}
\end{multicols}

\section{Step 4: Talents}

\begin{tipbox}
\textbf{You don't need Talents to be effective.} A character with just Attributes and Skills is a complete, viable character---especially at Tier I.
\end{tipbox}

Talents are special abilities. Start with \textbf{0--3 talents}.

\subsection*{Sample Talents}

\begin{center}
\begin{tabular}{lll}
\toprule
\textbf{Talent} & \textbf{Cost} & \textbf{Effect} \\
\midrule
\textit{None} & \textit{0 XP} & \textit{A valid, complete character.} \\
Keen Senses & 2 XP & +1 die to perception checks. \\
Weapon Mastery & 5 XP & +2 dice with a chosen weapon. \\
Second Wind & 2 XP & Clear 1 Fatigue once per scene. \\
Silver Tongue & 2 XP & +1 die to persuasion attempts. \\
Command Presence & 4 XP & +1 die to leadership and intimidation. \\
Defensive Survival & 3 XP & +1 to defense DV in melee. \\
\bottomrule
\end{tabular}
\end{center}

\section{Step 5: Bonds and Complications}

\subsection*{Bonds}
Establish 1-2 bonds with other player characters. Each bond grants a once-per-session Boon when you act on it with intricate description.

\textbf{Bond Example:} \textit{``She helped me escape the trial that ruined my family.''}

\subsection*{Complications}
You may take up to two complications (e.g., a feud, a cursed item, a debt). Each gives +2 XP but adds +1 banked Story Beat to early scenes.

\textbf{Complication Example:} \textit{``The magistrate who condemned my family still hunts me.''}

\section{Step 6: Gear}

You begin with:
\begin{itemize}
  \item One set of clothing appropriate to your culture.
  \item A melee weapon (Light or Medium) or a ranged weapon with ammunition.
  \item Light armor (or Medium if your concept demands it).
  \item A backpack, waterskin, 1d6 days of rations, a knife, flint and steel, a small lantern or candle.
  \item Any tools required for your skills (e.g., lockpicks, healer's kit).
\end{itemize}

\section{Build Examples}

\subsection*{The Duelist (32 XP)}
\begin{itemize}
  \item Body 3 (15 XP), Wits 2 (6 XP) $\rightarrow$ 21 XP
  \item Melee 2 (6 XP), Athletics 1 (2 XP), Endurance 1 (2 XP) $\rightarrow$ 10 XP
  \item \textbf{Total:} 31 XP (bank 1 XP)
\end{itemize}
\textit{Add a Bond for +2 XP to reach 33 XP.}

\subsection*{The Scholar (32 XP)}
\begin{itemize}
  \item Wits 3 (15 XP), Spirit 2 (6 XP) $\rightarrow$ 21 XP
  \item Lore 2 (6 XP), Investigation 1 (2 XP), Insight 1 (2 XP) $\rightarrow$ 10 XP
  \item \textbf{Total:} 31 XP (bank 1 XP)
\end{itemize}
\textit{Add a Bond for +2 XP to reach 33 XP.}

\subsection*{The Scout (32 XP)}
\begin{itemize}
  \item Wits 3 (15 XP), Body 2 (6 XP) $\rightarrow$ 21 XP
  \item Stealth 2 (6 XP), Survival 1 (2 XP) $\rightarrow$ 8 XP
  \item \textbf{Total:} 29 XP (bank 3 XP)
\end{itemize}
\textit{Add Keen Senses (2 XP) and a Bond (+2 XP) for 33 XP.}

\clearpage

% ============================================================
% CHAPTER 4: PRE-GENERATED CHARACTERS
% ============================================================
\chapter{Pre-Generated Characters}
\label{ch:pregens}

Choose one. All are balanced for the starter adventure. Each PC starts with \textbf{3 Boons} and \textbf{0 Fatigue/Harm}.

\section*{1. Levi --- Soldier}

\textit{Once a legionary, now a sellsword.}

\begin{tabular}{ll}
\textbf{Body} & 3 \\
\textbf{Wits} & 2 \\
\textbf{Spirit} & 1 \\
\textbf{Presence} & 1 \\
\end{tabular}

\textbf{Skills:} Melee 2, Athletics 1, Endurance 1, Command 1

\textbf{Talent --- Battle Instincts:} Once per scene, reroll a failed defense roll.

\textbf{Asset --- Trusted Blade:} Your sword never breaks on a Miss.

\textbf{Bonds:} (choose one with another PC) \textit{``I saved your life outside Millhaven.''}

\textbf{XP:} Body 3 (15), Wits 2 (6) = 21 XP. Melee 2 (6), Athletics 1 (2), Endurance 1 (2), Command 1 (2) = 12 XP. Battle Instincts (2 XP). \textbf{Total: 35 XP} (with one Bond).

\section*{2. Lyra --- Scholar}

\textit{Expelled from the Academy for forbidden research.}

\begin{tabular}{ll}
\textbf{Wits} & 3 \\
\textbf{Spirit} & 2 \\
\textbf{Body} & 1 \\
\textbf{Presence} & 1 \\
\end{tabular}

\textbf{Skills:} Lore 2, Arcana 1, Investigation 1, Insight 1

\textbf{Talent --- Lorekeeper:} Once per session, recall obscure history without rolling.

\textbf{Asset --- Scholar's Satchel:} Contains books, reagents, a small telescope.

\textbf{Bonds:} \textit{``We both lost our place in Silkstrand.''}

\textbf{XP:} Wits 3 (15), Spirit 2 (6) = 21 XP. Lore 2 (6), Arcana 1 (2), Investigation 1 (2), Insight 1 (2) = 12 XP. Lorekeeper (2 XP). \textbf{Total: 35 XP} (with one Bond).

\section*{3. Sera --- Shadow}

\textit{Former guild thief, now a wanderer.}

\begin{tabular}{ll}
\textbf{Wits} & 3 \\
\textbf{Body} & 2 \\
\textbf{Spirit} & 1 \\
\textbf{Presence} & 1 \\
\end{tabular}

\textbf{Skills:} Stealth 2, Subterfuge 2, Streetwise 1

\textbf{Talent --- Silver Tongue:} +1 die to persuade or deceive through speech.

\textbf{Asset --- Shadow's Cloak:} +1 die to stealth rolls in dim light.

\textbf{Bonds:} \textit{``You kept my secret when the guards came.''}

\textbf{XP:} Wits 3 (15), Body 2 (6) = 21 XP. Stealth 2 (6), Subterfuge 2 (6), Streetwise 1 (2) = 14 XP. Silver Tongue (2 XP). \textbf{Total: 37 XP} (with one Bond --- adjust by dropping Streetwise to 0 for 35 XP).

\section*{4. Mira --- Healer}

\textit{Accused of witchcraft for curing the incurable.}

\begin{tabular}{ll}
\textbf{Spirit} & 3 \\
\textbf{Wits} & 2 \\
\textbf{Body} & 1 \\
\textbf{Presence} & 1 \\
\end{tabular}

\textbf{Skills:} Medicine 2, Survival 1, Insight 2

\textbf{Talent --- Iron Stomach:} Immune to mundane poisons and spoiled food.

\textbf{Asset --- Healer's Kit:} Bandages, herbs, basic medical supplies.

\textbf{Bonds:} \textit{``You trusted me when no one else would.''}

\textbf{XP:} Spirit 3 (15), Wits 2 (6) = 21 XP. Medicine 2 (6), Survival 1 (2), Insight 2 (6) = 14 XP. Iron Stomach (3 XP). \textbf{Total: 38 XP} (with one Bond --- adjust: drop Insight to 1 for 35 XP).

\section*{5. Kael --- Runekeeper (Optional Magic User)}

\textit{A wanderer sworn to the Traveler.}

\begin{tabular}{ll}
\textbf{Spirit} & 3 \\
\textbf{Wits} & 2 \\
\textbf{Body} & 1 \\
\textbf{Presence} & 1 \\
\end{tabular}

\textbf{Skills:} Lore 2, Arcana 1, Survival 1, Insight 1

\textbf{Talents:} Familiar (2 XP), Codex (4 XP)

\textbf{Rites:} Road-Sense (4 XP), Traveler's Boon (5 XP)

\textbf{Bonds:} \textit{``The road brought us together.''}

\textbf{XP:} Spirit 3 (15), Wits 2 (6) = 21 XP. Lore 2 (6), Arcana 1 (2), Survival 1 (2), Insight 1 (2) = 12 XP. Familiar (2), Codex (4) = 6 XP. Road-Sense (4), Traveler's Boon (5) = 9 XP. \textbf{Total: 48 XP} (requires GM adjustment or starting at Tier II).

\clearpage

% ============================================================
% CHAPTER 5: STARTER ADVENTURE --- THE LANTERN AT DUSK
% ============================================================
\input{copyright}
\chapter{Starter Adventure: The Lantern at Dusk}
\label{ch:lantern-at-dusk}

\section{Setup}

The party is hired by \textbf{Elder Sarai} of the remote village of \textbf{Duskwood}. A terrible blight is spreading from the old barrow in the hills --- the same barrow where a cursed lantern was buried centuries ago. The lantern must be brought to a stone circle in the valley to break the curse. But the barrow is not empty.

\section{What the Party Knows}
\begin{itemize}
  \item The lantern is in the central chamber of the barrow.
  \item The barrow is guarded by restless spirits (the \textbf{Duskwights}).
  \item A rival treasure hunter, \textbf{Quick Lena}, has also entered the barrow.
  \item The curse will kill the village's crops in three days.
\end{itemize}

\section{Timers}

\begin{itemize}
  \item \textbf{Barrow Collapse [6]} --- ticks when the party fails a roll inside (GM advances 1 segment per Miss). At 6, the entrance seals --- escape becomes a Desperate group action.
  \item \textbf{Lena's Agenda [4]} --- ticks at the end of each scene inside the barrow. When it fills, Lena has already taken the lantern; the party must chase her.
\end{itemize}

\section{Scene 1 --- The Entry}

The barrow entrance is a narrow stone passage. A faint green glow pulses from deeper inside.

\textbf{Obstacle:} A cave-in blocks the way.

\begin{itemize}
  \item \textbf{Roll:} Body + Athletics (DV 3, Controlled).
  \item \textbf{Clean Success:} You clear the rubble and enter.
  \item \textbf{Partial:} You get through but disturb rubble --- tick Barrow Collapse +1.
  \item \textbf{Miss:} You are pinned by a rock --- mark Harm 1 and tick Collapse +1.
\end{itemize}

\section{Scene 2 --- The Spirit Corridor}

The passage widens into a hall where three \textbf{Duskwights} drift. They do not attack unless provoked, but they will not let anyone pass without an offering.

\textbf{Offer a memory or a secret.}

\begin{itemize}
  \item \textbf{Roll:} Presence + Sway (DV 3, Controlled).
  \item \textbf{Clean Success:} They bow and let you pass.
  \item \textbf{Partial:} They take your offering but also steal a random piece of equipment (GM chooses).
  \item \textbf{Miss:} They attack as a single mob (Cap 2, Harm 1 each, 2 Fatigue each on hit). Combat follows normal rules.
\end{itemize}

\section{Scene 3 --- The Lantern Chamber}

The lantern sits on a stone altar. \textbf{Quick Lena} is there, trying to pry it loose.

\textbf{Confront Lena:} She is not a fighter.

\begin{itemize}
  \item \textbf{Roll:} Presence + Sway (DV 3, Controlled) to offer a deal or intimidate her.
  \item \textbf{Clean Success:} She leaves peacefully, but warns that ``others will come for it.''
  \item \textbf{Partial:} She agrees to split any reward but demands a favor later (start a Debt Timer [4]).
  \item \textbf{Miss:} She triggers a trap --- a stone door starts closing (tick Collapse +2).
\end{itemize}

\section{Scene 4 --- Escape}

After you take the lantern, the barrow begins to collapse in earnest.

\textbf{Group Skill Challenge:} The party needs 3 successes before 2 failures to escape. Each PC describes how they help (e.g., Athletics to brace a door, Lore to read ancient escape runes).

\begin{itemize}
  \item \textbf{Roll:} Appropriate Attribute + Skill (DV 3, Position Desperate).
  \item Each failure ticks Collapse +1.
  \item If Collapse reaches 6 before you escape, the entrance seals --- you must find another way (start a second Timer [4] to dig out, each success removes 1 segment; on failure, mark Fatigue).
\end{itemize}

\textbf{Success:} You burst into the moonlight, lantern in hand.

\textbf{Failure:} You escape, but someone is left behind --- captured or lost. The remaining party must rescue them later.

\section{Resolution}

Return the lantern to the stone circle. Breaking the curse requires a simple ritual.

\begin{itemize}
  \item \textbf{Roll:} Spirit + Lore (DV 2).
  \item \textbf{Clean Success:} The blight lifts. Elder Sarai rewards each PC with a \textbf{String} --- ``The village owes you.''
  \item \textbf{Partial:} The curse lifts, but the lantern cannot be destroyed --- it will attract another danger soon.
  \item \textbf{Miss:} The curse is only pushed back one year. A dark spirit escapes and will become a recurring enemy.
\end{itemize}

\section{Advancement}

Each PC earns \textbf{6--10 XP} at the end. XP can be spent:

\begin{itemize}
  \item Raise an Attribute: new rating $\times$ 3 XP (downtime = new rating days).
  \item Raise a Skill: new level $\times$ 2 XP (downtime = new level days).
  \item Learn a Minor Talent (2--4 XP) --- examples: \textit{Keen Senses}, \textit{Second Wind}, \textit{Combat Reflexes}.
  \item Acquire a Minor Asset (4 XP) --- safehouse, contact, or special tool.
\end{itemize}

\textbf{Rush Rule:} Skip downtime by accepting a \textit{Haste timer [4]}. If it fills before you finish, the new ability has a flaw (GM's choice).

\clearpage

% ============================================================
% CHAPTER 6: SIMPLIFIED MAGIC
% ============================================================
\chapter{Simplified Magic}
\label{ch:simplified-magic}

This chapter covers two approaches to magic: \textbf{Free Casting} for players who want to improvise spells, and \textbf{Hedge Gifts} for players who want reliable, low-cost magical tricks without committing to a full magical path.

\section{Free Casting (Spellcraft)}

If a player wants an improvisational magical character, use these simplified rules (based on the \textit{Free Caster} path). For full magic systems (Runekeepers, Invokers, Cantors), see the complete \textit{Fate's Edge Player's Guide}.

\subsection*{Spellcasting}

Choose a magical \textbf{Element} (Fire, Water, Earth, Air, Fate, Life, Luck, Death). Describe your spell using \textbf{TAGS}. Each tag increases DV by 1. Dangerous tags (Teleport, Transform, Dominate) increase DV by +2 instead.

\[
\text{DV} = 1 + \text{number of tags}
\]

(maximum 6). Roll \textbf{Spirit + Arcana}.

\subsection*{Example}
\textit{``I throw a [Burning] [Area] blast''} = DV 3.

\subsection*{Backlash}

If you roll a Miss or the GM spends SB, magic backfires:

\begin{itemize}
  \item \textbf{1--2 SB:} minor flare, Fatigue 1, eerie sound.
  \item \textbf{3+ SB:} Harm 1, the spell affects an ally, or a Patron omen appears.
\end{itemize}

\subsection*{Sample Spells (DV 2--4)}

\begin{center}
\begin{tabular}{lll}
\toprule
\textbf{Spell} & \textbf{TAGS} & \textbf{DV} \\
\midrule
Light & [Illuminate] & 2 \\
HEAL & [HEAL] & 2 \\
Shield of Air & [Ward] [Defend] & 3 \\
Flame Cloak & [Burning] [Area] & 3 \\
Teleport & [Leap] [Self] & 4 \\
\bottomrule
\end{tabular}
\end{center}

\subsection*{Light [Illuminate] --- DV 2}
Create a small, sustained light.

\subsection*{HEAL [HEAL] --- DV 2}
Heal 1 Fatigue from yourself or a touched ally.

\subsection*{Shield of Air [Ward] [Defend] --- DV 3}
Gain Position +1 on next defense.

\subsection*{Flame Cloak [Burning] [Area] --- DV 3}
Enemies suffer --1 die to approach.

\section{Hedge Gifts (Simplified Witchcraft)}

Hedge Gifts are simple, no-roll magical abilities. A character with the \textbf{Craft of the Hedge} talent learns two Hedge Gifts of their choice. These are small, reliable effects that never require dice rolls and never incur Obligation.

\subsection*{Requirements}
\begin{itemize}
  \item \textbf{Craft of the Hedge (4 XP):} Requires Lore 1+ or Survival 1+.
  \item You learn \textbf{two Hedge Gifts} of your choice.
\end{itemize}

\subsection*{Hedge Gifts}

\begin{longtable}{@{}p{3.5cm}p{8cm}@{}}
\toprule
\textbf{Gift} & \textbf{Effect} \\
\midrule
\endhead
\bottomrule
\endfoot
\textbf{Steady Hand} & Once per scene, touch a wounded person (including yourself) and remove 1 Fatigue. No roll. May not be used on the same person twice in a scene. \\
\textbf{Knot of Favor} & Once per scene, tie a knot in a cord, thread, or cloth. Allies within Near gain +1 die to a single type of roll (choose when tying: stealth, persuasion, defense, or navigation). The knot must remain tied. \\
\textbf{Warm Hand} & Once per scene, touch someone. Their next physical action either gains +1 die (comfort) or suffers --1 die (warning). No roll. \\
\textbf{Hearth-Sense} & Once per scene, ask the GM one yes/no question about a threshold, boundary, or promise in the current scene. The GM must answer truthfully. \\
\textbf{Salt Line} & Once per scene, pour a handful of salt in a continuous line (move action). Any spirit, ghost, or cursed creature attempting to cross must test Spirit+Resolve (DV 3) or be stopped for that exchange. \\
\textbf{The Unlit Candle} & Once per session, declare that a light source you can see (lantern, candle, torch) falters or extinguishes. This creates a zone of dim light or darkness in Near range for one exchange. \\
\bottomrule
\end{longtable}

\subsection*{Example Build: The Hedge-Witch (32 XP)}

\begin{itemize}
  \item \textbf{Attributes:} Spirit 3 (15 XP), Wits 2 (6 XP) $\rightarrow$ 21 XP
  \item \textbf{Skills:} Lore 2 (6 XP), Insight 1 (2 XP) $\rightarrow$ 8 XP
  \item \textbf{Talent:} Craft of the Hedge (4 XP) $\rightarrow$ 4 XP
  \item \textbf{Hedge Gifts:} Steady Hand, Knot of Favor
  \item \textbf{Total:} 21 + 8 + 4 = \textbf{33 XP}
\end{itemize}

\subsection*{Promise Timers}

Some Hedge Gift effects or rituals create obligations. Represent these with a \textbf{Promise Timer} (typically 4 segments):

\begin{itemize}
  \item Mark the timer when the promise is ignored, deferred, or disrespected.
  \item When filled, the price comes due --- socially, spiritually, or narratively.
\end{itemize}

\clearpage

% ============================================================
% CHAPTER 7: GM TIPS --- RUNNING YOUR FIRST GAME
% ============================================================
\chapter{GM Tips: Running Your First Game}
\label{ch:gm-tips}

\section{Before You Start}

\subsection*{Read the Adventure First}
Read through \textit{The Lantern at Dusk} (Chapter 5) at least twice. You don't need to memorize it, but you should know the major scenes and timers.

\subsection*{Know the Core Rules}
The core mechanics (Chapter 2) are your engine. If you forget a rule, default to:
\begin{itemize}
  \item DV = 3
  \item Position = Controlled
  \item Effect = Standard
\end{itemize}
You can look up the rule after the session.

\subsection*{Print the Reference Sheet}
Keep the Quick Reference (Chapter 9) handy at the table.

\section{During the Session}

\subsection*{Start Strong}
Use a cold open: \textit{``The barrow's green light flickers. You smell rot. What do you do?''}

\subsection*{Set DV to 3 by Default}
Only raise to 4 or 5 if the task is truly legendary. Your job is not to challenge the players with high numbers --- it is to challenge them with interesting choices.

\subsection*{Spend Story Beats Early and Often}
A single SB can turn a quiet scene into a tense one. Don't hoard them.

\subsection*{Fail Forward}
On a Miss, never say ``nothing happens.'' Always introduce a new complication or advance a timer.

\subsection*{Use the Pre-Gens}
The pre-generated characters (Chapter 4) are designed to showcase different play styles. Let players choose one.

\subsection*{Keep the Spotlight Moving}
Ask quiet players directly: \textit{``Sera, what are you doing while this happens?''}

\subsection*{End on a Cliffhanger}
When the lantern is lifted, describe the ceiling cracking --- then say ``to be continued.''

\subsection*{Using Timers at the Table}

Timers are your primary tool for creating pressure. Use them visibly and consistently.

\begin{itemize}
  \item \textbf{Name them descriptively:} \textit{``Ritual Completion''} not \textit{``Timer 3''}.
  \item \textbf{Tick them aloud:} \textit{``The cult's chant grows louder --- Ritual Completion advances to 4 of 6.''}
  \item \textbf{Link timer advances to fiction:} Never just say \textit{``tick timer.''} Describe what changes.
  \item \textbf{Place trackers centrally:} Use index cards, poker chips, or dry-erase boards where all players can see them.
\end{itemize}

\subsection*{The Three-Timer Rule}
At any given time, maintain at most three active timers in a scene. More creates cognitive overload.

\begin{itemize}
  \item \textbf{Situation Timer:} Tracks progress toward the scene's immediate goal.
  \item \textbf{Counter-Clock:} Tracks opposition pressure.
  \item \textbf{Campaign Timer:} Tracks long-term front pressure.
\end{itemize}

\section{Common Pitfalls and Fixes}

\begin{center}
\begin{tabular}{p{4cm}p{7cm}}
\toprule
\textbf{Pitfall} & \textbf{Fix} \\
\midrule
Holding onto SB for a ``big moment'' & Spend SB early. Pressure creates drama. \\
Setting DV too high (4-5 for routine) & Default to DV 3. Use Position for tension. \\
Not knowing a rule & Make a ruling that keeps the story moving. \\
Players get stuck & Ask: \textit{``What's your goal?''} Then name the simplest obstacle. \\
\bottomrule
\end{tabular}
\end{center}

\section{Handling Player Creativity}

\begin{itemize}
  \item \textbf{Say ``Yes, but'' not ``No.''} Offer a cost, a timer, or a complication.
  \item \textbf{Use Setup and Assist actions.} Let non-specialists contribute meaningfully.
  \item \textbf{Embrace unexpected solutions.} If a player bypasses a planned encounter, let it work --- then escalate somewhere else.
  \item \textbf{Fail forward.} A missed roll should never stop the story; it should change direction.
\end{itemize}

\section{After the Session}

\begin{itemize}
  \item Award XP (6--10 XP per player).
  \item Advance or clear active timers.
  \item Ask players: \textit{``One thing you loved, one thing you want more of.''}
  \item Note any rulings you made to reconcile with rules text later.
\end{itemize}

\clearpage

% ============================================================
% CHAPTER 8: WHAT COMES NEXT
% ============================================================
\chapter{What Comes Next}
\label{ch:next-steps}

\section{Ready for More?}

This Essential Guide gives you a complete first session. For the full experience:

\begin{itemize}
  \item \textbf{Player's Guide:} Attributes, Skills, Talents, magic systems, character advancement, and the world.
  \item \textbf{GM Guide:} Timers, fronts, advanced Story Beat management, faction rules, and high-tier play.
  \item \textbf{Adventures:} \textit{Blood and Silk} (village defense), \textit{Beyond Millhaven} (mediation), \textit{The Brass Gate} (heist).
\end{itemize}

All available under CC-BY from the author.

\section{Where to Go Next}

\subsection*{Build Your Own Character}
Use the character creation rules in Chapter 3 to build a custom character. Experiment with different Attributes, Skills, and Talents.

\subsection*{Run Your Own Adventure}
Use the framework from Chapter 5 to design your own adventures:
\begin{itemize}
  \item Define a problem (the blight, the cursed lantern).
  \item Define a location (the barrow).
  \item Define a rival (Quick Lena).
  \item Set timers (Barrow Collapse, Lena's Agenda).
  \item Let the players choose their path.
\end{itemize}

\subsection*{Expand the World}
Fate's Edge is set in a rich world of ancient magic, fallen empires, and vibrant cultures. Explore the regions of the Amaranthine: Ecktoria's marble halls, the fog-shrouded Mistlands, the fractured kingdom of Vhasia, and more.

\section{Final Thought}

\begin{quote}
\textit{``The road remembers. Every broken wheel leaves a mark, every lit lamp bears witness. The only question is: what are you willing to owe?''}
\end{quote}

You have everything you need to play Fate's Edge. Now gather your dice, take a breath, and step into the Edge. The world is waiting.

% ============================================================
% CHAPTER 9: QUICK REFERENCE
% ============================================================
\appendix
\chapter{Quick Reference}
\label{ch:quick-reference}

\section{Core Resolution}

\begin{enumerate}
  \item Declare intent and approach (Attribute + Skill).
  \item GM sets Position and DV.
  \item Roll dice pool.
  \item Count successes (6+; 10 = 2 successes) and Story Beats (1s).
  \item Apply Outcome Matrix.
  \item GM spends SB.
  \item Player marks resources.
\end{enumerate}

\section{Difficulty Value (DV)}

\begin{center}
\begin{tabular}{ll}
\toprule
\textbf{DV} & \textbf{Situation} \\
\midrule
2 & Routine, no pressure. \\
3 & Default --- mild opposition. \\
4 & Hard --- serious resistance. \\
5+ & Extreme --- dramatic gamble. \\
\bottomrule
\end{tabular}
\end{center}

\section{Position}

\begin{center}
\begin{tabular}{lll}
\toprule
\textbf{Position} & \textbf{Narrative} & \textbf{Effect} \\
\midrule
Dominant & Advantage & Re-roll one failure. \\
Controlled & Normal & No re-rolls. \\
Desperate & Under duress & Re-roll one success. \\
\bottomrule
\end{tabular}
\end{center}

\section{Outcome Matrix}

\begin{center}
\begin{tabular}{lll}
\toprule
\textbf{Roll Result} & \textbf{Outcome} & \textbf{What Happens} \\
\midrule
S $\ge$ DV, SB = 0 & Clean Success & Intent achieved. \\
S $\ge$ DV, SB > 0 & Success with SB & Intent achieved; GM spends SB. \\
0 < S < DV & Partial & Progress; gain 1 Boon. \\
S = 0 & Miss & No progress; gain 2 Boons; GM escalates. \\
\bottomrule
\end{tabular}
\end{center}

\section{Boons}

\begin{itemize}
  \item \textbf{Max held:} 5.
  \item \textbf{Earn:} Partial (1 Boon), Miss (2 Boons), Bond-driven action (1 Boon).
  \item \textbf{Spend:} Re-roll a die, improve Position, activate Asset.
  \item \textbf{Convert:} 2 Boons $\rightarrow$ 1 XP (once per session, max 2 XP via conversion).
\end{itemize}

\section{Fatigue and Harm}

\begin{itemize}
  \item Fatigue track = Body.
  \item Each Fatigue worsens Position (or --1 die if already Desperate).
  \item Fatigue overflow $\rightarrow$ Harm +1, clear Fatigue.
  \item Harm: 1 (--1 die), 2 (--1 die), 3 (incapacitated).
\end{itemize}

\section{Armor Conversion}

\begin{center}
\begin{tabular}{llll}
\toprule
\textbf{Armor} & \textbf{Harm 1} & \textbf{Harm 2} & \textbf{Harm 3} \\
\midrule
Light & Fatigue 1 & Harm 1 + Fatigue 1 & Harm 2 + Fatigue 1 \\
Medium & Fatigue 1 & Fatigue 1 & Harm 2 + Fatigue 1 \\
Heavy & Fatigue 1 & Fatigue 1 & Fatigue 2 \\
\bottomrule
\end{tabular}
\end{center}

\section{Recovery}

\begin{itemize}
  \item \textbf{Fatigue:} Short rest (1 hour) removes 2. Full night removes all.
  \item \textbf{Harm:} Medical care or magical healing. Natural recovery: 1 week per level.
\end{itemize}

\section{Combat Quick Reference}

\begin{itemize}
  \item \textbf{Turn:} 1 Action + 1 Move.
  \item \textbf{Move:} Shift one range band (Close $\leftrightarrow$ Near $\leftrightarrow$ Far).
  \item \textbf{Dash (Action):} Move two bands; next defense Desperate.
  \item \textbf{Disengage (Action):} Move does not provoke opportunity attacks.
  \item \textbf{Opportunity Attack:} Leaving Close without Disengage provokes one free melee attack.
\end{itemize}

\section{Timers Quick Reference}

\begin{itemize}
  \item \textbf{4 segments:} Short-term (guard patrol, door breaking)
  \item \textbf{6 segments:} Standard scene (ritual, escape)
  \item \textbf{8+ segments:} Long-term (faction, campaign)
  \item \textbf{Tick on:} Partial/Miss, SB spend, fiction demand
  \item \textbf{When full:} Defined event occurs
  \item \textbf{Limit:} 3 active timers per scene maximum
\end{itemize}

\section{Hedge Gifts Quick Reference}

\begin{itemize}
  \item \textbf{Craft of the Hedge (4 XP):} Learn two Hedge Gifts.
  \item \textbf{No roll, no cost.} Gifts work within their stated limits.
  \item \textbf{Steady Hand:} Remove 1 Fatigue (once/scene).
  \item \textbf{Knot of Favor:} +1 die to one type of roll (once/scene).
  \item \textbf{Warm Hand:} Next physical action +1 die or --1 die.
  \item \textbf{Hearth-Sense:} Ask one yes/no question about a threshold.
  \item \textbf{Salt Line:} Stop spirits from crossing.
  \item \textbf{The Unlit Candle:} Extinguish a light source (once/session).
\end{itemize}

\vspace{1cm}
\begin{center}
  \rule{0.8\textwidth}{0.5pt}
  \vspace{0.5cm}

  \textit{``The coin that never spends is the one you don't remember taking.''}
\end{center}

\end{document}