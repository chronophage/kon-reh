...
Timers are visual trackers for ongoing challenges and accumulating pressure. They are the GM's primary tool for managing tension.

\subsection*{Timer Sizes}
\begin{itemize}
  \item \textbf{4 segments:} Short-term pressure (a guard patrol arrives, a door breaks, a quick ritual).
  ...
  \item \textbf{10+ segments:} Campaign timer (a war, a prophecy, a spreading curse).
\end{itemize}

\subsection*{When to Tick a Timer}
\begin{itemize}
  \item \textbf{On a Partial or Miss:} The party makes progress, but at a cost.
  \item \textbf{When you spend Story Beats:} 1 SB = 1 tick typically.
  \item \textbf{When the fiction demands it:} \textit{``While you argue, the fire spreads.''}
  \item \textbf{At the end of a session:} For long-term timers that advance between sessions.
\end{itemize}

\subsection*{What Happens When a Timer Fills}
When a timer reaches its final segment, the defined event happens. The event should be visible and change the situation.

\textbf{Examples:}
\begin{itemize}
  \item A 4-segment \textbf{Guard Patrol} timer fills: guards arrive. The party must hide, fight, or talk their way out.
  \item A 6-segment \textbf{Ritual Completion} timer fills: the ritual finishes. The demon arrives, the curse takes hold, or the portal opens.
  \item An 8-segment \textbf{Alliance Forming} timer fills: a faction commits to a course of action. The political landscape shifts.
\end{itemize}

\begin{tipbox}
\textbf{Timers are visible.} Show them to the players. Write \textit{Cult Influence [6]} on an index card and place it on the table. When you tick it, announce: \textit{``The cult's chant grows louder — the ritual advances.''}
\end{tipbox}
...
This adventure uses two visible timers to track pressure. You are the GM; you tick them when the fiction demands it.

\subsection*{Barrow Collapse [6]}
The barrow is ancient and unstable. Each failed roll inside the barrow ticks this timer.

\begin{itemize}
  \item \textbf{At 3 segments:} Dust falls from the ceiling. The party hears groaning stone.
  \item \textbf{At 5 segments:} Cracks appear in the walls. The exit is partially blocked.
  \item \textbf{At 6 segments:} The entrance seals completely. Escape becomes a Desperate group action.
\end{itemize}

\subsection*{Lena's Agenda [4]}
Quick Lena is also hunting the lantern. She is skilled and ruthless.

\begin{itemize}
  \item Tick this timer at the end of each scene inside the barrow.
  \item When it fills, Lena has taken the lantern. The party must chase her.
\end{itemize}

\begin{tipbox}
\textbf{Timers are your pacing tool.} Tick them when players fail rolls, when you spend SB, or simply when it feels right. Describe what changes each time: \textit{``The ceiling groans — dust rains down. Barrow Collapse is now at 3 of 6.''}
\end{tipbox}
...
\begin{itemize}
  \item \textbf{Name them descriptively:} \textit{``Ritual Completion''} not \textit{``Timer 3''}. Players should know what advancing it means.
  \item \textbf{Tick them aloud:} \textit{``The cult's chant grows louder — Ritual Completion advances to 4 of 6.''}
  \item \textbf{Link timer advances to fiction:} Never just say \textit{``tick timer.''} Describe what changes: \textit{``As the timer hits 5, the air smells of ozone and sulfur — the demon's presence warps reality nearby.''}
...
At any given time, maintain at most three active timers in a scene. More creates cognitive overload and dilutes tension.

\begin{itemize}
  \item \textbf{Situation Timer:} Tracks progress toward the scene's immediate goal (e.g., \textit{Chamber Collapse [8]}, \textit{Persuade the Duke [6]}).
  \item \textbf{Counter-Clock:} Tracks opposition pressure (e.g., \textit{Guard Patrols [4]}, \textit{Ritual Completion [6]}).
  \item \textbf{Campaign Timer:} Tracks long-term front pressure (e.g., \textit{Cult Influence [6]}, \textit{Sun's Fading [10]}).
\end{itemize}

Merge redundant timers. Retire timers that resolve naturally.
