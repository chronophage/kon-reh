...
{Łarge Mechanics serve narrative. Narrative feeds choice.}
\end{center}
...
\begin{tikzpicture}[
    node distance=2.5cm,
    every node/.style={align=center,font=Łarge},
...
\node[primary,right=5.5cm of core] (timers) {
\textbf{Timers}\\
Scene & Campaign\\
Track pressure
};
...
% Core connections
\draw[arrow] (core) −− (sb);
\draw[arrow] (core) −− (timers);
\draw[arrow] (core) −− (modules);
\draw[arrow] (core) −− (boons);

\draw[arrow] (sb) −− (core);
\draw[arrow] (timers) −− (core);
...
\node[secondary,above left=3.8cm of sb] (supply) {Supply Timer };
...
\draw[arrow] (timers) −− (supply);
\draw[arrow] (timers) −− (mandate);
\draw[arrow] (timers) −− (crisis);

\draw[arrow] (supply) −− (fatigue);
\draw[arrow] (supply) −− (resources);

\draw[arrow] (resources) −− (boons);
\draw[arrow] (resources) −− (timers);
...
\textbf{SB Budget = 4 + Tier   |   Max 3 active timers per scene}
\end{center}
