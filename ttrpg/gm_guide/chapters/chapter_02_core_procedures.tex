...
$S = 0$ & Miss\index{Miss} & No progress. Cash/bank SB, Award one Boon\index{Boon!acquiring} to the player. \\
\bottomrule
\end{tabular}
\end{center}
...
\subsection{Harm Integration with SB Economy}\index{harm integration}\index{SB economy!harm}

Harm tracks directly tie to the SB economy, creating cascading consequences:

\begin{itemize}
    \item \textbf{Minor (-)}\index{harm!Minor}: Generate 1 SB on next 2 rolls
    \item \textbf{Moderate (=)}\index{harm!Moderate}: Generate 1 SB on next roll, -1 die to relevant actions
    \item \textbf{Severe (≈)}\index{harm!Severe}: Generate 2 SB on next roll, -2 dice to relevant actions  
    \item \textbf{Critical (†)}\index{harm!Critical}: Generate 3 SB on next roll, out of action until treated
\end{itemize}
