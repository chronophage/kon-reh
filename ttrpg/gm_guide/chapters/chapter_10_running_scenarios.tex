...
The PCs split up: one charms a servant for key access (Presence + Sway), another distracts the guards with a fake alarm (Wits + Skullduggery), and a third scales the wall (Body + Athletics). Each roll adds tension---successes advance the plan, but Story Beats\index{Story Beats} trigger new dangers.
...
\begin{itemize}
    ...
    \item \textbf{Apply harm integration}\index{harm integration}: Minor harm generates 1 SB on next 2 rolls; Moderate harm generates 1 SB next roll with -1 die.
\end{itemize}
