...
\begin{itemize}[leftmargin=*]
  \item \textbf{Desperate:} +2  \textbf{Risky:} +1  \textbf{Controlled:} +0  \textbf{Dominant:} –1
\end{itemize}
...
A player declares a \textbf{Controlled} action to \textbf{pick a lock} while guards patrol above. They roll \textbf{Wits + Subterfuge}, get 3 successes and 1 Story Beat.
...
% ===== GM SCREEN: COMMON ROLLS QUICK REFERENCE =====
...
\item Consider \textbf{Position Effects}: Dominant (-1), Controlled (±0), Risky (+1), Desperate (+2)
...
\section{Quick GM Advice (Expanded Guide)}

\subsection*{Your Job in One Line}
\textbf{State the truth, raise the stakes, ask “what do you do?”} Repeat until a clock fills or the fiction turns.

\subsection*{Table On-Ramp (5–10 Minutes)}
\begin{enumerate}
  \item \textbf{Pitch the Situation in One Breath.} Where are we? What just changed? Who wants what right now?
  \item \textbf{Name Three Pressures.} Pick (or invent) up to three clocks: \emph{Alarm [4]}, \emph{Structural Stress [6]}, \emph{Tempers Flare [4]}.
  \item \textbf{Ask Two Questions.} “Whose problem is this?” “What are you willing to risk to solve it?”
  \item \textbf{Lay Tags.} Give the scene 1 Edge tag and 1 Hindrance tag (p.\pageref{tag-quick-impact}).
  \item \textbf{Start in Motion.} “You hear boots on the stairs; the door latch lifts. What do you do?”
\end{enumerate}

\subsection*{The Core Loop (Art of Consequence)}
\begin{enumerate}
  \item \textbf{Set \underline{Intent} \& \underline{Approach}.} Player states goal and how.
  \item \textbf{Pick Position \& Effect.} See quick dials below.
  \item \textbf{Set DV.} Default DV 2--3 for routine risk, DV 4 for hard, DV 5+ for desperate/boss beats.
  \item \textbf{Roll \& Read.} Count 6+ as successes; 1s feed \textbf{SB} to the GM.
  \item \textbf{Resolve.} Map to outcome, tick clocks, spend SB, hand out Boons.
  \item \textbf{Ask.} “What do you do next?”
\end{enumerate}

\subsection*{Position / Effect Quick Dials}
\begin{center}
\begin{tabular}{p{0.42\linewidth}p{0.52\linewidth}}
\textbf{Shift Position +1} & High ground, solid cover, surprise, leverage, ritual sanctum, correct etiquette key \\
\textbf{Shift Position -1} & Exposed, rushed, split focus, bright ward-lamps, wrong register, hostile crowd \\
\textbf{Effect +1} & Superior tool, help, precise intel, perfect element/symbol, tagged vulnerability \\
\textbf{Effect -1} & Bad angle, wrong tool, too little time, target is armored/ warded/ entrenched \\
\end{tabular}
\end{center}

\subsection*{DV Defaults (If You’re Unsure)}
\begin{itemize}
  \item \textbf{DV 2}: practiced task or clear advantage; failure is mostly pacing.
  \item \textbf{DV 3}: contested task in fair conditions.
  \item \textbf{DV 4}: tight timing, skilled opposition, or risky terrain.
  \item \textbf{DV 5}: boss move, ritual under fire, or changing the situation at scale.
\end{itemize}

\subsection*{SB (Story Beats) \& Boons: Quick Menu}
\paragraph{GM SB Spend (pick 1 on success \& cost / miss)}
\begin{itemize}
  \item \textbf{1 SB}: position drop, tick a minor clock +1, tool wear, reveal a soft complication.
  \item \textbf{2 SB}: split the party’s attention, inflict \emph{Harm 1} or \emph{+1 Fatigue}, escalate a front +1, trigger an enemy move.
  \item \textbf{3 SB}: advance a major clock +2, force a hard choice (goal vs safety), separate allies, introduce a fresh threat in Near.
  \item \textbf{4+ SB}: reframe the scene (fire spreads; reinforcements arrive), promote a scene tag to a \emph{campaign tag}.
\end{itemize}

\paragraph{Player Boon Uses (defaults)}
\begin{itemize}
  \item \textbf{1 Boon}: re-roll 1 die; gain Position +1 (once/scene); activate a small asset.
  \item \textbf{2 Boons}: add +1 success after the roll; resist a \emph{Harm 1} to \emph{Fatigue}; seize brief initiative.
  \item \textbf{Convert}: 2 Boons $\rightarrow$ 1 XP (max 2 XP/session via conversion).
\end{itemize}

\subsection*{Tags — Quick Impact Rule}
\label{tag-quick-impact}
\begin{fatebox}
\textbf{Edge tag} (e.g., High Ground, Consecrated, Darkness): \textbf{Position +1} \emph{or} \textbf{Effect +1}.\\
\textbf{Hindrance tag} (e.g., Slippery, Echoing, Choking Smoke): \textbf{Position −1} \emph{or} \textbf{DV +1}.\\
Conflicts on the same axis cancel 1 step. Two allied tags cap at \textbf{+1 Position} and \textbf{+1 Effect} unless a talent says otherwise.
\end{fatebox}

\subsection*{Clocks That Carry Sessions}
\begin{itemize}
  \item \textbf{Alert/Alarm [4]}: noticing, triangulating, closing. 
  \item \textbf{Injury/Stress [4]}: fatigue, morale, panic spread. 
  \item \textbf{Structure [6]}: heat, flood, fire, collapse.
  \item \textbf{Mandate [6]}: boss agenda / external deadline.
  \item \textbf{Opportunity [4]}: window for the best outcome.
\end{itemize}
\textit{Rule of Three:} keep \emph{three} active clocks in a scene. New pressure? Retire or merge one.

\subsection*{Harm \& Fatigue (Fast Math)}
\begin{itemize}
  \item \textbf{Baseline}: \emph{Harm} lands unless armor/talents/fiction reduce. 
  \item \textbf{Conversion}: If allowed by build, convert Harm$\rightarrow$Fatigue per your talent/body rules.
  \item \textbf{Mitigation Order}: Armor/Shield $\rightarrow$ Talents $\rightarrow$ Harm$\rightarrow$Fatigue transposition $\rightarrow$ Boons.
  \item \textbf{Bleed}: Full Fatigue converts overflow to \emph{Harm 1} (unless \emph{Hardy} or similar).
\end{itemize}

\subsection*{Scene Starters (Roll-In Prompts)}
\begin{itemize}
  \item \textbf{Heist}: “Show me the inside friend, the route in, and the false exit. Which one just failed?”
  \item \textbf{Parley}: “Lay the table (venue/tag), name stakes (two ledgers), who brings witness? Start with \emph{Board [4]}.”
  \item \textbf{Travel Leg}: “Pick your pressure: weather, watch, or want. One is true, one is rumor, one arrives late.”
  \item \textbf{Ritual}: “Who holds the line, who pays, who keeps count? Name one omen; I’ll name one cost.”
\end{itemize}

\subsection*{NPC Tiers \& Quick Stats}
\begin{itemize}
  \item \textbf{Mob (Tier I)}: DV 2–3 to outpace; Harm 1 on hit; breaks on a scare or setback. Use group clocks.
  \item \textbf{Professional (Tier II)}: DV 3–4; has 1 reliable move; inflicts Harm 1–2; retreats on clock fill.
  \item \textbf{Captain/Expert (Tier III)}: DV 4–5; 2 moves; has armor or ward; scripted reaction at 50\%.
  \item \textbf{Boss (Tier IV)}: DV 5; 3 moves; arena tags; phases at 50\% and 25\%; victory changes the map.
\end{itemize}

\subsection*{Boss Pattern (Three-Beat)}
\begin{enumerate}
  \item \emph{Read}: Telegraph moves with tags (Smoke, Echo, Tight Space). Place “Core [6]” + “Shield [4]”.
  \item \emph{Turn}: Spend 2–3 SB for arena changes (add Flood [2], cut lights, call adds).
  \item \emph{Break}: When Shield breaks, lower DV by 1 or remove a move; raise stakes elsewhere (hostage, fireline).
\end{enumerate}

\subsection*{Social Scenes as Boards}
\begin{itemize}
  \item \textbf{Create a Board [4–6]}. Name seats (Host, Rival, Swing). Each success places ticks; each cost places a clause for the other side.
  \item \textbf{Tools}: \emph{Good Faith}, \emph{Two Ledgers}, \emph{Deal Architect}, \emph{Crescendo}. 
  \item \textbf{Ends}: On fill, either \emph{Concession} (your frame) or \emph{Compromise} (split wins, reduce heat).
\end{itemize}

\subsection*{Travel in Three Checks}
\begin{enumerate}
  \item \textbf{Leg Setup}: set two tags (Weather, Watch). Pick one clock to track: \emph{Supply [4]} or \emph{Attention [4]}.
  \item \textbf{Way Risk}: Lead makes \textbf{Survey/Survival} (DV 3–4). On cost, tick chosen clock +1 and add a scene.
  \item \textbf{Arrival Stakes}: If any clock at 3+, start at Risky; at 4, begin in motion (ambush, storm, closed gate).
\end{enumerate}

\subsection*{Magic Adjudication (Unified Ruling)}
\begin{itemize}
  \item \textbf{Casters}: flexible, fast, swingy. Backlash = SB fuel and positional risk; big shaping needs time or tags.
  \item \textbf{Runekeepers}: stable, process-first. Consecrate sites/tools; watch for loops and obligation fronts.
  \item \textbf{Invokers}: oaths, symbols, witnessed space. Work is safe if the symbol stands \& the rite is clean; break the frame, pay the price.
\end{itemize}
\textit{Scope guard}: Scene-scale at DV 2–4; battlefield-scale at DV 5+ with set-up, tags, or a project clock.

\subsection*{Talents: Fast Reminders}
\begin{itemize}
  \item \textbf{Scene Steal} (\emph{revised}): Once/session, spend 2 Boons to cancel a declared GM spend of 3+ SB; replace with a 2 SB complication that fits the fiction (cannot negate completed clocks or patron edicts).
  \item \textbf{Transcendent Harmony} (\emph{revised}): Once/session set Position to \emph{Controlled} for one scene-scale casting (DV $\leq$ 4); mark 2 Fatigue; if larger, only Position +1.
  \item \textbf{Diamonds}: one-use, fiction-backed bypass/discount (max 2 unspent/PC). If physical, can be found/lost.
\end{itemize}

\subsection*{Prep in 15 (Crown Mini-Spread)}
\begin{enumerate}
  \item \textbf{Crisis (Top)}: name the event, 1–2 scene tags, 2 clocks.
  \item \textbf{Power (Left)}: who benefits, what they \emph{won’t} risk.
  \item \textbf{Price (Right)}: what changes if PCs win/lose.
  \item \textbf{Thread (Bottom)}: 2 rumors, 1 omen, 1 soft lead.
\end{enumerate}

\subsection*{Pacing \& Spotlight}
\begin{itemize}
  \item \textbf{Two-Turn Rule}: if a PC hasn’t acted in two exchanges, cut to them; offer a relevant prompt.
  \item \textbf{Escalate Every Third Beat}: if clocks sit, change a tag, spawn a rival claim, or shorten a window.
  \item \textbf{End on a Choice}: hand them a fork, not a denouement (“Which debt do you pay before dawn?”).
\end{itemize}

\subsection*{When You’re Stuck}
\begin{itemize}
  \item \textbf{Ask the Table}: “What here looks dangerous?” Make it true.
  \item \textbf{Introduce a Ledger}: a cost, a witness, or a due that matters.
  \item \textbf{Advance a Clock}: pick one meaningful consequence and move it.
  \item \textbf{Flip a Tag}: Edge $\leftrightarrow$ Hindrance (rain becomes flood).
\end{itemize}

\subsection*{Safety \& Lines in Fiction}
\begin{itemize}
  \item \textbf{Lines/Veils}: set them in session zero; keep them visible on the GM side.
  \item \textbf{Check-Ins}: before hard spend (3+ SB) or content turns, pause: “Good to proceed?”
  \item \textbf{Redirect, Don’t Punish}: if a line is bumped, reframe the threat or cost without heat loss.
\end{itemize}

\subsection*{Rewarding Play}
\begin{itemize}
  \item \textbf{Boons}: lean in when players choose risk, keep pace when they fail interestingly.
  \item \textbf{XP}: scenes that change the map, pay a debt, or teach a truth. Offer \emph{Bonus XP} for a declared \emph{cost accepted}.
  \item \textbf{Strings}: hand them out for favors earned, oaths witnessed, routes proven.
\end{itemize}

\subsection*{One-Page Cheat (Table Tents)}
\begin{itemize}
  \item \textbf{DV:} 2 easy / 3 fair / 4 hard / 5+ boss.
  \item \textbf{Position:} Ctrl / Risky / Desp (\textit{and why}).
  \item \textbf{Effect:} Lim / Std / Great (\textit{and why}).
  \item \textbf{SB Spend:} 1 drop / +2 tick / 3 reframe.
  \item \textbf{Boons:} re-roll / +1 succ / resist small harm.
  \item \textbf{Mitigate Order:} armor $\rightarrow$ talents $\rightarrow$ harm$\rightarrow$fatigue $\rightarrow$ boons.
  \item \textbf{Rule of Three:} max 3 clocks per scene.
  \item \textbf{End Each Beat With:} “What do you do?”
\end{itemize}

\subsection*{Worked Micro-Example}
\textbf{Hook}: Night heist at the Sluice House. Tags: \emph{Echoing Stone} (Hindrance), \emph{Blueprints in Hand} (Edge). Clocks: \emph{Patrol Loop [4]}, \emph{Water Level [6]}.

\textbf{Action}: Kes fake-stamps a writ under a ward-lamp.\\
\textbf{Position}: Risky (seen, but plausible). \textbf{Effect}: Standard (writ is good, not perfect). \textbf{DV}: 3 (trained clerks).\\
\textbf{Roll}: 2 successes, one die shows 1 $\Rightarrow$ GM +1 SB. \\
\textbf{Outcome}: Success \& Cost. Gate opens; GM spends 1 SB: \emph{Position −1 next exchange} (clerk asks for a verification bell). \\
\textbf{Ask}: “You hear boots on the stair above and the bell chain rattles—what do you do?”

\bigskip
\noindent\rule{\linewidth}{0.4pt}

\subsection*{Final Reminders}
\begin{itemize}
  \item Lead with fiction; let rules \emph{explain} what the fiction already suggests.
  \item State risks aloud before dice touch the table.
  \item Pay players for \emph{bold, consistent choices}—with Position, with tags, with Strings.
  \item Spend your SB. Empty clocks don’t scare anyone; moving ones do.
\end{itemize}

\section{Flow-First GMing}

You are not a rules engine. You are a director choosing the next shot to keep the story moving forward. Fate's Edge is designed to maintain narrative momentum above all else.

\subsection{The 30-Second Adjudication Loop}

Use this streamlined process for almost any action to maintain pace:

\begin{enumerate}
    \item \textbf{Clarify intent and approach.} "What do you want, and how?"
    \item \textbf{Set stakes and Position.} "If it works, what changes? If it fails, what bites?" Start Controlled/Standard unless fiction says otherwise.
    \item \textbf{Roll \& read.} Count 6+ as successes; each 1 gives you Story Beats (SB). Compare successes to DV.
    \item \textbf{Spend one beat well.} Cash SB on one memorable twist or tick a relevant Clock.
...
\begin{itemize}
    \item \textbf{Narrative Primacy:} Mechanics exist to shape how the story changes, not whether it moves.
    \item \textbf{Keep Mechanical Resolution Under 10 Seconds:} If it takes longer, you're doing it wrong.
    \item \textbf{Always Spend SB Immediately:} Don't hoard complications—resolve them now.
    \item \textbf{Trust Quick Decisions:} Perfect mechanical resolution is less important than maintaining story momentum.
\end{itemize}

\subsection{Rookie GM Comfort Dials}

These simplifications help new GMs focus on story flow:

\textbf{Soft SB (First 2 Sessions):} Cap each roll's SB spend to 1--2 unless it's a set-piece scene.

\textbf{Visible Clocks:} Put clocks on the table. Name them aloud: "Guards Incoming [4]". Tick them visibly.

\textbf{One Move, One Sentence:} Every ruling should end with one sentence that states the new situation.

\textbf{Default to DV 3:} When in doubt, DV 3 provides appropriate challenge for most meaningful actions.
...
\hline
Analysis Paralysis & Use default DV 3 and make quick decisions \\
\hline
Over-cranking SB & Halve SB spends or convert to visible Clocks \\
\hline
Clock Sprawl & Merge redundant clocks; maintain 2--3 active clocks per scene \\
\hline
Rules Drift & Pick ruling that keeps flow, note TODO for reconciliation \\
\hline
Player Disengagement & Ensure spotlight rotation; connect to character bonds \\
\hline
\end{tabularx}
...
\begin{enumerate}
    ...
    \item Strip down to three active clocks maximum
...
Remember: You are not the rules expert—you are the story facilitator. Focus on pacing, tension, and narrative flow. The rules are rails you lay just ahead of the train.
