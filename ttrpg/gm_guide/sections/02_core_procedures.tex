...
Fate's Edge uses \textbf{narrative initiative} by default: the GM frames spotlight based on the fiction. However, some tables prefer more structure, especially during complex combat. The following optional procedures are compatible with the core rules.

\subsection{When to Use Structured Initiative}
\label{subsec:when-structured-initiative}
...
\begin{figure*}[t]
  ...
  \begin{tikzpicture}[
      ...
      box/.style={rectangle, draw=black!70, thick, fill=white, 
                  text width=2.8cm, inner sep=6pt, rounded corners=2pt},
      arrow/.style={-{Stealth[length=3mm,width=2mm]}, thick, draw=black!60},
      looparrow/.style={-{Stealth[length=3mm,width=2mm]}, thick, draw=black!60, bend right},
      title/.style={font=\bfseries\sffamily, text width=3.5cm, align=center}
  ]
  
  % === Core Loop ===
  \node[box, fill=blue!5!white] (core) {Core Resolution\\Loop\\\tiny(Declare → Roll → Outcome)};
  
  % === Outputs of the Core Loop ===
  \node[box, above right=0.8cm and 2.2cm of core] (sb) {Story Beats (SB)};
  \node[box, right=of core] (clocks) {Clocks};
  \node[box, below right=0.8cm and 2.2cm of core] (boons) {Boons};
  \node[box, below=of core] (modules) {Player‑Managed\\Modules};
  
  % === Pressure Systems ===
  \node[box, fill=gray!10, below left=1.2cm and -1cm of modules] (pressure) {Pressure Systems\\\tiny(Supply, Fatigue, Tactical Clocks)};
  \node[box, fill=gray!10, right=of pressure] (consequences) {Consequence Storage\\\tiny(Harm, Exposure, Asset State)};
  
  % === Narrative Feedback ===
  \node[box, fill=orange!5!white, above=5cm of modules, text width=3.5cm] (narrative) {Narrative Outcomes\\\tiny(Changed Fiction, Twists, New Hooks)};
  
  % === Connections ===
  \draw[arrow] (core.east) -- node[above, font=\tiny] {drives} (clocks.west);
  \draw[arrow] (core.north east) -- node[above right, font=\tiny] {generates} (sb.south west);
  \draw[arrow] (core.south east) -- node[below right, font=\tiny] {awards} (boons.north west);
  \draw[arrow] (core.south) -- node[right, font=\tiny] {triggers} (modules.north);
  
  % From SB/Clocks/Boons/Modules to pressure/consequences
  \draw[arrow] (sb.south) -- ++(0,-0.6) -| (pressure.north);
  \draw[arrow] (clocks.south) -- ++(0,-0.5) -| (pressure.north);
  \draw[arrow] (modules.east) -- node[below, font=\tiny] {feed} (pressure.west);
  \draw[arrow] (boons.south) -- ++(0,-0.5) -| (consequences.north);
  \draw[arrow] (pressure.east) -- node[above, font=\tiny] {store} (consequences.west);
  
  % Narrative feedback loop
  \draw[arrow] (consequences.north) -- ++(0,0.8) -| (narrative.south);
  \draw[arrow] (narrative.west) -- ++(-0.6,0) |- (core.north);
  \draw[arrow] (sb.north) -- ++(0,0.4) -| (narrative.east);
  
  % Additional feedback: Boons and SB used to fuel pressure
  \draw[arrow] (sb.north east) -- ++(0.5,0) |- (pressure.east);
  \draw[arrow] (boons.east) -- ++(0.6,0) |- (consequences.south);
  
  % Labels for major flows (optional)
  \node[font=\tiny\itshape, above left=0.1cm and -0.5cm of sb] {escalate};
  \node[font=\tiny\itshape, above right=0.1cm and -0.5cm of boons] {mitigate};
  \node[font=\tiny\itshape, left=0.1cm of pressure] {absorb};
  
  \end{tikzpicture}
  \caption{Fate’s Edge systems: the Core Resolution Loop drives Story Beats (SB), Clocks, Player‑Managed Modules, and Boons; pressure systems store consequences (Supply, Fatigue, Harm, Exposure); narrative outcomes feed back into play, closing the loop.}
...
\label{subsec:lazy-dv}
\index{DV!lazy table}\index{GM guidance!DV}

Use this table for quick rulings. DV represents the \emph{significance of the challenge}, not its intrinsic difficulty.
...
\label{subsec:lazy-consequences}
...
\label{subsec:fatigue-first}
\index{Fatigue!design philosophy}\index{Harm!design philosophy}
...
\paragraph{GM Guidance.}
Use Fatigue to say: “You can keep going, but it will cost you.”  
Use Harm to say: “You cannot ignore this.”  
...
\label{subsec:equipment-damage}
\index{equipment!damage}\index{GM guidance!equipment}
...
\end{itemize}
\emph{Example:} A shattered shield during a last stand is meaningful. A snapped sword on a routine miss is not.

\subsection{DV Is Not Difficulty — It Is Commitment}
\label{subsec:dv-as-commitment}
\index{DV!philosophy}
...
\label{subsec:high-tier-reminder}
...
\begin{itemize}
\item Boon scarcity (fewer misses and partials mean fewer Boons)
...
\addcontentsline{toc}{section}{First Session Cheat Sheet: The Pressure Engine in Action}
\label{sec:first-session-cheatsheet}
\index{first session!checklist}

\begin{tcolorbox}[title=The Pressure Engine at a Glance, colback=white!97!gray, colframe=black!80!gray]
...
\textbf{Aligns with core system logic.} \\
\midrule
...
This table is not a rulebook. It is a compass. Use it until the rhythm of play becomes second nature.
\end{tcolorbox}

\clearpage
\section*{Your First Combat: A GM Cheat Sheet}
\addcontentsline{toc}{section}{Your First Combat: A GM Cheat Sheet}
\label{sec:first-combat-cheatsheet}
\index{combat!first session}\index{GM guidance!combat}
...
\midrule
Setup &
Describe the immediate situation clearly: “You’re in a narrow hallway, back to the wall. Three bandits advance.”  
...
\textbf{The GM always states Position before the roll.} “This is Desperate. If it goes wrong, you’ll be disarmed.” \\
\midrule
Dice Pool &
Roll \textbf{Attribute + Skill} (e.g., Body + Melee). No extra math. Player states approach in fiction. \\
\midrule
Outcomes &
\textbf{Clean Success}: Goal achieved cleanly.  
\textbf{Success with Cost}: Goal achieved; spend 1 SB for complication.  
\textbf{Partial}: Progress at reduced scale, improved position, or hard choice.  
\textbf{Miss}: No progress; spend SB to escalate. \\
\midrule
Story Beats &
Spend SB aggressively but cleanly:  
1 SB — Minor complication (“Steel sparks; you gain ground, but the noise carries.”)  
2 SB — Moderate setback (“Your ally is knocked off-balance; their next roll is Desperate.”)  
3+ SB — Major shift (“Reinforcements arrive. New Clock: Escalation [4].”) \\
\midrule
Clocks &
Use \textbf{one clock maximum} in first combat (e.g., “Enemy Harm [4]”). Advance on Miss or Success-with-Cost. All clocks visible. \\
\midrule
Multiple Enemies &
Treat a group as \textbf{one threat} unless individuals act differently. One clock covers the group. \\
\midrule
Boons &
Players may spend 1 Boon to re-roll a die, improve Position, or activate an Asset. Gain 2 Boons on Miss, 1 on Partial. \\
\midrule
What to Ignore &
No miniatures or grids. No advanced maneuvers. No fatigue-position interactions. No multi-clock combat. \\
\midrule
Next Session &
Add \emph{one} new element: a second clock, environmental pressure, or a structured combat arc. Introduce Three-Beat Combat later. \\
\bottomrule
\end{longtable}
\end{center}

\paragraph{First Combat Rule of Thumb.}
Describe situation → State Position → Roll → Apply outcome → Spend SB once → Move forward. If the scene is moving, you’re doing it right.
