...
$S = 0$ & Miss: No progress. Cash/bank CP, award one Boon to player \\
\bottomrule
\end{tabularx}
\end{fatebox}
...
\begin{fatebox}[CP Spend Menu]\index{CP Spend Menu}
\begin{tabularx}{\textwidth}{lX}
\toprule
\textbf{CP Cost} & \textbf{Example Complications} \\
\midrule
1 CP & Minor pressure: noise, trace, +1 Supply segment, brief distraction \\
2 CP & Moderate setback: alarm raised, lose position/cover, lesser foe appears \\
3 CP & Serious trouble: reinforcements arrive, key gear breaks, tactical disadvantage \\
4+ CP & Major turn: trap springs, authority intervenes, scene shifts dramatically \\
\bottomrule
\end{tabularx}
\end{fatebox}
...
\begin{fatebox}[Harm Integration with CP Economy]\index{harm integration}\index{CP economy!harm}
\begin{tabularx}{\textwidth}{lX}
\toprule
\textbf{Harm Level} & \textbf{Effects \& CP Generation} \\
\midrule
Minor (-) & -1 die to related actions; generate 1 CP on next 2 rolls \\
Moderate (=) & -1 die to most actions; generate 1 CP on next roll until treated \\
Severe (≈) & -2 dice to relevant actions; generate 2 CP on next roll; incapacitation risk \\
Critical (†) & Character incapacitated/dying; generate 3 CP on next roll if conscious \\
\bottomrule
\end{tabularx}
\end{fatebox}
...
Position can shift during combat based on CP spending and narrative triggers:

\begin{itemize}
    \item \textbf{GM Spend (1 CP)}: Shift position one step worse for current action
...
\begin{enumerate}
    ...
    \item \textbf{Roll \& Read}: Count successes (6+) and CP (1s). Compare to DV.
...
\midrule
Over-cranking CP & Halve CP spends temporarily or convert to visible Clocks \\
