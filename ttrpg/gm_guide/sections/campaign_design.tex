...
\index{clocks!design principle}

You do not need to know exactly how a faction will act in session 12. Instead, give them a \textbf{clock} with a trigger condition.
...
When the clock fills, the cult achieves its goal---and you improvise the specifics based on the current fiction.
...
Visible clocks create \textbf{tension} and \textbf{informed choices}. Players can prioritize which clock to slow.

\begin{tcolorbox}[colback=blue!5!white, colframe=blue!75!black, title=Physical Trackers for Clocks]
\index{trackers!physical}
Use tactile tools to make clocks visible and engaging:
\begin{itemize}
  \item \textbf{Dry-erase boards or laminated sheets} with pre-drawn clock circles (4/6/8/10 segments). Mark ticks with a wet-erase marker.
  \item \textbf{Index cards with pie-chart wedges} drawn in pen; use a paperclip to mark current segment.
  \item \textbf{Poker chips or glass beads} placed on a numbered track from 0 to clock size.
  \item \textbf{Magnetic whiteboard with a clock template} and magnetic tokens.
\end{itemize}
...
\begin{itemize}
  ...
  \item At the end of a session (for long-term clocks like \emph{Cult Influence}).
\end{itemize}

\subsubsection{Clock Size Guidelines}
\begin{itemize}
  \item \textbf{4 segments:} Short-term tension (a patrol arrives, a door breaks).
  \item \textbf{6 segments:} Standard scene clock (a ritual completes, a rival escapes).
  \item \textbf{8 segments:} Multi-scene or session clock (a faction consolidates power).
  \item \textbf{10+ segments:} Campaign clock (a war, a prophecy, a curse).
\end{itemize}

\subsection{Factions and Fronts}
\index{factions}\index{fronts}

A \textbf{faction} is any organized group with goals that intersect the party's. A \textbf{front} is a collection of related factions and clocks that represent a major threat.
...
A rival party is a faction of 3--5 NPCs with their own Patrons, assets, and a visible \textbf{Rival Agenda clock} (6--8 segments). Tick their clock when the PCs ignore them or the GM spends SB on their off-screen activities. When the clock fills, the rivals achieve a major milestone.
...
\begin{enumerate}
  \item \textbf{Update clocks:} Tick any faction, travel, or campaign clocks based on last session's outcomes.
  ...
  \item \textbf{Prepare 2--3 flexible scenes:} Not scripts, but situations with a clear stake and a clock.
...
\begin{tcolorbox}[colback=green!5!white, colframe=green!75!black, title={Arcs Generate Obligation and Patron Interest}]
  ...
  \begin{itemize}
    ...
    \item \textbf{Whenever a player advances their arc, mark +1 Obligation to a relevant Patron or tick a Patron's Interest clock.}
  \end{itemize}
\end{tcolorbox}
