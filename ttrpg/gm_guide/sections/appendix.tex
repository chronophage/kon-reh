...
\begin{itemize}
  ...
  \item \textbf{Over-Stack:} Two or more structural advantages on the same action \rightarrow start one obstacle at \(+1\) DV or GM banks \(+1\) SB.
\end{itemize}
\end{tcolorbox}
...
\index{combat!mass}
Treat an army as a high-Cap Follower (see \ref{sec:followers-assets-gm}) with its own clocks:
\begin{itemize}
...
\end{itemize}
When the clock fills, the rivals achieve a major milestone (steal the relic, secure the Patron's favor, etc.). The GM reveals their progress through rumors, looted sites, or a direct confrontation.
...
\begin{itemize}
  ...
  \item \textbf{Corruption Clock:} Size = Spirit. When full \rightarrow bloom (gain benefit \& burden from last patron), reset to Tier (min 1).
