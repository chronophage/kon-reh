...
\subsection*{GM Guidance: Cantor-Led Cults}
Cantor cults (\emph{Choruses}) are an engine for drama, favors, and fallout. Treat the Chorus as a living faction tied to the Cantor’s \textbf{Corruption} and the community’s \textbf{Obligation}. Keep the bookkeeping light: 1–2 clocks, clear triggers, visible rewards—and swift consequences.

\begin{fatebox}[Chorus as a Front --- GM Tools]
\begin{tabularx}{\textwidth}{lX}
\toprule
\textbf{Track} & \textbf{Rule of Thumb} \\
\midrule
\textbf{Scale} & \emph{Choir} (dozens), \emph{Flock} (scores–hundreds), \emph{Sect} (hundreds+). Scale \(\approx\) Cantor’s \textbf{Presence} and recent wins. \\[2pt]
\textbf{Core Clock} & \textbf{Devotion Sours [6]}. Tick when: promises slip, doctrine wavers, rivals tempt, or the Cantor leverages the Chorus for risky schemes. On fill: fracture, scandal, or rival claimant. Reset by public service (\emph{Vigil}) or costly restitution. \\[2pt]
\textbf{Leverage} & Once/scene if present (or reachable), the Chorus grants one: \emph{+1 die} (Performance/Sway), rumor placement, safehouse for a night, or small crowd-work (block, disperse, carry). Each use: mark \textbf{+1 Obligation} (Patron or community). \\[2pt]
\textbf{Heat} & After any public \emph{Inspire Chorus} or Resonant performance that shifts a scene’s balance, start \textbf{Unwanted Attention [4]} (magistrates, rivals, Patrons). On fill: raid, edict, or counter-cult. \\[2pt]
\textbf{Dials} & Softer Chorus: benefits cost no Obligation but tick \textbf{Devotion Sours}. Harsher Chorus: double Heat in law-heavy districts; failures auto-tick \textbf{Devotion Sours}. \\
\bottomrule
\end{tabularx}
\end{fatebox}

\paragraph*{GM Moves (Pick 1–2 on tension spikes).}
\begin{itemize}
  \item Offer an easy vow now that becomes hard later (advance \emph{Devotion Sours} on refusal).
  \item Split the Chorus: elders vs.\ youths; orthodoxy vs.\ spectacle.
  \item Present a desperate petitioner whose need conflicts with Patron doctrine.
  \item Elevate a lieutenant; give them agency, appetite, and a private debt.
  \item Invoke the Patron’s agenda through a zealot who “hears the harmony” too loudly.
  \item Make the price visible: tithes, time, safety—someone meaningful pays.
\end{itemize}

\paragraph*{Sample Complication Clocks.}
\begin{itemize}
  \item \textbf{Splinter Doctrine [4]:} A charismatic follower rewrites the Song; on fill, they peel away half the Chorus (new faction).
  \item \textbf{Patron Interference [6]:} Miracles with strings; on fill, mandatory rite or geas targets the Cantor.
  \item \textbf{Civic Heat [4]:} Edicts, informants, fines; on fill, venue shuttered or permit revoked mid-performance.
  \item \textbf{Devotee in Peril [4]:} A named follower’s crisis escalates until addressed; on fill, tragedy stains the Chorus.
\end{itemize}

\paragraph*{Scene Prompts.}
\begin{itemize}
  \item A rival cantor countersongs at the market; the crowd tilts between them.
  \item A funeral turns into a recruitment surge—along with contested obligations.
  \item A magistrate demands the Chorus disperse or tithe; the Patron wants defiance.
  \item A Chorus rumor you seeded comes home crooked; fix it or wear it.
\end{itemize}

\paragraph*{Safety \& Tone.}
Cults touch loyalty, coercion, and faith. Use lines/veils and check-ins. Keep consequences fictional and reversible through play (service, restitution, confession), not punitive bookkeeping.
