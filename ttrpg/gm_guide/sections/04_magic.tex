...
\begin{itemize}
  \item \textbf{Minor change (1 TAG):} The player’s intent affects only themselves or a single object. Examples: \texttt{Light}, \texttt{Mend}, \texttt{Veil} (self only). DV 2–3.
...
If the player’s description would \emph{skip over a scene} (e.g., “I teleport us past the entire fortress”), that’s a major change – require components, a clock, or a steep cost. If it would only \emph{add flavor} (e.g., “I make my eyes glow red”), that’s free. Let the fiction guide the DV, not a pre‑computed formula.
\end{tcolorbox}
...
% Insert after the "When in Doubt, Measure Tags by Narrative Impact" box
% and before \section{Story Beats and Backlash (GM Tools)}
% ============================================================
...
Some spell effects are so profound that they cannot be treated as ordinary TAGS. They **break the normal physics of the scene**, **skip over significant narrative obstacles**, or **permanently alter characters or the world**. These are **dangerous TAGS** and they incur **+2 DV** instead of the usual +1.
...
\begin{enumerate}
  ...
  If the spell would allow the party to bypass a challenge that you intended to be a whole scene (e.g., “I teleport us into the vault” instead of “I pick the lock, evade the guard, and disable the trap”), it is **dangerous**.

  \item \textbf{Does this effect permanently change the character, a major NPC, or the setting?}  
  If the change is not easily reversible by mundane means (e.g., turning a guard into a frog, erasing a memory, raising a corpse), it is **dangerous**.

  \item \textbf{Does this effect introduce a new major asset or remove a major threat with a single roll?}  
  If the spell would instantly defeat a boss (``I transform Garrick into a mouse''), destroy a fortress (``I collapse the mountain''), or create a powerful ally (``I summon a dragon''), it is **dangerous** – and likely requires a ritual, not a single action.
\end{enumerate}

\paragraph{Edge Cases}
\begin{itemize}
  \item \texttt{Leap} used to jump a chasm that would otherwise require an Athletics roll – **not dangerous** (that’s a minor bypass).  
  \item \texttt{Leap} used to teleport past a locked gate, a patrol, and a guard post – **dangerous** (bypasses multiple obstacles).  
  \item \texttt{Summon} calling a Cap 1 rat to scout a room – **not dangerous** (minor utility).  
  \item \texttt{Summon} calling a Cap 3 demon to fight a boss – **dangerous** (adds a major combatant).
\end{itemize}

\paragraph{GM Principle}
When in doubt, **rule in favour of the story**. A dangerous tag is not a prohibition – it is a **higher cost**. Tell the player:

> “That effect is powerful enough to reshape the scene. It’s a dangerous tag, so the DV will be +2 higher than usual. On a failure, the backlash will be severe. Are you sure you want to try?”

This keeps player creativity alive while ensuring magic feels weighty and consequential.

\textbf{Mend restores Fatigue, not Harm.} When a player uses a cantrip with only \texttt{Mend}, it removes 1 Fatigue. This is safe, fast, and low‑risk.

\textbf{Healing Harm costs more.} To restore 1 level of Harm, the player must either:
\begin{itemize}
  \item accept a \textbf{+2 DV modifier} on the spell, or
  \item use \texttt{Mend} with another tag (e.g., \texttt{Purify}, \texttt{Strengthen}).
\end{itemize}
The injury’s severity can raise DV further (e.g., Harm 2 → +1 DV on top). Requiring a second tag also makes healing a full two‑action spell (Weave + Cast) unless the caster has a talent that compresses casting.

\textbf{GM Tools:} When a player heals Harm, you may spend any Story Beats generated by the roll to introduce thematic complications (e.g., the wound leaves a scar, the healing draws supernatural attention, or the patient feels a strange hunger). This keeps magical healing dramatic, not trivial.

\end{tcolorbox}

% ============================================================
% HEALING MAGIC (GM RULING)
% Insert after \subsection*{Handling Creative Player Ideas}
% and before \section{Story Beats and Backlash (GM Tools)}
% ============================================================
...
Healing in \textbf{Fate's Edge} is deliberately gated.  \texttt{Mend} restores \textbf{Fatigue} quickly and safely; restoring \textbf{Harm} requires more power, more risk, and often a narrative price.  This keeps injuries meaningful while still allowing magical support.

\paragraph{Mend Restores Fatigue, Not Harm.}
When a player uses a cantrip (or a free‑form casting) with only the \texttt{[MEND]} tag, it removes \textbf{1 Fatigue} from the caster or a touched ally.  
\begin{itemize}
  \item \textbf{DV:} 2 (cantrip speed)  
  \item \textbf{Action:} 1 action  
  \item \textbf{Risk:} None inherent – the GM may still spend Story Beats (from any 1s on the roll) for thematic colour (e.g., a scar that tingles, faint spiritual attention).
\end{itemize}
...
\begin{enumerate}
  \item Accept a \textbf{+2 DV modifier} on the spell, or
  \item Use \texttt{[MEND]} together with \textbf{at least one other appropriate tag}, such as:
    \begin{itemize}
...
3 SB & The healing draws a Patron’s curiosity: mark +1 Obligation or advance a Corruption clock. \\
...
\begin{itemize}
  \item \textbf{First Aid (Wits + Medicine, DV 3):} Stabilises a dying character (prevents Harm progression).  Does not heal Harm.
...
\begin{quote}
  \emph{Marcus (Harm 1) asks Kestra to heal him.  Kestra uses \texttt{[MEND]} + \texttt{[PURIFY]} and accepts the +2 DV, rolling against DV 5.  She succeeds, but rolls two 1s.  The GM spends 2 SB: “The wound closes, but a faint scar remains – and you feel a pair of cold eyes watching from the shadows.  Something in the fog took notice.”}
\end{quote}
...
Players track their own Obligation and Corruption as described in \ref{sec:player-managed-modules-gm}. \textbf{Your job is to enforce thresholds and frame Patron intrusions as story hooks, not punishments.}
...
\begin{itemize}
  ...
  \item On a fill, impose the last Patron’s benefit \& burden (from Patron tables in the \textbf{Player's Guide}). Remember that the player then \textbf{sets their current Corruption segments to their Tier} (minimum 1). This means high‑Tier Cantors always carry a trace of past blooms – use that as a story hook.
  \item Spend SB to represent social fallout, Patron omens, or magical residue from their performances. For example, a Cantor who Pushes a song in a crowded tavern might generate a rumor clock or attract the attention of a witch‑hunter.
\end{itemize}
...
If a Cantor performs a \textbf{Resonant Rite} (a Song performed with dramatic intent, witnessed by 10+ people, and dedicated to a specific Patron) they may found a \textbf{Chorus}. This grants a minor follower benefit (see \ref{subsec:followers-assets}), but they must mark Obligation to the Cult's Patron (the patron of the Resonant Rite sung) each session they use it.

\textbf{Managing a Chorus Cult:}
\begin{itemize}
  \item Define the benefit concretely with the player. Good examples: +1 die to Performance once/session, a single rumour per session, or +1 Boon when singing in public.
  \item Track the cult’s health with a \textbf{Schism Clock [4]}. Tick it when the player ignores the cult (no contact, refuses a request) or when the cult’s demands go unmet. On a full clock, introduce a fracture: a rival sect, false rumours, or alliance with a Patron’s enemy.
...
\begin{itemize}
  ...
  \item A Resonant Rite always advances the Corruption Clock (mark 1 segment). If the clock fills, the bloom occurs during the performance – describe the Patron’s touch manifesting in front of the audience.
\end{itemize}
...
> *“The Cantor’s voice is a candle in the dark. It illuminates, but it also draws the eye of things that hunt by light. Let them sing. Let them bloom. Let their chorus become legend – or a warning.”*  
> — The Gray Wanderer, \textit{The Grey Wanderer’s Grimoire}

\index{rival factions}
\textbf{Rival Factions:} A neglected Chorus can become a rival faction (see Section \ref{ch:gm-advanced}, \textit{Advanced GM Techniques}). They may split into a competing sect, spread false rumors about the Cantor, or even ally with a Patron’s enemy. Use this to generate long‑term plot hooks.
