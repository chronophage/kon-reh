...
Instead of rolling for every soldier, treat opposing armies as a single \emph{War Clock [8]}. Player actions (command, sabotage, rally) tick the timer. When it fills, that side breaks or routs. Personal combat can happen in the same scene -- but the mass conflict is abstracted to a single visible tracker.

\subsection*{Political Intrigue: Reputation as a Resource}
Track a \emph{Reputation Clock [6]} for the party in a given court. Social actions (gossip, favors, public deeds) tick it. When it fills, they become ``trusted'' -- they can call in a major favor before the timer resets. But enemies also watch their rise, and may work to degrade the timer.
