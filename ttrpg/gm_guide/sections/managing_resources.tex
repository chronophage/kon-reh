...
Players can invest XP in \textbf{Followers} (on-screen allies) and \textbf{Assets} (off-screen resources). These are story agents with their own motivations and risks. The following sections present simplified, low-bookkeeping systems that avoid numeric clocks while preserving meaningful consequences.
...
Refusing to honor the debt turns the patron into an enemy. The GM may advance associated \textit{Crisis} clocks or introduce complications.

\begin{tcolorbox}[colback=gray!5!white, colframe=gray!75!black, title={Use Debt Clocks Aggressively}]
\index{Debt Clock!GM advice}
\textbf{Debt clocks are story fuel.} Call them in at the worst possible moment:
\begin{itemize}
...
\begin{itemize}
  \item Track each patron's Debt Clock and the party's Obligation visibly.
  ...
  \item When a Debt Clock fills, give a hook that fits the patron's personality.
...
\subsection{Crisis Clock for Offscreen Characters}
Each offscreen character tracks a private \textbf{Crisis Clock [6]}. This clock represents mounting pressure, danger, or narrative trouble that accumulates while the character is away.

\subsubsection{When to Tick the Crisis Clock}
Tick the Crisis Clock +1 when:
\begin{itemize}
...
\subsubsection{When the Crisis Clock Fills}
...
\end{itemize}
After the crisis, reset the Crisis Clock to 0. The crisis should create a story hook, not remove the character from play.

\subsubsection{Reducing the Crisis Clock}
Players may reduce the Crisis Clock by:
\begin{itemize}
...
\end{itemize}
The GM may also reduce the clock by 1 if the player’s Downtime Log shows proactive risk management.
...
If multiple players have offscreen characters, they may nominate a \textbf{Buddy} (another player with an active character) to help track Downtime Logs and Crisis Clocks. The Buddy gains \textbf{+1 additional Boon} for the session for this support role (this Boon does not count toward the session cap).
...
For player-facing rules on Echo Characters (how to declare absence, what a Downtime Log is, and the benefits of returning), see the Player’s Guide, Chapter \ref{ch:assets-followers}. The player version omits Crisis Clocks and GM consequence mechanics; those are your tools as GM.