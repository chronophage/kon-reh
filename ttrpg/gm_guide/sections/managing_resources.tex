...
\begin{tcolorbox}[colback=blue!5!white, colframe=blue!75!black, title={Philosophy of Resources},breakable]
...
\begin{tcolorbox}[colback=gray!5!white, colframe=gray!75!black, title={The Roller-Coaster Effect}, label={sec:roller-coaster},breakable]\index{Fatigue!roller-coaster effect}
...
\begin{tcolorbox}[colback=yellow!5!white, colframe=yellow!75!black, title={Why You Should Spend Boons -- The Math of Fun},breakable]
...
A Follower (Cap 3+) can become a \textbf{successor} if the player's main character retires, dies, or ascends. This is the core of the Legacy Engine (see \ref{ch:gm-epic} in the full GM Guide).
...
\section{Terrestrial Patrons: Favors and Obligation}
\label{sec:terrestrial-patrons-gm}
\index{Patrons!terrestrial}\index{Obligation Timer }
...
When a character accepts a favor from a terrestrial patron, start an \textbf{Obligation Timer } with the patron's name (e.g., \textquotedbl Prefect Marcellus's Obligation [4]\textquotedbl). Mark one segment each time the character accepts a significant favor. When the timer fills, the patron calls in the debt -- a mission, a political favor, or a direct demand.
...
\begin{tcolorbox}[colback=gray!5!white,colframe=gray!75!black,title={Use Obligation Timers Aggressively},breakable]
\index{Obligation Timer !GM advice}
\textbf{Obligation timers are story fuel.} Call them in at the worst possible moment:
\begin{itemize}
...
\end{itemize}
Obligation that is never called is wasted potential.
\end{tcolorbox}
...
\bottomrule
\end{tabularx}
\caption{Terrestrial Patron Tiers and Capacity}
\label{tab:patron-tiers}
\end{table}
...
\bottomrule
\end{tabularx}
\caption{Asset Costs by Patron Tier}
\label{tab:asset-costs}
\end{table}
...
\begin{tcolorbox}[title=Sample Terrestrial Patrons, colback=white!97!gray, colframe=black!80!gray,breakable]
\begin{tabularx}{\linewidth}{lX}
\toprule
...
\begin{itemize}
  \item Track each patron's Obligation Timer and the party's Obligation visibly.
  ...
  \item When an Obligation Timer fills, give a hook that fits the patron's personality.
  \item If the party refuses, tick a \textit{Crisis} timer or introduce a complication.
  \item Patrons compete. If the party owes Obligation to two rival patrons, those patrons will eventually demand conflicting loyalties.
\end{itemize}
...
\midrule
Street\par\nopagebreak
{\scriptsize local crime boss, guild steward, hedge-court judge}
& Spirit + Presence (3--4) & 2 & Minor (safehouse, contact network)
& Demands a \textit{minor service}: deliver a message, pay a bribe, rough up a rival. \\[4pt]

City\par\nopagebreak
{\scriptsize noble house factor, syndicate capo, temple prefect}
& Spirit + Presence + 2 (5--6) & 4 & Standard (spy ring, guild charter, fortified warehouse)
& Demands a \textit{significant task}: escort a caravan, break a siege, forge a document. \\[4pt]

Regional\par\nopagebreak
{\scriptsize duchy spymaster, merchant prince, coven leader}
& Spirit + Presence + 4 (7--8) & 6 & Major (fortress lease, city license, regional network)
& Demands a \textit{major service}: broker a treaty, assassinate a rival, steal a relic. \\[4pt]

Imperial\par\nopagebreak
{\scriptsize archduke, syndicate master, high priest}
& 8+ & 8+ & Major or unique (legendary asset)
& Demands a \textit{campaign--defining favor}: start a war, erase a bloodline, wake or seal a patron. \\
\bottomrule
\end{tabularx}
\tcblower
...
\bottomrule
\end{tabularx}
\caption{Trust Tiers and Capacity}
...
\begin{itemize}
  \item The Cap 5 follower can manage up to \textbf{4 Asset Slots} (any combination of Assets and Followers).
...
Assets and Followers acquired through a Trust belong to the \textbf{party as a whole}. Any member may activate them by spending their own Boons, following the normal Asset rules (see \ref{ch:running-assets-followers}).

\begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,title=Trusts and Terrestrial Patrons,breakable]
...
\begin{tcolorbox}[colback=gray!5!white,colframe=gray!75!black,title=Trusts Summary,fonttitle=\bfseries,breakable]
\textbf{Forming:}
\begin{itemize}
  \item Players contribute XP collectively.
  \item Purchase Trust Tier (8/16/32 XP) for slot capacity.
\end{itemize}

\textbf{Capacity by Tier:}
\begin{itemize}
  \item Tier I (8 XP): 2 slots, max Standard Asset
  \item Tier II (16 XP): 4 slots, max Major Asset
  \item Tier III (32 XP): 6 slots, max Major Asset
\end{itemize}

\textbf{Upgrading:}
\begin{itemize}
  \item Pay difference to next Tier (requires downtime scene).
\end{itemize}

\textbf{Spending:}
\begin{itemize}
  \item Use Trust XP to buy Assets and Followers (4/8/9 XP).
  \item Each consumes one slot.
\end{itemize}

\textbf{Upkeep:}
\begin{itemize}
  \item Pay 1 XP or 1 scene per asset/follower per downtime.
  \item Or delegate to a Cap 5 follower (manages 4 slots).
\end{itemize}

\textbf{Activation:}
\begin{itemize}
  \item Any Trust member spends their own Boons to activate.
\end{itemize}
\end{tcolorbox}
...
\subsubsection{When to Tick the Crisis Timer }
...
\subsubsection{Reducing the Crisis Timer }
...
For player-facing rules on Echo Characters (how to declare absence, what a Downtime Log is, and the benefits of returning), see the Player's Guide, Chapter \ref{ch:running-assets-followers}. The player version omits Crisis Timers and GM consequence mechanics; those are your tools as GM.