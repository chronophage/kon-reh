...
\begin{itemize}
  ...
  \item Neglect \rightarrow follower becomes \textit{Wary} (or \textit{Seized} if already Wary).
...
\end{description}
State: \textit{Maintained} (full) \rightarrow \textit{Neglected} (--1 die) \rightarrow \textit{Compromised} (unavailable).
...
\begin{itemize}
  ...
  \item Failure to pay \rightarrow asset becomes \textit{Neglected} (or \textit{Compromised} if already Neglected).
\end{itemize}
...
Not every patron is a cosmic entity. Powerful mortals---nobles, crime lords, generals, merchant princes---can act as \textbf{terrestrial patrons}. They offer tangible aid (escorts, legal protection, supplies, even ongoing Assets like a trade route or a safehouse) in exchange for favors.

\subsection*{Assets Without XP Cost, With Obligation}
A terrestrial patron can provide the same benefits as a Minor or Standard Asset (see \ref{subsec:assets-gm}), but the party does not pay XP for it. Instead, they accept a \textbf{Debt Clock [4]} (or larger, for major favors) and track Obligation to the patron just as they would to a supernatural Patron (see \ref{sec:obligation-corruption-gm}). The patron's \textquotedbl Rites\textquotedbl\ are favors; the \textquotedbl Obligation\textquotedbl\ is the growing weight of what the party owes.
...
The party also tracks a separate \textbf{Obligation} tally for each terrestrial patron, exactly as they would for a supernatural Patron (see \ref{sec:obligation-corruption-gm}). Each accepted favor typically adds +1 Obligation. When Obligation exceeds the character's (or party's) capacity, they mark Fatigue and the patron may send a reminder -- a messenger, a withdrawn favor, or a public hint that the debt is due.

Refusing to honor the debt turns the patron into an enemy (or at least a reliable antagonist). The GM may advance associated \textit{Crisis} clocks or introduce complications from the patron's faction.
...
\begin{itemize}
  \item \textbf{Early (1--2 segments):} A messenger arrives with a \textquotedbl reminder.\textquotedbl\ The patron asks for a small favor (deliver a package, spread a rumor).
...
Some campaigns may track a separate \textbf{Reputation Track} for each patron. High reputation grants better terms (more favors before the debt clock fills). Low reputation may cause the patron to demand immediate payment before offering any help. Reputation can be advanced by completing the patron's tasks on time or by publicly supporting their cause.

\subsection{Clearing Terrestrial Obligation}
Obligation to a terrestrial patron is cleared the same way as Obligation to a supernatural Patron: by performing an Act of Service during downtime. The service must be concrete -- repaying a loan, performing a public favor, or delivering a rival's secret. The GM sets the scope: a small favor clears 1--2 segments; a major undertaking may clear the entire clock.

\subsection{Cross-Reference}
\begin{itemize}
  \item For player-facing rules on terrestrial patrons, see (Player's Guide).
  \item For Asset upkeep and the XP alternative, see \ref{subsec:assets-gm} and.
  \item For Obligation mechanics, see \ref{sec:obligation-corruption-gm}.
\end{itemize}
