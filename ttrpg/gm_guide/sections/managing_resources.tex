...
Not every patron is a cosmic entity. Powerful mortals---nobles, crime lords, generals, merchant princes, and the old Stone-Sealed Families---can act as \textbf{terrestrial patrons}. They offer tangible aid (escorts, legal protection, supplies, even ongoing Assets) in exchange for \textbf{Influence} (the same mechanic as Obligation, tracked separately per patron).

\subsection{How Terrestrial Patrons Work}

When a character accepts a favor from a terrestrial patron, they mark \textbf{+1 Influence} for that patron. Influence capacity is the same as Obligation: \textbf{Spirit + Presence}. Exceeding capacity causes Fatigue (1 per segment over), and the character cannot call on that patron for further favors until they clear segments below capacity.

Unlike supernatural Patrons, terrestrial patrons also track a \textbf{Favor Timer [4--8]} (size based on the patron's power and the favor's scope). When the timer fills, the patron calls in the debt — a mission, a political favor, or a direct demand. The timer does not tick automatically; the GM advances it when the fiction demands (e.g., when the party is far from help, when they are already in crisis, or when refusing would be catastrophic).

\begin{tcolorbox}[colback=gray!5!white, colframe=gray!75!black, title={Use Favor Timers Aggressively}]
\index{Favor Timer!GM advice}
\textbf{Favor timers are story fuel.} Call them in at the worst possible moment:
\begin{itemize}
...
\end{itemize}
Favors that are never called are wasted potential.
\end{tcolorbox}

\subsection{Asset Slots and Patron Power}
...
\subsection{Clearing Terrestrial Influence}

Influence to a terrestrial patron is cleared by performing \textbf{tasks} for the patron. The GM sets the scope:
\begin{itemize}
...
\end{itemize}
Each task becomes a story hook. Refusing a task when Influence is full may turn the patron into an enemy.

\subsection{Examples of Terrestrial Patrons}

\begin{tcolorbox}[title=Sample Terrestrial Patrons, colback=white!97!gray, colframe=black!80!gray]
\begin{tabularx}{\linewidth}{lX}
\toprule
\textbf{Patron} & \textbf{Typical Favors and Influence Triggers} \\
\midrule
Prefect Marcellus (Ecktorian commander) & Safe passage, food, guards. \textit{Influence cost:} dangerous mission behind enemy lines. \\
Censor Cassia (judicial authority) & Legal immunity, archive access. \textit{Influence cost:} expose a political rival. \\
Khatun Sarnai (Ykrul chieftain) & Remounts, safe camp, guides. \textit{Influence cost:} blood-price or raid on a rival clan. \\
Mistress Ilvar (Silkstrand crime lord) & Black Bazaar access, cutpurses. \textit{Influence cost:} public assassination. \\
House Contarini (Vilikari Archivolt) & Sealed charters, document forgery. \textit{Influence cost:} recover a lost seal. \\
House Morosini (Vilikari bankers) & Letters of credit, market access. \textit{Influence cost:} collect a debt from a bankrupt noble. \\
\bottomrule
\end{tabularx}
\end{tcolorbox}
...
\begin{itemize}
  \item Track each patron's Influence total and Favor Timer visibly.
  \item When a player asks for a favor, state the cost: \textquotedbl That's worth 2 Influence and starts a Favor Timer [4]. Are you willing?\textquotedbl
  \item When a Favor Timer fills, give a hook that fits the patron's personality and the current situation.
  \item If the party refuses, tick a \textit{Crisis} timer or introduce a complication (the patron works against them).
  \item Terrestrial patrons can also be enemies — a party that refuses to pay may find its credit cut, its safehouses burned, or its members declared outlaws.
\end{itemize}
...
If the party disbands, remaining XP in the pool is lost. Assets may be transferred to individual characters who pay the XP cost (or accept Influence to a Terrestrial Patron to cover the difference).

\subsection{Trusts and Terrestrial Patrons}

A Trust can itself become a Terrestrial Patron for the party — or for other characters. A mercenary company Trust that reaches sufficient scale (Standard Asset + Cap 3 Follower) may begin accepting its own favors in exchange for Influence. The GM should treat this as a narrative milestone, not a mechanical formula.

\subsection{Example: The Silver Crane Trust}

A party of four contributes 2 XP each (8 XP total) to form a Trust. They purchase a Standard Asset: \textit{Silver Crane Trading Guild} (8 XP), gaining a warehouse in Silkstrand, a caravan route to Fhara, and a Cap 3 factor who manages operations. The factor handles upkeep for the warehouse, freeing the party to adventure. Later, they add a Minor Asset: \textit{Hidden Cache in the Valewood} (4 XP) using pooled XP. The factor cannot manage the cache (it is outside their territory), so the party must maintain it themselves or hire a second follower.

When the party retires, they may transfer the Trust's assets to a single character who pays the XP cost — or dissolve the Trust and let the assets pass to a Terrestrial Patron (e.g., House Contarini) in exchange for clearing their Influence.
