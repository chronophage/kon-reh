...
Corruption (Cantors, Pact-Whisperers) fills from pushes, resonant rites, and your SB spends. When the clock fills (see \ref{subsec:corruption-blooms}):
\begin{itemize}
...
\begin{itemize}
  \item Track the Corruption Clock only when the player marks it (they manage it per).
...
\begin{itemize}
  ...
  \item Track the cult's health with a \textbf{Schism Clock [4]}. Tick it when the player ignores the cult (no contact, refuses a request) or when the cult's demands go unmet. On a full clock, introduce a fracture: a rival sect, false rumors, or alliance with a Patron's enemy.
