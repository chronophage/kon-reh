...
\begin{itemize}
  \item Track the Corruption Timer only when the player marks it (they manage it per).
...
\section{GM Guidance: Healing as Story Fuel}
\label{sec:gm-healing}
\index{GM!healing}

This section is for Game Masters. It contains advice on using healing as a narrative tool.

\subsection{When to Complicate Healing}

\begin{itemize}
  \item \textbf{Use 1 Story Beat:} A faint scar, a lingering ache, a recurring dream.
  \item \textbf{Use 2 Story Beats:} The wound attracts a minor spirit. The patient gains a thematic Condition (e.g., Phantom Pain).
  \item \textbf{Use 3 Story Beats:} The healing draws attention from a rival, a witch-hunter, or a Patron who disapproves.
  \item \textbf{Use 4+ Story Beats:} Reality warps briefly. Something *else* is healed --- or not.
\end{itemize}

\subsection{Denying Healing}

\begin{itemize}
  \item \textbf{The wound is cursed.} No magic works until the curse is broken.
  \item \textbf{The healer lacks the right tags.} \texttt{[HEAL]} alone cannot mend a broken soul.
  \item \textbf{The patient refuses.} Emotional wounds cannot be forced.
  \item \textbf{The place is wrong.} Healing on sacred ground, in a liminal space, or during a celestial event may change the outcome.
\end{itemize}

\subsection{Ritual Requirements}

For major healing (Harm 2+), require:
\begin{itemize}
  \item A rare component (tears of a phoenix, water from a crossroads at midnight)
  \item A witness (the patient must confess a truth)
  \item A price (a memory, a year of life, a promise to a spirit)
\end{itemize}