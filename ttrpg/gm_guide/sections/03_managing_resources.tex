...
\begin{itemize}
  \item Extended travel through hostile lands without proper provisioning (see Travel Framework, \ref{ch:gm-travel}).
...
\begin{itemize}
  ...
  \item Medical attention (\textbf{Presence + Medicine}, DV 2) during downtime can remove 1 Fatigue.
\end{itemize}
...
\begin{itemize}
  \item On a \textbf{Partial} (1 Boon) or \textbf{Miss} (2 Boons) with meaningful consequences (see “Fail Forward” in the \textit{Player’s Guide}).
...
\end{itemize}
If a player has no Boons remaining, they may substitute \textbf{1 Fatigue} for 1 Boon (GM may veto if the fiction does not support it).
...
\begin{itemize}
  \item If the party \textbf{enters} a scene with \(\geq 3\) structural advantages, the GM \textit{may} apply Over‑Stack once for that scene.
