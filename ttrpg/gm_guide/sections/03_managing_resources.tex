...
Not every patron is a cosmic entity. Powerful mortals—nobles, crime lords, generals, merchant princes—can act as \textbf{terrestrial patrons}. They offer tangible aid (escorts, legal protection, supplies) in exchange for favours. Treat them as a \textbf{Debt Clock [4]}.
...
\begin{itemize}
  \item Track each patron’s Debt Clock on a visible handout.
  \item When a player asks for a favour, tell them the cost: “Marcellus will give you the keys to the armory, but that’s worth one segment. Are you willing?”
  \item When a Debt Clock fills, the favour is due. Give the players a hook that fits the patron’s personality and resources.
  \item If the party refuses, tick the associated \textit{Crisis} clock or introduce a complication (e.g., loss of safe passage, arrest warrant, hired thugs).
\end{itemize}

\subsection{Optional: Terrestrial Patron Reputation}
Some campaigns may track a separate \textbf{Reputation Track} for each patron. High reputation grants better terms (more favours before the debt clock fills). Low reputation may cause the patron to demand immediate payment before offering any help.
...
\begin{tcolorbox}[title=XP Award Guidelines, colback=white!97!gray, colframe=black!80!gray]
\begin{tabularx}{\textwidth}{lX}
\toprule
\textbf{Award} & \textbf{Range} \\
\midrule
Table Attendance & +2 XP \\
Major Objective Completed & +2–4 XP \\
