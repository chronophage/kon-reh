...
\small\textit{Example:} Benjamin (Body 3, Fatigue 3, Harm 0) takes Harm 1 in Heavy Armor. Conversion $\rightarrow$ Fatigue 1, remaining Harm 0 $\rightarrow$ Harm does not increase $\rightarrow$ Fatigue not cleared. Add Fatigue 1 $\rightarrow$ Fatigue = 4 ($\geq$ Body 3) $\rightarrow$ Overflow: Harm 0$\rightarrow$1, clear all Fatigue. Final: Harm 1, Fatigue 0.
\end{tcolorbox}
...
\begin{itemize}
  \item Instead of ``target loses 1 Boon'', say: ``The target hesitates — they lose their next action.''
  \item Instead of ``target marks 1 Fatigue'', say: ``The target staggers, winded. Their next attack is at \texttt{Desperate Position}.''
  \item Instead of ``target loses 1 Boon and marks 1 Fatigue'', say: ``The target drops their weapon (disarmed) and stumbles back one range band.''
\end{itemize}
...
\begin{itemize}
  \item When an NPC would use a special ability, look at its threat level (TL) and the fiction.
  \item \textbf{Spend SB} from your pool (earned from player 1s) to trigger the effect.
  \item \textbf{Narrate first} — describe what the NPC does, then call for a player roll or apply a consequence.
  \item \textbf{Don't roll for the NPC.} The player always rolls to resist or avoid.
\end{itemize}
...
\begin{itemize}
  \item \textbf{Two or more active scene‑long buff spells or rites} on the party (e.g., \texttt{[FORTIFY]}, \texttt{[VEIL]}).
  \item \textbf{An unused Follower Initiative} carried over from a previous scene (i.e., the follower is present but hasn't acted yet).
  \item \textbf{An Asset activation} that applies a mechanical bonus at scene start (e.g., ``My spy network delayed the patrol'' — the patrol is now at a disadvantage).
  \item \textbf{The opponent is Surprised, Trapped, or otherwise disadvantaged} by prior setup (e.g., you scouted the ambush point and prepared an escape route).
  \item \textbf{A ritual completed before the scene} that directly affects the scene's conditions (e.g., a consecrated circle, a warding).
\end{itemize}

\paragraph{Do NOT count as structural advantages:}
\begin{itemize}
  \item A single buff or spell (one is fine).
  \item Advantages created \textbf{during} the scene (e.g., knocking an enemy prone, taking cover).
  \item Flavor or roleplay preparations (e.g., ``I sharpened my sword'' — no mechanical benefit).
  \item Followers who are actively participating on‑screen (they are characters, not structural advantages).
\end{itemize}
...
\begin{itemize}
  \item \textbf{Increase Difficulty:} Start one named obstacle or challenge at \textbf{+1 DV} (e.g., the already well‑guarded door now has DV 4 instead of 3).
  \item \textbf{Bank a Story Beat:} The GM gains \textbf{+1 SB} to spend during the scene (representing narrative pushback).
\end{itemize}
...
\begin{itemize}
  \item \textbf{Valid Over‑Stack:} The party has cast \texttt{[FORTIFY]} on everyone, activated a Safehouse Asset (``guards are bribed''), and a Follower Initiative left unused from a previous scene. That's 3 structural advantages. The GM decides the gatekeeper's suspicion is higher (+1 DV to the Persuasion roll).

  \item \textbf{No Over‑Stack:} The party enters a room quietly, then uses a \texttt{Setup} action to knock over a brazier, creating a distraction. The distraction was created \textbf{during} the scene — no Over‑Stack applies.

  \item \textbf{Optional Choice:} The GM may choose \textbf{not} to apply Over‑Stack if the scene is already difficult or if the players' preparation is particularly creative. The rule is a dial, not a mandate.
\end{itemize}
...
\begin{itemize}
  \item \textbf{Discourage pre‑scene buff stacking} without eliminating preparation entirely.
  \item \textbf{Reward improvisation} — advantages created during the scene are always free.
  \item \textbf{Keep scenes tense} — a single small penalty prevents the party from trivializing every obstacle before dice are rolled.
\end{itemize}
