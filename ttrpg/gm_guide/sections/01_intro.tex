...
\begin{itemize}
    ...
    \item A \textbf{Partial} means you make progress, but the situation remains unresolved—you can open the lock but it will take time and risk discovery, or you can sway part of the crowd but alienate another faction.
    \item A \textbf{Miss} means you fail, and the situation escalates—a new threat appears, a hidden factor is revealed, the stakes are raised.
\end{itemize}
...
Your primary tool for managing this risk is the \textbf{Story Beat (SB)}\index{Story Beats (SB)} economy. When the dice show a 1, it is not merely a failure; it is the world reacting. The GM gains SB to introduce complications, escalate existing threats, or reveal hidden dangers. \textbf{Important:} SB are generated on \emph{any} roll that produces a 1, whether the overall roll succeeds or fails. They are not punishments; they are fuel for an unpredictable, responsive narrative.
...
\begin{itemize}
    \item \textbf{The Dice Pool:} Players roll a number of d10s equal to an Attribute + a Skill. Each die showing 6–10 counts as a \textbf{Success}. Each die showing 1 generates a \textbf{Story Beat (SB)} for you to spend. Compare the total number of Successes to a Difficulty Value (DV) set by the stakes.
    
    \item \textbf{Position \& Effect:} Before a roll, you set the character's \textbf{Position} (Dominant, Controlled, or Desperate), which defines the stakes of failure and may grant a re‑roll (failure for Dominant, success for Desperate). You also set their \textbf{Effect} (Limited, Standard, or Great), which describes what a clean success will achieve.
...
\begin{itemize}
    \item The first chapters cover the core principles and basic procedures.
    \item Later chapters delve into advanced systems for conflict, travel, and long‑term play.
    \item The final sections provide guidance for high‑tier campaigns, world‑building, and the specific setting of the Amaranthine Sea region.
    \item The appendices offer practical advice for running scenarios and a comprehensive collection of tools and tables.
\end{itemize}

You don't need to memorize everything. Use this book as a reference. Return to it when you need clarification or inspiration. The most important chapters to internalize are those on the core philosophy (this chapter) and the basic action resolution.
...
Flavor enriches the narrative and makes the world feel real and lived-in. It doesn't change the mechanical outcome, but it defines the \textit{how} and the \textit{why}. The GM should encourage this and reciprocate by painting vivid pictures of the world.
...
\hline
Skill Checks & Roll Attribute+Skill dice pool. Count successes (6+). Compare total successes to Difficulty Value (DV). Any 1s give the GM Story Beats (SB). \\
\hline
...
\hline
Spell Slots / Mana & Magic uses the same core system. Any 1s on a magic roll generate SB as normal. Powerful spells may require extra time, resources, or additional risk. \\
\hline
