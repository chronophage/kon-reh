...
\begin{itemize}
    \item \textbf{Zones of Control (ZoC)}: Each character exerts control over the 8 adjacent squares. Enemies cannot move \textit{through} your ZoC without engaging you or using \textbf{Push-through} (below). They may move \textit{around} it.
    \item \textbf{Movement}: Move $5 + \text{Body}$ squares. Difficult terrain (mud, rubble) halves this or requires an Athletics roll (GM's call).
    \item \textbf{Ranges}: 
        \begin{itemize}
            \item \textbf{Engaged}: Same or adjacent square (melee).
            \item \textbf{Near}: Within $5+\text{Body}$ squares (thrown, short charges).
            \item \textbf{Far}: Requires a full Move to reach \textbf{Near} (bows).
            \item \textbf{Distant}: Beyond \textbf{Far}; requires multiple Moves.
        \end{itemize}
\end{itemize}
...
\begin{itemize}
    \item \textbf{Engage}: Enter an enemy's ZoC to begin melee.
    \item \textbf{Attack}: Roll normally. Position is set by the fiction (flanking an enemy may improve your Position to \textbf{Controlled}).
    \item \textbf{Disengage}: Spend your Action to leave an enemy's ZoC safely.
    \item \textbf{Take Cover}: Move behind terrain to improve Position against ranged attacks (e.g., \textbf{Desperate} $\rightarrow$ \textbf{Controlled}).
    \item \textbf{Flank}: Allies on opposite sides of a target grant each other $+1$ die on attacks (max $+3$ total assistance).
\end{itemize}

\subsection*{Push-Through}

You may force movement through an enemy's ZoC without Disengaging by accepting risk:

\begin{itemize}
    \item \textbf{Test}: Roll $\text{Body} + \text{Athletics}$ vs.\ DV $= 2 + \text{enemy's Tier}$ (or Cap for Followers/summons).
    \item \textbf{Cost}: Choose one:
        \begin{itemize}
            \item Reduce your Position by one step (minimum \textbf{Desperate}) on your next action this turn, \textbf{or}
            \item Suffer $-1$ die on your next roll, \textbf{or}
            \item Grant the enemy an immediate opportunity attack at $+1$ die.
        \end{itemize}
    \item \textbf{Failure}: You stop in the enemy's ZoC and are \textbf{Engaged} (your movement ends).
\end{itemize}

\textit{\textbf{Tier Scaling}: At Tier IV+, you may spend $1$ Boon to ignore ZoC restrictions for one Move action, treating the grid as narrative space for that beat only.}
