...
\section*{GM Guide: Position as Narrative Leverage}
...
\begin{tcolorbox}[colback=yellow!5!white, colframe=yellow!75!black, title=⛔ Warning: Don’t Inflate Difficulty Values]
  \index{DV!inflation warning}\index{GM guidance!scaling}
  
  As characters grow more competent, it can be tempting to raise Difficulty Values (DV) to “keep them challenged.” **Resist that urge.**
  
  \medskip
  \noindent\textbf{Why DV inflation breaks the game:}
  \begin{itemize}
    \item Higher DV reduces the frequency of Partials and Misses, which are the primary sources of **Boons**. Fewer Boons means players have fewer resources to spend on re‑rolls, assets, and creative moves – their competence becomes a liability.
    \item It punishes players for investing in high Attributes and Skills, making their growth feel pointless.
    \item It forces you to inflate DVs again next session, creating an arms race that only hurts player satisfaction.
  \end{itemize}
  
  \medskip
  \noindent\textbf{How Fate’s Edge naturally scales challenge:}
  \begin{enumerate}
    \item \textbf{Higher die pools already generate more Story Beats (SB).} A seasoned character rolls 8–10 dice, so each action produces multiple 1s on average. Those SB are your primary tool for complicating success – spend them to introduce twists, escalate clocks, or trigger Patron intrusions.
    \item \textbf{Success becomes cleaner, so Boons become scarcer.} As players miss less, they earn fewer Boons. That scarcity is the real resource pressure at high tiers – not higher DVs.
    \item \textbf{Tags and critical skills already inflate DV when appropriate.} A \texttt{[DANGEROUS]} tag (e.g., \texttt{[TELEPORT]}) adds +2 DV by itself. Complex rituals or large‑scale effects naturally call for DV 4–5. Trust the tag system.
  \end{enumerate}
  
  \medskip
  \noindent\textbf{What to do instead:}
  \begin{itemize}
    \item Use \textbf{Clocks} to add time pressure and branching consequences.
    \item Spend SB aggressively – a single roll can tick multiple clocks or trigger a Patron demand.
    \item Worsen \textbf{Position} when the fiction calls for danger, not DV.
    \item Introduce \textbf{tactical complications} (reinforcements, environmental shifts, hostage situations).
  \end{itemize}
  
  \medskip
  \noindent\textbf{Golden rule:} 
  \emph{Let the dice pools grow. Let the SB flow. Let the Boons become precious. Your game will scale naturally.}
  \end{tcolorbox}
  
\subsection{GM Mindset}

\begin{itemize}
\item \textbf{At Tier I:} Focus on personal stakes—how does Position affect the character's survival?
\item \textbf{At Tier III:} Focus on regional impact—how does Position shape the city or region?
\item \textbf{At Tier V:} Focus on world-shaping consequences—how does Position reshape reality?
\end{itemize}

Position is not a progression system. It's a lens through which you frame the narrative. The mechanics stay consistent so the story can grow as large as your imagination allows.

\begin{tcolorbox}[colback=blue!5!white, colframe=blue!75!black, boxrule=0.5pt, arc=2mm]
\textbf{Golden Rule:} Never add mechanical bonuses to Position. Instead, deepen the narrative weight:

\begin{itemize}
\item \textbf{Tier I:} "Position shows whether you live or die"
\item \textbf{Tier V:} "Position shows how the world bends to your will"
\end{itemize}
\end{tcolorbox}
