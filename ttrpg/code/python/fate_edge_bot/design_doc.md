...
5. **Resource Tracking**
   ```
   Prompt: "Create a resource manager for Fate's Edge that tracks: supply timer (0-4), character fatigue, character harm, boons (0-5), and story beats"
   ```

### Day 5-6: Game Systems
6. **Combat System**
   ```
   Prompt: "Create a combat manager for Fate's Edge that tracks: position states (Dominant/Controlled/Desperate), range bands (Close/Near/Far), tactical timers (Mob Overwhelm, etc.), and party conditions"
   ```
...
```
"Create a combat timer system similar to the supply timer but with named timers like 'Mob Overwhelm' and 'Morale Collapse'"
```
