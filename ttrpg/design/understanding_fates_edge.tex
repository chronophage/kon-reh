...
\begin{itemize}
    ...
    \item \textbf{Assets \& Followers:} At high tiers, players shifted XP from personal attributes to followers and assets. A Cap 3 follower provides +3 assist dice for 9 XP, cheaper than raising an attribute from 4→5 (15 XP for +1 die). This emergent behavior is intended.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Resource Cycle:} Partial → 1 Boon → spend Boon to improve Position → fewer failures → fewer Boons. Closes the loop and encourages active resource use.
...
\begin{itemize}
    ...
    \item \textbf{Resource conversion loops:} Boons → XP → Boons? We capped XP conversion and reset Boons per scene to prevent farming.
\end{itemize}
...
\begin{enumerate}
    \item Lyra rolls [9,6,4,2,1] → successes: 9,6 = 2 successes (DV 3 → Partial). She gains 1 Boon. The 1 gives the GM 1 SB.
...
\textbf{Heuristic}: 4 dice is unreliable. Spend a Boon to improve Position (Controlled → Dominant) before rolling: re-roll one failure effectively adds ~1 die, raising success to ~66\%. Better than re-rolling after.
