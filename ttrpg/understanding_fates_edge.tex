...
\begin{itemize}
    ...
    \item \textbf{Corruption \& Spirit:} Cantors use Spirit only for their Corruption clock size, not for their Performance rolls. This makes Cantors MAD (Wits, Presence, Spirit) but gives them a safety valve (larger clock). Balanced by the fact that they never pay Obligation.
...
\begin{itemize}
    ...
    \item \textbf{High tiers (III–IV):} Players stopped rolling because success was nearly certain. The shift to SB spending as the primary difficulty lever (clocks, complications) brought tension back. GMs learned to spend SB on narrative twists, not just penalties.
...
\begin{itemize}
    ...
    \item \textbf{Visible Tension:} Clocks advance on GM SB – the GM is forced to show pressure building (e.g., “Reinforcements clock ticks +1”).
    \item \textbf{Anti‑Death Spiral:} Harm clears Fatigue. Model: injury as a reset, not a penalty.
\end{itemize}

\subsection{Anti‑patterns (Avoid)}
\begin{itemize}
    \item \textbf{Roll to avoid rolling:} A spell or talent that prevents all future rolls (e.g., “auto‑succeed on all stealth checks”). Instead, use clocks or escalating costs (e.g., “Each use advances a Suspicion clock”).
