...
\begin{enumerate}
    ...
    \item GM sets Position (Dominant/Controlled/Desperate)
...
\begin{enumerate}
    \item Set Position (Dominant/Controlled/Desperate) and DV (2--5+).
...
\subsection{The GM Quick-Kit: Fast, Fluid Adjudication}
\label{subsec:gm-quick-kit}
\index{GM Tools}

When the game is moving fast, you do not need deep rules recall—you need simple tools that make confident decisions.  
The Quick-Kit turns the entire SRD into fast, actionable choices.

% ------------------------------------------------------
\subsubsection{1. Five-Second Position \& Effect}
\index{Position}
\index{Effect}
...
\begin{description}
  \item[What is the Risk?] \hfill \\
    \textbf{Controlled:} Safe, prepared, low threat \\
    \textbf{Risky (default):} Pressure, danger, uncertainty \\
    \textbf{Desperate:} Immediate danger, overmatched, exposed
  \item[What is the Impact?] \hfill \\
    \textbf{Limited:} Partial progress or minor effect \\
    \textbf{Standard (default):} Normal success \\
    \textbf{Great:} Powerful, overwhelming, high-impact
\end{description}

\noindent
\textit{Example:}  
A player charges a cultist across open ground.  
Risk = standard danger ($\Rightarrow$ Risky).  
Impact = standard attack ($\Rightarrow$ Standard).  
\textbf{Risky / Standard. Roll.}

% ------------------------------------------------------
\subsubsection{2. The Lazy DV Table}
\index{Difficulty Value}

Pick DV based on narrative weight, not math.
...
% ------------------------------------------------------
\subsubsection{4. The Three-Clock Rule}
\index{Clocks}

Only three active clocks at once:
\begin{itemize}
  \item One for the \textbf{Scene} (e.g., Guards Alerted [4])
  \item One for the \textbf{Journey} (e.g., Mist Encroaches [6])
  \item One for the \textbf{Campaign} (e.g., Baron's Suspicion [8])
\end{itemize}

If a new complication appears, fold it into an existing clock.  
This keeps the table focused and the fiction tight.

% ------------------------------------------------------
\subsubsection{5. Magic Adjudication Shortcut}
\index{Magic}
...
\begin{enumerate}
  ...
  \item Assign a fitting \textbf{TAG} (Veil, Ward, Barrier, Glamour, etc.)
...
Example: Creating a force wall under fire?  
TAG = Barrier, scale = Scene $\Rightarrow$ DV 3.  
Casting in melee $\Rightarrow$ Risky.  
\textbf{Risky / Standard, DV 3. Roll.}

% ------------------------------------------------------
\paragraph{The Director Mindset.}
\index{GM Philosophy}

You are not a rules engine.  
You are a director choosing the next shot:
\begin{itemize}
  \item Clean win
  \item Costly win
...
Use the Quick-Kit to make fast decisions and keep the story moving.