...
\begin{itemize}
  ...
  \item Lyra rolls [9, 6, 4, 2, 1] (Wits 4 + Tinker 2 = 6 dice) $\rightarrow$ 2 successes (DV 3 Partial) $\rightarrow$ \textbf{Gains 1 Boon}.
...
\end{itemize}
$\rightarrow$ \textit{Teaches:} Partial success = progress + Boon; 1s = GM currency for complications.

\paragraph{Arcane Backlash (Sera the Caster, Section~\texttt{[MISSING REF: sec:quickbuild-caster]}):}
\begin{itemize}
  \item Sera casts [FIRE][STRIKE] (DV 3) vs. a fire elemental $\rightarrow$ rolls [10, 1, 1] (Wits 4 + Arcana 2 = 6 dice) $\rightarrow$ 3 successes (10=2, 1s=2 SB) $\rightarrow$ \textbf{Success with SB} (GM spends 1 SB for complication).
  \item Backlash: Dominant element is Fire $\rightarrow$ GM describes: ``The bolt hits, but the flash blinds you---your next action is Desperate. Smoke triggers a hanging lantern; the room starts catching fire (Clock: Fire Spread [4]).''
\end{itemize}
$\rightarrow$ \textit{Teaches:} 1s generate SB \emph{even on success}; elemental backlash coloring; SB spent on visible consequences.
