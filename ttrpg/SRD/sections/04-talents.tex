...
\begin{itemize}
  \item \textbf{2 XP} — Minor edge (e.g., Spellcraft, +1 situational bonus, small narrative trick).
...
\begin{description}[leftmargin=1.5em, style=nextline]
  \item[Spellcraft (6 XP) \textnormal{[FIRE] [WATER]}:] Grants access to Weave \& Cast freeform spellcasting using the Eight Elements. Without this, characters cannot freeform cast.
  \item[Familiar (2 XP) \textnormal{[SPIRIT] [BOND]}:] Required to access Patron features such as \emph{Patron's Gift}. Binds a Thiasos.
  \item[Codex (4 XP) \textnormal{[FATE] [RITUAL]}:] Required to fully join a Patron's service as a Runekeeper. Grants access to that Patron's Rites and Obligation system.
  \item[Patron's Symbol (4 XP) \textnormal{[WARD] [SIGIL]}:] Minor Asset. Allows an Invoker to access a Patron's Rites via ritual precision. Each Patron requires its own Symbol.
\end{description}

\subsection{Imbuements}
\textbf{Patron's Gift (Free, Requires Thiasos) \textnormal{[BOND] [POWER]}}\\
...
\subsubsection*{Light Fingers \textnormal{(3 XP) \textnormal{[STEALTH] [MANIPULATE]}}}
...
\subsubsection*{Face Like Water \textnormal{(4 XP) \textnormal{[DISGUISE] [SILENCE]}}}
...
\begin{itemize}
  ...
  \item Ignore any Harm or Fatigue penalties while raging.
  \item Lasts 3 rounds.
...
\subsubsection{Hollow-Touched (8 XP) \textnormal{[AELAEREM] [THRESHOLD]}} \textbf{Requirements:} Aelaerem affinity, Spirit 3+, Stealth 2+ \ \textbf{Effect:} When you would take \textbf{Harm}, you may instead \textbf{mark that amount as Fatigue} and choose one: the GM gains \textbf{1 Story Beat} (the Neighbors whisper of your debt in the root-ways), or mark \textbf{1 Obligation} to the \textbf{Pale Shepherd}. If this conversion fills your Fatigue track, resolve normal overflow (clear Fatigue, increase Harm by 1) and immediately suffer the chosen cost as the Hollow claims its tithe. When using \textbf{Protect} (page 365) for an ally, you may have them \textbf{mark 1 Fatigue} in your place; if they do, they are \textbf{bound by red thread} until next dawn and cannot willingly leave \textbf{Near} range without taking \textbf{1 Harm}. \ \textbf{Narrative:} You do not bleed on the threshold---you step through it, and the earth drinks your debt.
...
\subsubsection{Stone-Bound Interdictor (8 XP) \textnormal{[AELINNEL] [WARD]}} \textbf{Requirements:} Aelinnel affinity, Body 3+, Wits 3+ \ \textbf{Effect:} When you would take \textbf{Harm}, you may instead \textbf{mark that amount as Fatigue}. If you do, choose one: \begin{itemize} \item \textbf{Exact Change:} Spend \textbf{1 Boon} immediately (you pay the precise toll to the earth). \item \textbf{Geas:} Speak a binding promise aloud (e.g., ``I shall not pass this threshold again until dawn''). Until you fulfill this geas, you suffer \textbf{--1 die} to all social rolls with the fae and stone-kin. \end{itemize} \textbf{Overflow Cost:} If this conversion fills your Fatigue track, resolve the normal overflow (clear Fatigue, increase Harm by 1) and immediately suffer the chosen cost as the ledger demands balance.

\textbf{Communal Binding:} If you use this ability while performing the \textbf{Protect} action (page 365) for an ally, you may choose to have that ally \textbf{mark 1 Fatigue} in your place. If they do, they are bound by a \textbf{spoken geas} of your choosing (a single, simple imperative) until next dawn; breaking it costs them \textbf{1 Harm} (the stones remember the broken word).

\subsubsection{Large Soul (8 XP) \textnormal{[PRESTIGE]}} \textbf{Requirements:} Aelaerem or Aelinnel affinity, Body 4+, Spirit 3+ \ \textbf{Effect:} You may wear \textbf{Heavy Armor} and wield \textbf{Heavy Weapons} without restriction. However, you lose your Small Folk racial bonuses: you no longer gain \textbf{+1 Position when Dodging} or \textbf{+1 die to Hide/Evasion rolls while in Cover}. You have grown too solid, too grounded---the earth holds you with the same weight it grants to taller folk. \ \textbf{Narrative:} You have eaten the stone-bread or spoken the heavy word; your soul has grown large enough to fill the armor of the big folk. But the threshold no longer yields to your step, and the shadows no longer part for you as they once did.
...
\emph{Sable earns 5 XP from fulfilling a Drive and 1 XP from trading in 2 Boons at session's end. He now has 4 XP. He buys the Spellcraft (6 XP) and saves the other 2 XP toward a Patron's Symbol. Next downtime, he will be able to invoke Ikasha's rites through ritual.}
...
\item Total enchantments cannot exceed item's Tier (1-5, based on quality/rarity)
\end{itemize}
...
\item Examples: +1 die to specific rolls, minor damage resistance, [TAG] once/session
\end{itemize}
...
\item Examples: Ignore armor, special damage types, area effects, [TAG] with limitations
\end{itemize}
...
\begin{itemize}
\item \textbf{Keen Edge (2 XP) \textnormal{[SHARP] [STRIKE]}:} +1 die to hit against armored targets
\item \textbf{Flaming Blade (4 XP) \textnormal{[FIRE] [BURNING]}:} Deals +1 Fire damage, +1 Effect vs cold creatures
\item \textbf{Soulfire Weapon (6 XP) \textnormal{[SPIRIT] [DAMAGE]}:} Ignores 1 point of armor/resistance, deals Spirit damage
\item \textbf{Thunder Hammer (8 XP) \textnormal{[STRIKE] [THUNDER]}:} On 10+, knockback adjacent targets (Hazard +2)
\item \textbf{Luckblade (4 XP) \textnormal{[LUCK] [STRIKE]}:} Once/scene, re-roll 1s on damage dice
\end{itemize}

\textbf{Armor Enchantments:}
\begin{itemize}
\item \textbf{Shadowweave (2 XP) \textnormal{[STEALTH] [SILHOUETTE]}:} +1 die to Stealth rolls while moving silently
\item \textbf{Runed Plate (4 XP) \textnormal{[WARD] [PROTECTION]}:} Reduce magical Backlash by 1 SB
\item \textbf{Wraithmail (8 XP) \textnormal{[PHASE] [INVULNERABILITY]}:} Once/scene, become intangible for 1 exchange
\item \textbf{Spellweave Cloak (6 XP) \textnormal{[WARD] [RESISTANCE]}:} +2 dice to resist one chosen magical effect
\end{itemize}

\textbf{Gear Enchantments:}
\begin{itemize}
\item \textbf{Boots of Sure Step (3 XP) \textnormal{[STABILITY] [MOVEMENT]}:} Ignore difficult terrain effects
\item \textbf{Lantern of Revelation (5 XP) \textnormal{[REVEAL] [LIGHT]}:} Reveal [VEIL] effects within Near range
\item \textbf{Ring of Sustenance (4 XP) \textnormal{[ENDURANCE] [RESISTANCE]}:} Ignore first level of Fatigue each scene
\end{itemize}

\subsubsection{Enchantment Limitations}

\begin{itemize}
\item Maximum enchantments = Item's Tier (Light/Minor=1, Medium/Standard=2, Heavy/Major=3)
\item Enchantments on same item cannot stack if they provide identical benefits
\item Damaged/Neglected equipment may lose enchantment benefits temporarily (-1 die to use)
\item Compromised equipment loses all enchantment benefits until repaired
\item Removing enchantments requires ritual (Wits + Arcana DV 4) and costs 1 XP per enchantment level
\item Enchanted items count as Assets for upkeep purposes (see §8.3)
\end{itemize}
...
\begin{itemize}
\item Minor (2-4 XP): Equivalent to small narrative tricks or situational bonuses (+1 die, [TAG] limited)
\item Major (6-8 XP): Comparable to significant mechanical edges or new capabilities ([WARD], [COMMAND] minor)
\item Prestige (10+ XP): Campaign-defining effects similar to Patron Rites or Epic Talents ([BANISH], [REALITY] minor)
\end{itemize}

Price enchantments relative to their mechanical impact and campaign power level. Consider the item's base Tier as a cost modifier—higher Tier items can support more powerful enchantments at the same XP cost.