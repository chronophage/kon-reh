...
\begin{fatebox}[Difficulty Ladder]\index{Difficulty Ladder}
  ...
  \midrule
  2 & Routine: clear intent, modest stakes, stable setting \\
  3 & Pressured: time limits, mild resistance, incomplete information \\
...
\begin{description}
\item[\indexterm{1--3 Fatigue}] Each Fatigue step worsens your \textbf{Position} by one level 
(Controlled $\rightarrow$ Risky $\rightarrow$ Desperate). 
If you are already Desperate, instead apply a --1 die penalty per Fatigue. 
\end{description}
...
Each character takes \emph{two Actions} on their turn, in any order; repeating the same Action is allowed unless noted.
