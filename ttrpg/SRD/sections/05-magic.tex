...
\noindent
\textbf{Lesser Imbuement.}
Borrowed Grace allows an Invoker to momentarily channel their Patron’s favor into a single decisive act.

\medskip
\begin{description}[leftmargin=1.25em]
  \item[Cost:] 1 Boon \emph{or} 1 Fatigue; 1 Action.
  \item[Effect:] Gain \textbf{+1 die} to a single combat action, \emph{or} gain \textbf{+1 Effect} to a single social or stealth action.
  \item[Duration:] One action.
  \item[Requirement:] The Invoker must visibly display their Patron’s symbol.
  \item[Limits:] Once per scene; the effect cannot be stacked, extended, or shared. If the Patron’s symbol is \emph{Compromised}, Borrowed Grace may still be invoked at \textbf{–1 die}.
\end{description}
