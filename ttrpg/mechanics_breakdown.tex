...
Successes $\geq$ DV, SB > 0 & \textbf{Success with Cost} \\
...
\begin{itemize}
    \item 1 SB: Minor complication (noise, distraction, +1 clock segment)
...
\textbf{Why they matter strategically:} Clocks turn abstract threats into \textit{tangible resources}. A ``Barrow Collapse [6]'' clock means:
\begin{itemize}
    \item You have 6 ``safe'' actions before the entrance seals
    \item Each failed roll or GM SB spend brings you closer to disaster
    \item Smart players treat clocks like \textit{countdown timers}—allocating actions to delay or accelerate based on goals
\end{itemize}

\textbf{High-level insight:} At higher tiers, clocks become \textit{the primary threat meter}. The GM doesn't just add complications—they use clocks to make pressure \textit{visible and inevitable}. Players must manage clock state alongside their resources.
...
\begin{itemize}
    ...
    \item \textbf{[Ward]}: Uses ward mechanics (DV = [Ward]'s base, usually 3) \textit{creates an integrity clock}—the shield can take hits before failing. \textbf{This is superior for sustained defense}—[Air][Defend] is one-time only.
\end{itemize}
