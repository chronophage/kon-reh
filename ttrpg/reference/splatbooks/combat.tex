...
\fancyhf{}
\rhead{\thepage}
\lhead{Fate's Edge Combat Compendium}
...
\subsubsection{Supply Lines Timer (8 segments)}
...
\subsubsection{Morale Timer (6 segments)}
...
\subsubsection{Strategic Position Timer (8 segments)}
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Controlled Position:} All units act normally, one unit can act as Cap 5
...
\begin{itemize}[leftmargin=*]
    ...
    \item 3-4 SB: Command structure compromised, -1 die to all units next round
...
\begin{itemize}
    ...
    \item Success: Enemy position shifts from Controlled to Controlled, or Controlled to Desperate
...
\begin{itemize}
    ...
    \item Successful opposed roll (Wits + Tactics vs. opponent's Wits + Tactics)
    \item Success: Allied units gain start Controlled for defensive actions, +1 to defense DV
...
\subsubsection{Fire Hazard Timer (6 segments)}
...
\begin{itemize}[leftmargin=*]
    \item Water/chemical suppression: Body + Athletics (DV 2), success reduces timer by 1
    \item Firebreak creation: Wits + Survival (DV 3), success reduces timer by 2
    \item Magical suppression: Wits + (Appropriate Art) (DV 2), success reduces timer by 1-3
\end{itemize}

\subsubsection{Flood Hazard Timer (8 segments)}
...
\begin{itemize}[leftmargin=*]
    \item Light cover: +1 to defense DV, start Controlled vs. ranged attacks
    \item Heavy cover: +2 defense DV, -1 die to attacker's actions
...
\subsubsection{Siege Conditions Timer (10 segments)}
...
\subsubsection{Rapid Supply Timer Filling}
...
\begin{itemize}
    ...
    \item Complications: 1-2 SB delays construction, 3+ SB causes partial collapse
\end{itemize} \\
\hline
Dig Approach Trench & 
\begin{itemize}
    ...
    \item Success: Provides cover for assault troops, -2 to defense DVer's ranged attacks
    \item Complications: 1-2 SB causes delays, 3+ SB results in cave-in or enemy counter-mining
\end{itemize} \\
\hline
Build Battering Ram & 
\begin{itemize}
...
\begin{itemize}[leftmargin=*]
    ...
    \item 3-4 SB: Noticeable setback (hazard timer, condition, new pressure)
...
\begin{itemize}[leftmargin=*]
    ...
    \item Timer creation: Magic can create or advance tactical timers
...
\subsubsection{War Timer Management}

\textbf{During Extended Conflicts:}
\begin{itemize}[leftmargin=*]
    \item Update timers at the end of each major engagement
    \item Players can influence timers through strategic actions
    \item Critical thresholds trigger narrative consequences
    \item Timers can be linked for cascading effects
\end{itemize}

\textbf{Victory Conditions:}
\begin{itemize}[leftmargin=*]
    \item Fill 2+ war timers to critical levels
