...
\begin{center}
{ŁARGE Sample Playthrough: The Broken Caravan}\\
\vspace{0.5em}
{łarge A Fate's Edge Annotated Narrative}
\end{center}
...
\gm{The highest rank is 6, so we have a 6−segment travel timer. The scene: The party approaches a bridge where two Free Company factions are engaged in a feud. They have a monastery letter that should grant them safe passage.}
...
\gm{That's a Controlled position − they're clearly on edge. Difficulty Value 2.}

\player{Kestra} Rolling Presence 3 + Sway 2 = 5 dice. \roll{5}{8, 6, 4, 1, 1} ≻ess{2} \cp{2}
...
\gm{Controlled position, DV 2. The hillside is rocky but offers some cover.}

\player{Theron} Rolling Wits 3 + Stealth 3 = 6 dice. \roll{6}{9, 7, 5, 3, 1, 1} ≻ess{3} \cp{2}
...
\player{Lyra} Rolling Body 4 + Melee 3 = 7 dice. \roll{7}{10, 8, 6, 4, 3, 1, 1} ≻ess{4} \cp{2}
...
\player{Kestra} Rolling Wits 4 + Arcana 3 = 7 dice. \roll{7}{9, 7, 6, 4, 3, 1, 1} ≻ess{4} \cp{2}
...
\gm{Controlled position, DV 2. You're trying to reason with armed men while a bridge crumbles.}

\player{Theron} Rolling Wits 3 + Sway 2 = 5 dice. \roll{5}{7, 6, 4, 2, 1} ≻ess{3} \cp{1}
...
\player{Lyra} Rolling Body 4 + Athletics 2 = 6 dice. \roll{6}{8, 6, 5, 3, 1, 1} ≻ess{3} \cp{2}
...
\player{Kestra} Rolling Wits 4 + Survival 1 = 5 dice. \roll{5}{9, 6, 4, 3, 2} ≻ess{3} \cp{0}

øutcome{Clean Success}
...
\player{GM} Let's check our travel timer. We had 6 segments. We've resolved the major conflict, found an alternate route, and survived the encounter.
...
ℹtem \textbf{Risk as Drama}: Every successful action still generated complications through SB.
ℹtem \textbf{Player Agency}: The players had multiple approaches (diplomacy, stealth, direct action) that all contributed meaningfully.
ℹtem \textbf{Narrative Primacy}: The collapsing bridge wasn't just a mechanical effect − it changed the story and forced new decisions.
ℹtem \textbf{Meaningful Growth}: XP was earned through engagement with complications, not just success.
ℹtem \textbf{Collaborative Storytelling}: The glowing letter was a player detail that I incorporated into the fiction to create new possibilities.
\end{itemize}

\gm{Key mechanics shown:}
\begin{itemize}[leftmargin=*]
ℹtem Outcome Matrix application (Success & Cost was the most common result)
ℹtem Story Beat spending to add tension and story elements
ℹtem Description Ladder use (Intricate action for Kestra's geomancy)
ℹtem Travel timer advancement based on resolution quality
\end{itemize}
