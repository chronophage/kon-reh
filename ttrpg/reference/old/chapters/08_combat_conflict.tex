...
\begin{description}
\item[Minor (-)] Generate 1 SB on next 2 rolls\index{Combat!Harm!Minor}
\item[Moderate (=)] Generate 1 SB on next roll, -1 die to relevant actions\index{Combat!Harm!Moderate}
\item[Severe (≈)] Generate 2 SB on next roll, -2 dice to relevant actions\index{Combat!Harm!Severe}  
\item[Critical (†)] Generate 3 SB on next roll, out of action until treated\index{Combat!Harm!Critical}
\end{description}
...
\begin{itemize}
    \item \textbf{1 SB}: Shift position one step (GM choice)\index{Combat!Position Shift}
    \item \textbf{Player Spending}: 1 SB to improve position one step\index{Combat!Player Spending}
...
\begin{itemize}
    \item Channel/Weave Backlash SB applies to tactical situation\index{Magic!Combat}
...
\begin{itemize}
    \item \textbf{Follower Risk}: 2+ SB spent in combat can endanger assisting followers\index{Combat!Follower Risk}
...
\item[Success \& Cost] Intent achieved, but GM spends SB for combat consequences\index{Combat!Outcome!Success \& Cost}
...
\hline
Severe (>) & 2 SB on next roll & -2 dice to relevant actions & Extended medical care (DV 3) \\
\hline
Critical ($\dagger$) & 3 SB on next roll & Out of action until treated & Major medical intervention (DV 4) \\
\hline
\end{tabular}
\end{center}
