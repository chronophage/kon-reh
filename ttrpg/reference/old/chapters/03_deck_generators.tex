...
\begin{enumerate}
  \item Player rolls; each 1 generates a Story Beat (SB).\index{Story Beats!Generation}
  \item GM chooses one method for that roll:
  \begin{enumerate}
    \item \textbf{Direct Spend}: translate SB into immediate consequences/clock ticks; or\index{Story Beats!Direct Spend}
    \item \textbf{Deck Draw}: draw up to \textbf{min(SB, 3)} cards and \textbf{synthesize a single twist}\index{Story Beats!Deck Draw}
...
\item[All one color] GM gains 1 free Story Beat in that scene\index{Decks!Color Combos}
\end{description}
