...
\item \textbf{Execution} — Build a dice pool equal to \textbf{Attribute + Skill} and roll that many d10s. Each die showing \textbf{6+} counts as a success. Each \textbf{1} rolled generates \textbf{1 Complication Point (CP)} for the GM. \index{Core Mechanic!Execution}\index{Complication Point (CP)}
\item \textbf{Outcome} — The GM compares total successes to the task’s Difficulty Value (DV) and then spends CP to weave setbacks, collateral costs, or escalating danger. \index{Core Mechanic!Outcome}
\end{enumerate}
...
\begin{description}
\item[\textbf{Basic Action}] Roll the pool as-is. All 1s generate CP. \index{Description Ladder!Basic}
...
\noindent\textbf{Important:} Re-rolling 1s does \emph{not} remove CP already generated; new 1s add more CP. \index{Complication Point (CP)!Re-rolling 1s}
...
Let $S$ be successes ($\geq 6$) and $C$ be CP generated (number of 1s).
...
\item[\textbf{Success \& Cost}] ($S \geq DV$ and $C > 0$) — Grant the intent; the GM spends or banks CP to add friction (noise, time loss, resource wear, new observers). \index{Outcome Matrix!Success and Cost}
...
\item[\textbf{Miss}] ($S = 0$) — No direct progress. The GM spends or banks CP to introduce immediate consequences. \index{Outcome Matrix!Miss}
\end{description}

\section{CP Spend Menu}
\label{sec:cp-spend-menu}
\index{Complication Point (CP)!spend menu}

\subsection{Universal CP Options}
\label{subsec:universal-cp}
\index{Complication Point (CP)!universal}

\begin{description}
\item[\textbf{1 CP}] Minor pressure: suspicious noise, trace left behind, +1 Supply segment, minor time loss. \index{Complication Point (CP)!1 CP}
\item[\textbf{2 CP}] Moderate setback: alarm raised, lose favorable position/cover, lesser foe appears, added obstacle. \index{Complication Point (CP)!2 CP}
\item[\textbf{3 CP}] Serious trouble: reinforcements arrive, key gear breaks, significant complication introduced. \index{Complication Point (CP)!3 CP}
\item[\textbf{4+ CP}] Major turn: trap springs, authority arrives, scene shifts dramatically. \index{Complication Point (CP)!4+ CP}
\end{description}

\subsection{Combat-Specific CP Options}
\label{subsec:combat-cp}
\index{Complication Point (CP)!combat}

\begin{description}
\item[\textbf{1 CP}] Lose footing (next defense –1 die), minor environmental shift.
\item[\textbf{2 CP}] Weapon jam or battlefield momentum shifts (fire spreads, cave-in starts, cavalry arrives).
\item[\textbf{3 CP}] Pinned, disarmed, or separated from allies.
\item[\textbf{4+ CP}] Enemy reveals a special ability, terrain collapses, a major reinforcement wave hits.
\end{description}

\subsection{Stealth \& Intrusion CP Options}
\label{subsec:stealth-cp}
\index{Complication Point (CP)!stealth}

\begin{description}
\item[\textbf{1 CP}] Footstep heard, door squeaks, shadow noticed.
\item[\textbf{2 CP}] Patrol adjusts, lock resists (extra test), guard becomes suspicious.
\item[\textbf{3 CP}] Partial alarm triggered (localized response).
\item[\textbf{4 CP}] Full alarm and lockdown protocol.
\end{description}

\subsection{Social Interaction CP Options}
\label{subsec:social-cp}
\index{Complication Point (CP)!social}

\begin{description}
\item[\textbf{1 CP}] Faux pas (future interactions with this contact –1 die), rumor spreads.
\item[\textbf{2 CP}] Concession required (gift, favor, or compromise to proceed).
\item[\textbf{3 CP}] Rival interjects with leverage; negotiation turns against you.
\item[\textbf{4 CP}] Patron turns hostile; audience becomes antagonistic.
\end{description}

\subsection{Travel \& Survival CP Options}
\label{subsec:travel-cp}
\index{Complication Point (CP)!travel}

\begin{description}
\item[\textbf{1 CP}] Lose time, minor injury, weather worsens.
\item[\textbf{2 CP}] Supply clock +1 segment, mount lamed, gear damaged.
\item[\textbf{3 CP}] Wrong path or blocked pass; all characters gain Fatigue 1.
\item[\textbf{4 CP}] Major environmental event—storm, rockslide, flood—scene fundamentally changes.
\end{description}

\subsection{Arcana \& Ritual CP Options}
\label{subsec:arcana-cp}
\index{Complication Point (CP)!arcana}

\begin{description}
\item[\textbf{1 CP}] Backlash prickle, sensory bleed, minor magical residue.
\item[\textbf{2 CP}] Unintended side-effect (e.g., cold off a fire working; echoes draw attention).
\item[\textbf{3 CP}] Residue anchors a hex or attracts supernatural attention.
\item[\textbf{4 CP}] Significant backlash condition or manifestation; ritual mark persists with ongoing effects.
\end{description}

\paragraph{High-Tier CP Sinks} \index{Complication Point (CP)!high-tier}
For major 3–6+ CP spends that affect the campaign world (reputation cascades, faction instability, magical resonance, prophecy triggers), use advanced complications rules. A practical default: \emph{at the end of a journey leg,} \textbf{3 CP → advance 1 Campaign Front}.
...
\item \textbf{Consequence Lands} — The GM spends or banks CP, applies a condition, or advances a thread. \index{Consequences}
\end{enumerate}
...
\begin{itemize}
\item \textbf{Failure Limit:} Max \textbf{2 Boons from failures per character per scene}. Further misses still generate CP but no Boon.
...
\begin{itemize}
\item \textbf{Boon Awarded:} Picking a lock under watch (Risky, DV 3). Stakes: success opens door; miss triggers alarm. Roll misses; GM spends 2 CP to start \emph{Guards Incoming} [6]. Player gains 1 Boon.
\item \textbf{No Boon:} Tapping flagstones “just in case” (Controlled, no stakes). Info-only; no CP spent. No Boon.
