...
\item All \textbf{PCs and NPCs} present must resist (\textbf{Spirit + Presence} vs.\ DV Tier) or gain a minor trait from that Patron’s \indexterm{Corruption Table}, or else experience a direct influence (e.g., whispered compulsions, sudden urges to further the Patron’s will).
\item The DV for this effect \emph{overrides} any other DV calculation (including Rite DVs or situational modifiers).
...
\item All traits and influences fade at the next \indexterm{Downtime}.
\end{itemize}
...
\end{itemize}
When you walk the Cantor’s Path, your corruption does not remain contained. 
Mark 1--2 segments on the scene’s \indexterm{Environmental Corruption} track. 
All characters present must resist (\textbf{Spirit + Presence} vs.~DV Tier) or accept a tract from that Patron’s \indexterm{Corruption Table}. 
\indexterm{Cap 1} and \indexterm{Cap 2} NPCs also gain a tract in the area, manifesting traits of the Patron’s influence in visible and unsettling ways. 

\textbf{Push It:} Add +1 tract to those affected, increase the DV of the resistance roll by +1, and extend the effect to include \indexterm{Cap 3} NPCs.

\textit{Corruption traits gained in this way fade at the next \indexterm{Downtime}.}
