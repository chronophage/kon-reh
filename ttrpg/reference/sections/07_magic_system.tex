...
\begin{description} \item[\indexterm{Natural Fading}] At the beginning of each Downtime, reduce a character's current Corruption by 1 level, but pesky effects continue until fully resolved.

\item[\indexterm{Act of Contrition}] Perform a genuine act that goes against the Patron's influence: \begin{itemize} \item Publicly oppose the Patron's themes \item Help those harmed by similar Corruption \item Destroy symbols of the Patron's influence \item Make amends with those offended by your actions \end{itemize} \textbf{Effect}: Remove 1 Corruption segment + clear one pesky effect

\item[\indexterm{Ritual Purification}] Perform appropriate cleansing rites: \begin{itemize} \item Seek absolution from opposing religious figures \item Undertake pilgrimage or penitent journey \item Perform community service for those harmed \end{itemize} \textbf{Effect}: Remove 2 Corruption segments + clear all pesky effects

\item[\indexterm{Patron Bargain}] Negotiate with the Patron: \begin{itemize} \item Accept a significant quest or service \item Trade Corruption for Obligation (1:2 ratio) \item Offer something of personal value \end{itemize} \textbf{Effect}: Remove 1-3 Corruption segments based on bargain quality

\item[\indexterm{Persistence}] Corruption effects don't clear through simple rest. They require active resolution through roleplay and narrative actions. \end{description}
