...
\chapter{Deck-Based Generators}\label{chap:deck_generators}

\section{Introduction to Deck Generators}\label{sec:deck_intro}
\index{Deck Generators}
...
\section{Standard Deck Structure}\label{sec:deck_structure}
\index{Decks!Structure}

\textbf{Fate's Edge} uses several card‑based tools, each with specialised suit meanings:

\begin{description}
    \item[\textbf{Travel Decks} (regional, 52‑card)] \index{Decks!Travel}
        ...
        \end{itemize}
    \item[\textbf{Deck of Consequences} (scene drama)] \index{Decks!Consequences}
...
\section{Rank Severity and Timer Size}\label{sec:rank_severity}
\index{Decks!Rank Severity}
\index{Timers!Size Determination}

Card rank sets the size / significance of the primary Timer :

\begin{itemize}
    \item \textbf{2--5 (Minor):}  4‑segment Timer \index{Timers!4‑segment}
    \item \textbf{6--10 (Standard):} 6‑segment Timer \index{Timers!6‑segment}
    \item \textbf{J, Q, K (Major):} 8‑segment Timer \index{Timers!8‑segment}
    \item \textbf{Ace (Pivotal):} 10‑segment Timer \index{Timers!10‑segment}
\end{itemize}
...
\section{Draw Procedures}\label{sec:draw_procedures}
\index{Decks!Draw Procedures}

\subsection{Quick Hook (2 cards)}\label{subsec:quick_hook}
...
\begin{enumerate}
    ...
    \item Use the higher rank to set Timer size.
    \item Combine elements into a simple, compelling scenario.
\end{enumerate}

\subsection{Full Seed (4 cards)}\label{subsec:full_seed}
...
\begin{enumerate}
    \item Draw until one card of each suit appears:
        ...
        \end{itemize}
    \item The highest rank sets the main Timer size.
    \item If multiple face cards or Aces appear, create parallel Timers for secondary threats or opportunities.
\end{enumerate}

\subsection{Act Builder}\label{subsec:act_builder}
...
\section{Using the Deck in Play}\label{sec:deck_in_play}
\index{Decks!Usage}

\begin{enumerate}
    \item Players roll; each die showing \textbf{1} generates \textbf{1 Story Beat (SB)}. \index{Story Beats}
    \item The GM chooses one method for that roll:
        \begin{enumerate}
            ...
            \item \textbf{Deck Draw:} Draw up to \textbf{min(SB, 3)} cards and synthesise a single twist guided by suit and highest rank.
        \end{enumerate}
    \item Interpret the cards to create a coherent complication that advances the narrative.
\end{enumerate}

\section{Combo Rules}\label{sec:combo_rules}
...
\begin{description}
    ...
    \item[\textbf{Run} (3+ sequential ranks)] Momentum -- reduce the main Timer by 1 segment. \index{Decks!Runs}
    ...
    \item[\textbf{All one colour}] GM gains \textbf{+1 SB} to use in that scene. \index{Decks!Colour Combos}
\end{description}

\section{Regional Generator Summary}\label{sec:regional_summary}
\index{Decks!Regional Summary}

\begin{longtable}{p{3.3cm}p{5.3cm}p{5.0cm}}
\caption{Regional Generator Summary}\label{tab:regional_summary}\\
\toprule
...
\section{NPC Generation Deck}\label{sec:npc_generation}
...
\subsection{Generation Categories}\label{subsec:npc_categories}
...
\subsection{Using the NPC Generator}\label{subsec:npc_usage}
...
\caption{NPC Generation Categories}\label{tab:npc_categories}\\
\toprule
...
\section{Practical Deck Usage Examples}\label{sec:deck_examples}
\index{Decks!Examples}

\subsection{Example 1: Quick Scene Generation}\label{subsec:example_quick}
...
\subsection{Example 2: Consequences During Play}\label{subsec:example_consequences}
...
\subsection{Example 3: NPC Creation}\label{subsec:example_npc}
...
\section{GM Guidance for Deck Usage}\label{sec:deck_guidance}
\index{Decks!GM Guidance}

\subsection{When to Use Which Deck}\label{subsec:deck_selection}
\begin{itemize}
...
\subsection{Interpreting Card Draws}\label{subsec:card_interpretation}
\begin{itemize}
    \item Prioritise narrative coherence over literalism.
...
\subsection{Balancing Randomness and Narrative}\label{subsec:balancing_randomness}
\begin{itemize}
