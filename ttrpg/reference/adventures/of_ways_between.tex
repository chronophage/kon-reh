...
\item \textbf{Management:} Players must spend Boons to prevent the Dread Timer from advancing. This creates a crucial resource management challenge.
...
\begin{itemize}
\item \textbf{1. Reality Fracture Timer (12 segments)}
\begin{itemize}
...
\item \textbf{2. Collective Sanity Timer (8 segments)}
\begin{itemize}
...
\item \textbf{3. Existential Threat Timer (15 segments)}
\begin{itemize}
...
\item \textbf{Life Drain:} Successful attack inflicts Fatigue 1 in addition to harm, and the target must make a Spirit + Resolve (DV 3) test or gain +1 segment on the Dread Timer (the mist feeds on fear/sorrow). (Spirit + Resolve DV 3 to resist Dread)
...
\item \textbf{Weaknesses:}
\begin{itemize}
\item \textbf{Vulnerable to bright light:} Starts Dominant vs. illuminated targets. Sustained bright light can force it to flee or solidify.
...
\item Highest rank sets the main Timer size (2--5 $\rightarrow$ 4, 6--10 $\rightarrow$ 6, J/Q/K $\rightarrow$ 8, A $\rightarrow$ 10).
\item Diamonds are codified outcomes (knowledge/tools/leverages) that change the situation rather than call for a roll.
\item If any \textbf{Ace} appears, add a fundamental law of reality that becomes unreliable (GM's choice, ties to campaign timers).
...
\begin{itemize}
\item \textbf{Dream-Logic Navigation:} Any Ace not only adds a reality distortion but also a metaphysical requirement (sacrifice, taboo, or transformation) that must be fulfilled to pass. The requirement always seems reasonable until you try to fulfill it. \textit{Integrates with Reality Fracture Timer .}
\item \textbf{Memory Currency:} Diamonds (especially knowledge/revelations) can be traded for information about other travelers or previous paths taken. The information comes with the emotional weight of the original experience. \textit{Integrates with Dread Timer .}
\item \textbf{Time-Drift:} Any 2-4 on a navigation roll indicates the path moves through time as well as space; arrival may be before departure (or in a different timeline). You might meet yourself coming from the opposite direction. \textit{Integrates with Temporal Distortion rules and Collective Sanity Timer .}
...
\end{enumerate}
\item \textbf{Dread Timer Advancement:}
\begin{itemize}
...
\end{itemize}
\item \textbf{Campaign Timer Advancement:}
\begin{itemize}
...
\end{itemize}
\item \textbf{Sanity Management (Dread Timer ):}
\begin{itemize}
...
\item Advances Dread Timer by 2 segments (prevent with 2 Boons).
...
\begin{itemize}
\item \textbf{Blessed Warding Stone:} Provides +2 dice to resist supernatural influence and reduces Dread Timer advancement by 1 (minimum 1). Crumbles to dust if Dread Timer fills completely. The stone whispers warnings in a language that predates human speech.
\item \textbf{Sanctified Salt:} Can be used to create protective circles or barriers. One use per session to prevent Dread Timer advancement or gain start Dominant vs. supernatural threats. The salt burns with cold fire when the entity is near.
\item \textbf{Prayer Book of the Steadfast:} Once per session, allows reroll of failed Spirit + Resolve roll and prevents 1 segment of Dread Timer advancement. The pages turn of their own accord, revealing passages that speak to the character's deepest fears.
\end{itemize}
...
\item \textbf{Mirror of the Forgotten:} Shows glimpses of possible futures and past events. Provides valuable insights (Wits + Lore, DV 3) but each use advances Dread Timer by 1 and generates 1 SB. The mirror shows not reflection, but possibility - and some possibilities are too terrible to contemplate.
...
\begin{itemize}
\item \textbf{Dread Timer Management}
\begin{itemize}
...
\item \textbf{Campaign Timers Quick Reference}
\begin{itemize}
\item \textbf{Reality Fracture Timer (12):} Reality laws unstable. Triggers: Impossible phenomena, research, alien geometry.
\item \textbf{Collective Sanity Timer (8):} Party-wide psychological effects. Triggers: Reality Fracture, alien entities.
\item \textbf{Existential Threat Timer (15):} Cosmic entity manifestation. Triggers: Other timers filling, communication attempts.
\end{itemize}
