...
\subsubsection{Clocks (Visible to Players)}
...
\begin{itemize}
    ...
    \item \textbf{Integration:} When a character touches a fragment or succeeds on a knowledge roll about Khasurah, the GM may offer a \textbf{Bargain}: accept a vision of a future outcome (true, but not inevitable) in exchange for a Story Beat or a tick on the \textit{Fragment's Whispers} clock.
...
See Act I for the four campaign clocks. Summary:
...
When a clock fills, immediate consequences occur. The GM should announce the tick and describe the effect.
...
\begin{itemize}
    ...
    \item[$\square$] Note which clocks are near filling; plan consequences.
...
When tension lags:
\begin{itemize}
    \item Introduce a time pressure (a clock ticks audibly).
...
\begin{itemize}
    ...
    \item \textbf{Clock Tracker:} A printable sheet with the four visible clocks.
...
\begin{itemize}
    ...
    \item \textbf{Clock Triggers Summary:} One page.
...
\begin{itemize}
    ...
    \item[$\checkmark$] Integration with core Fate's Edge mechanics (Boons, SB, Clocks, Position).
    \item[$\checkmark$] Custom mechanics (Varash's Whisper) that serve theme.
    \item[$\checkmark$] Multiple valid approaches to conflicts.
    \item[$\checkmark$] Clear escalation patterns (clocks).
