...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Key Timers:}
    \begin{itemize}
        \item \timerbox{Mor'iraath's Expansion}{8} (Stops his plan)
        \item \timerbox{Cinder Market Stability}{6} (Prevents city collapse)
        \item \timerbox{Livaea's Manipulation}{6} (Reveals her full scheme)
    \end{itemize}
\end{itemize}
...
\begin{itemize}[leftmargin=*]
    ...
    \begin{itemize}
        ...
        \item \textbf{Push It:} The effect lingers subtly on the target's dealings for the rest of the session; mark 1 SB (Clubs) as financial records become\dots difficult.
    \end{itemize}
...
\begin{itemize}[leftmargin=*]
    ...
    \begin{itemize}
        ...
        \item \textbf{Push It:} The effect is permanent (until countered) and the Consumption Timer ticks faster (+1/2 automatically per scene); mark +2 Obligation.
    \end{itemize}
\end{itemize}
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Soulfire:} The primary energy source. It burns bright but consumes the user's essence over time. Prolonged presence in Cinder or use of Cinderite magic/assets should tick a ``Soulfire Exposure'' timer for characters, leading to Fatigue, Conditions, or worse.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
    ...
    \item Timer Tick: Advance a relevant campaign/front timer by 1 segment.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Fills (Negative):} They corner the market -- prices spike, access denied. Start a ``Guild Monopoly Pressure [4]'' timer affecting the party's operations.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Fills (Negative):} A champion goes rogue/mad, causing havoc. Start an ``Arena Riot [6]'' timer threatening a district.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Fills (Positive):} Gain one powerful piece of forbidden lore (like a major ritual secret or a vulnerability of a key antagonist) usable once. Clear 1 segment from any Investigation-based timer.
    \item \textbf{Fills (Negative):} A catastrophic truth is revealed (perhaps about a PC's past or a Patron's true goal), causing immediate Harm 2 (Stress) to one character and starting a ``Revelation Fallout [4]'' timer.
\end{itemize}
