...
\begin{storybox}
\textbf{The Hook:} The Aeler have sent for help, but few have answered. The PCs arrive to find the cathedral's outer doors grinding open and closing erratically, as if breathing. The Aeler elders beg for assistance, explaining that the cathedral's heart is failing and will collapse the mountain if not repaired. They cannot enter themselves---the cathedral rejects all Aeler, recognizing only those who can repair its sacred timerwork.
\end{storybox}
...
\begin{itemize}
    \item \textbf{Calm (0--2 segments):} The cathedral's architecture is stable. Position is typically Dominant or Controlled for repairs. DV for timerwork-related checks is -1.
    \item \textbf{Tense (3--5 segments):} The cathedral shows signs of stress. Corridors shift slowly. Position is typically Controlled. DV for timerwork-related checks is normal.
    \item \textbf{Stressed (6--8 segments):} The cathedral is actively shifting. Corridors change position each scene. Position is typically Controlled or Desperate. DV for timerwork-related checks is +1.
    \item \textbf{Fracturing (9--10 segments):} The cathedral is falling apart. Corridors shift during scenes. Position is typically Desperate. DV for timerwork-related checks is +2.
\end{itemize}

\textbf{How the Mood Changes:}
\begin{itemize}
    \item \textbf{Worsens by 1:} On a miss when repairing timerwork, when the players cause damage to the cathedral, when the players ignore a "living gear" request.
    \item \textbf{Improves by 1:} On a clean success when repairing timerwork, when the players successfully interact with a "living gear," when the players perform a ritual to stabilize the cathedral.
\end{itemize}
...
The entrance hall is a vast, domed chamber. In the center, a gear the size of a wagon wheel spins erratically, grinding against the floor. Stone dust fills the air, and the walls are etched with intricate timerwork patterns that seem to shift when not directly observed.
...
The gear is a \textbf{Living Gear}---a sentient timerwork component that has become disconnected from the cathedral. It communicates through vibrations, clicks, and the shifting of its teeth.
...
\textbf{Critical Success (10+ on a die):} The gear not only calms but also reveals a hidden compartment containing a \textbf{Cathedral Key} (a minor asset that can unlock any timerwork mechanism in the cathedral).
...
In one of the shifting corridors, the players encounter a set of \textbf{Guardian Gears}---smaller, mobile timerwork constructs that serve as the cathedral's protectors. They are not hostile unless the players threaten the cathedral.
...
The inner sanctum contains the cathedral's heart---a massive, intricate timerwork mechanism suspended in the center of a vast chamber. But something is wrong: the heart is turning backward, and black, oily tendrils are spreading across its gears, corrupting them.
...
The Time Devourer is a spectral entity that manifests as a swirling vortex of black oil and broken timerwork. It attacks by manipulating time around the players.
...
\begin{itemize}
    \item \textbf{Description:} The Aeler are a small, elderly people with silver hair and timerwork amulets. They are desperate to save the cathedral but cannot enter it themselves.
    \item \textbf{Role:} They provide the hook and guide the players to the cathedral's entrance.
    \item \textbf{Secret:} They know the cathedral rejected them because they failed to properly maintain its sacred timerwork, but they fear the truth will make the players abandon the mission.
\end{itemize}

\subsection{The Time Devourer}

\begin{itemize}
    \item \textbf{Description:} A swirling vortex of black oil and broken timerwork, with no fixed shape. It emits a low, grinding hum.
...
If the players repair the heart and defeat the Time Devourer, the cathedral stabilizes. The Aeler return to their holy site, and the cathedral's timerwork heart begins to turn forward again. The Aeler are eternally grateful and offer the players:

\begin{itemize}
    \item A \textbf{Cathedral Blessing} (a one-time +2 to any roll)
    \item A \textbf{Cathedral Key} (a minor asset that can unlock any timerwork mechanism in the cathedral)
...
If the players defeat the Time Devourer but fail to fully repair the heart, the cathedral stabilizes but with a lingering corruption. The Aeler can return, but the cathedral's timerwork is now imperfect. The players receive:
...
\begin{itemize}
    ...
    \item \textbf{Specialist (Mechanic):} Repairs the timerwork and interfaces with the heart.
\end{itemize}
...
\begin{itemize}
    ...
    \item Increase the size of the timers by 2 (e.g., Broken Gear [6] instead of [4]).
