...
\textbf{Global Timers.}
\begin{itemize}
  \item \textbf{Stalled Passing} \timer{6} (dead linger; hauntings escalate).
  \item \textbf{Bargain Net Tightens} \timer{6} (oaths spread across civic life).
  \item \textbf{Gate Repair or Replacement} \timer{8} (relics, name, covenant).
\end{itemize}
\textbf{Hard Triggers.}
\begin{itemize}
...
\begin{itemize}
  \item \textbf{Public Panic} \timer{4} (feeds other timers).
  \item \textbf{Hidden Oath Cartel} \timer{6} (guild + magistrates).
  \item \textbf{Gate's Old Name Remembered} \timer{4} (Shepherd-leaning discovery).
\end{itemize}
...
\begin{itemize}
  \item \textbf{Bargain Net Tightens} \timer{6$\to$8} if bargains are used for civic order.
  \item \textbf{Gate Repair or Replacement} \timer{8} ticks with relics, name, and covenant.
\end{itemize}
...
\begin{itemize}
  \item Gate collapses: \textbf{Stalled Passing} \timer{6$\to$10}; duchy becomes a liminal haunt.
...
\begin{itemize}
  \item \textbf{Stalled Passing} \timer{6} (advances on public fear, failed vigils).
  \item \textbf{Bargain Net Tightens} \timer{6}/\timer{8} (advances when bargains maintain order).
  \item \textbf{Gate Repair or Replacement} \timer{8} (ticks with relics, name, covenant).
  \item \textbf{Public Panic} \timer{4} (feeds others).
  \item \textbf{Hidden Oath Cartel} \timer{6} (expose or dismantle).
  \item \textbf{Vigil of Stillness} \timer{4} (hold the line during rites).
\end{itemize}
...
\vfill
\noindent\footnotesize\textit{Running Note:} Map Position to DV plainly (Dominant=DV2, Controlled=DV3, Desperate=DV4--5+). Favor Position shifts over raw dice creep. Name costs before rolls. Keep total active timers $\leq$~5 to avoid overload.
...
\begin{itemize}
  ...
  \item \textbf{Malachai (Curses \& Corruption):} Antagonism---Malachai corrupts, Morag exacts payment.  
...
\begin{tcolorbox}[colback=black!3,colframe=black!40!white,title={Theme \& Atmosphere},breakable]
...
\item \textbf{Library} --- Books rearrange themselves; reading out loud advances timers.
...
\begin{itemize}
\item Draw until you have all four suits: Spade = room, Heart = resident, Club = complication, Diamond = revelation. Highest rank sets the main Timer (2--5 $\rightarrow$ 4, 6--10 $\rightarrow$ 6, J/Q/K $\rightarrow$ 8, A $\rightarrow$ 10).
...
\item \textbf{Location Corruption Timer :} Each Club result adds 1 tick. At full, house collapses into nightmare realm.
\item \textbf{Dread Timer :} Each failed Fear test advances Dread by 1. At max, PC suffers permanent Scar or Corruption.
\end{itemize}

\paragraph{Timer Template.}
\begin{itemize}
  \item \textbf{Dread Timer (10):} PCs" psychological unraveling.
...
\begin{itemize}
  ...
  \item Establish Dread Timer (start at 2/10).
...
\begin{itemize}
  \item Location Corruption Timer begins advancing: doors vanish, mirrors show alternate selves.
