...
\item Advances Dread Timer by 1 segment (prevent with 1 Boon)
\item May reveal useful but disturbing information
\end{itemize}

\textbf{Whisper Examples:}
\begin{itemize}
\item ``The books remember your name, and they are hungry\dots''
\item ``Knowledge has a price, and it is always more than you can pay\dots''
\item ``The Matriarch waits for you in Synod Hall, where all judgments are final\dots''
\item ``Your companion's thoughts are not their own --- count the shadows that follow them\dots''
\item ``The geometry of this place is wrong. You are not where you think you are\dots''
\end{itemize}
...
\begin{itemize}
\item +1 die to Lore rolls involving forbidden knowledge, but each use advances the Dread Timer by 1
...
\textbf{Advancement Triggers:}
\begin{itemize}
\item Dread Timer advances: +1 segment --- madness is contagious
...
\begin{itemize}
\item \textbf{Whispering Tome:} A book that hums with voices inaudible to most. Reader gains +1 die to Lore rolls but must make Spirit + Resolve (DV 3) each session or advance Dread Timer by 1. The voices speak of things that should not be known, and their words carry the weight of cosmic truth.
