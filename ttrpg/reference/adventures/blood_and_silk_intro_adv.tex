...
\begin{enumerate}
\item \textbf{Social Encounter:} Negotiating with Garrick (Presence + Diplomacy or Command, DV 3)
\item \textbf{Combat Encounter:} Fighting Rothari bandits (Body + Melee or Wits + Skullduggery)
\item \textbf{Investigation:} Learning about the village's troubles (Wits + Insight or Lore)
\end{enumerate}
...
\item Rothari successfully intimidates villagers: +1 segment
...
\subsection{Opening Scene: Consequences}

Garrick Rothari doesn't take kindly to being challenged. He returns to Millhaven with more men and makes it clear that the PCs' interference will be paid for in blood.

\textbf{New Threats:}
\begin{itemize}
\item \textbf{Reinforcements:} More Rothari bandits arrive
\item \textbf{Retaliation:} Rothari begins targeting villagers who helped the PCs
\item \textbf{Time Pressure:} Harvest season is approaching - if the village can't deal with the Rothari soon, they'll lose their crops
\end{itemize}
...
\item \textbf{Supply Run:} Getting supplies for the defense (Body + Athletics or Survival)
\item \textbf{Ambush:} Fighting Rothari raiders (Combat encounter)
\end{enumerate}
...
\begin{itemize}
\item Helped organize the village defense
...
\begin{itemize}
\item PCs must flee and seek redemption elsewhere
\item They gain experience from the attempt but must start over
\item The adventure can continue with a new location and challenges
\end{itemize}
...
\textbf{Political Intrigue:}
\begin{itemize}
\item A local noble is secretly supporting the Rothari
\item The village must choose between different political factions
\item PCs discover that their past in Silkstrand connects to the conflict
\end{itemize}
...
\subsection{Village Falls}

If the PCs fail to save Millhaven:
\begin{itemize}
\item \textbf{Consequences:}
   \begin{itemize}
   \item 6-8 XP for each player (experience from the attempt)
   \item Guilt and responsibility for the outcome
   \item Displaced villagers become refugees
   \end{itemize}
\item \textbf{New Challenges:}
   \begin{itemize}
   \item Hunted by Rothari for making them look bad
   \item Villagers blame the PCs for the tragedy
   \item Need to find redemption elsewhere
   \end{itemize}
\item \textbf{Story Continuation:}
   \begin{itemize}
   \item Help refugees find new homes
   \item Hunt down Rothari leadership
   \item Prove themselves worthy of trust again
   \end{itemize}
\end{itemize}
