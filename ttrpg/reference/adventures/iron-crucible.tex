...
\emph{The Iron Crucible} is a three-part military campaign for \emph{Fate's Edge} that puts players in command of a mercenary company. Each part escalates the scale of conflict -- from fortress siege to river war to city-wide annihilation -- while keeping the core mechanics of \emph{Fate's Edge} (Position, Effect, timers, Story Beats) front and center.
...
\item \textbf{Supplies [6]} -- Food, ammunition, and repair materials. Resupply costs silver or favors.
...
The old general retreats to the inner keep. He offers a deal: safe passage for his garrison in exchange for the gate's keys. The party must choose -- honor the deal (gains Reputation, loses loot) or fight (tough battle, gains plunder but loses Morale).
...
\item \textbf{3 SB} -- A traitor among the coalition reveals the party's plan. One approach timer ticks backward by 2.
...
Each weapon is a \Tier\ IV obstacle that requires a specialized skill challenge to seize.
...
\item \textbf{4+ SB} -- The Hollow's attention turns to the city. A Oath-Keeper appears and demands that the party stop using the WMDs -- or it will claim the souls of everyone in the district.
\end{itemize}
...
\item \textbf{Network (Presence + Sway, DV 3)} -- Gain a new contact or a rumor about the next mission.
\end{itemize}

\subsection*{Squad Specialisations}
Once the company has Morale 6+ and has completed two missions, squads can be specialized. Choose one:
...
Medics & Reduce squad casualties by 1 level (e.g., Harm 2 $\rightarrow$ Harm 1)\\
\bottomrule
\end{tabular}
...
\subsection*{Tone}
This campaign is meant to be gritty, desperate, and morally gray. Emphasize the cost of war -- dead allies, ruined cities, and the weight of command. Reward creative tactics and penalise reckless bloodshed.
