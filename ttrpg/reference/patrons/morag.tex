...
The \textbf{Codex} of Morag is never a book written in plain ink. It may be a leather satchel of knotted cords, each knot representing a promise kept or broken; a slate tablet that only reveals its writing when a debt is paid; or a wooden spindle wrapped with thread dyed in three colors (white for truth, black for secret, red for blood). Because Morag's truths are written in the spaces between words, the Codex requires \textit{upkeep}: the Runekeeper must spend an hour each week reviewing all current bargains and knots, tightening those that hold and loosening those that have been fulfilled, or the Rites become uncertain (treat as Compromised until re-knotted).
...
\paragraph{Rite of the Cauldron's Secret (High, 12 XP)} \hfill \\
