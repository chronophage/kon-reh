...
\textbf{Patron's Gift -- The Key to Sustainable Magic}\\[3pt]
\textbf{Cost:} Free (requires Thiasos) \\
\textbf{Activation:} 1 Action, once per scene \\
\textbf{Duration:} Scene \\
\textbf{Range:} Touch \\
\textbf{Effect:} Imbue one item with temporary magical power related to your Patron's domain. 
The item functions as a magical weapon (+1 Melee) and specialized tool (+1 thematic Skill) for the scene. \\
\textbf{Push It:} The item's power persists for one additional scene but marks +1 Obligation. \\[6pt]

\paragraph*{Key Tips for Effective Patron's Gift Usage}
\begin{itemize}
  \item \textbf{Plan Your Gift:} Decide at the start of each scene what enhancement you need most.
  \item \textbf{Weapon + Skill Combo:} Always enhance both combat and a key skill for maximum utility.
  \item \textbf{Thematic Consistency:} Choose enhancements that match your Patron's domain for narrative coherence.
  \item \textbf{Push Strategically:} Use the Push option when you need extended duration, but manage Obligation carefully.
\end{itemize}
...
\begin{tabular}{|l|l|l|}
\hline
\textbf{Patron} & \textbf{Weapon Enhancement} & \textbf{Skill Enhancement} \\
\hline
Oath of Flame & +1 Radiant Melee & +1 Sway (inspiring presence) \\
Silent Choir & +1 Melee (quiet strike) & +1 Investigation (silent observation) \\
Ra\'{e}yn & +1 Melee (tidal strike) & +1 Survival (sea-sense) \\
Sealed Gate & +1 Melee (warded blade) & +1 Craft (secure construction) \\
\hline
\end{tabular}
\end{center}
...
\textbf{Borrowed Grace -- Invoker's Resource Efficiency}\\[3pt]
\textbf{Type:} Invoker Talent -- Lesser Imbuement \\
\textbf{Cost:} 1 Boon, 1 Action \\
\textbf{Effect:} On use, pick one: +1 Melee \emph{or} +1 Thematic (your table's signature/thematic Skill). \\
\textbf{Duration:} Single action/attack (instantaneous boost only). \\
\textbf{Requirement:} Wield or display the relevant Patron's Symbol. \\
\textbf{Obligation:} Immediately mark +1 Obligation to that Patron. \\
\textbf{Limits:} Cannot be extended, stacked, or Pushed for duration. \\[6pt]

\paragraph*{Key Tips for Effective Borrowed Grace Usage}
\begin{itemize}
  \item \textbf{Symbol Management:} Always carry your Patron's Symbol as a Minor Asset (4 XP).
  \item \textbf{Boon Economy:} Use Borrowed Grace when you have excess Boons or need a crucial boost.
  \item \textbf{Timing:} Activate immediately before key rolls to maximize impact.
  \item \textbf{Obligation Tracking:} Plan your Obligation spending to avoid capacity overflow.
\end{itemize}

\paragraph*{Advanced Borrowed Grace Strategies}
\begin{itemize}
  \item \textbf{Combat Setup:} Use before entering major combat to ensure peak performance.
  \item \textbf{Social Boosting:} Enhance Presence-based skills for important negotiations.
  \item \textbf{Skill Synergy:} Combine with Assistance to create powerful collaborative effects.
  \item \textbf{Resource Conversion:} Turn excess Boons into reliable dice bonuses.
\end{itemize}
