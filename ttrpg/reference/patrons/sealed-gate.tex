% ============================================================================
% PATRON ENTRY: THE SEALED GATE (THRESHOLDS & CONTAINMENT)
% ============================================================================
...
A Runekeeper sworn to the Sealed Gate does not bind a living creature --- they bind a \textit{threshold given will}. Their \textbf{Thiasos} is a small object imbued with boundary-sense: a bronze key that fits no lock yet trembles when a boundary is near, a brass bell that rings only when a ward is about to break, or a small iron gate that stands on the Runekeeper's desk and opens only when a lie is spoken. This object requires no food, but it must be anointed with oil and placed on a threshold at dawn each day. If neglected for a week, the Thiasos becomes \textit{Compromised} (it emits a low hum that imposes --1 die on Stealth rolls for its bearer).

The \textbf{Codex} of the Sealed Gate is never a simple book. It may be a leather strap carved with runes that tighten when a ward is violated, a set of iron rings linked in a chain, or the inside of a lock. Because boundaries are physical, the Codex requires upkeep: the Runekeeper must spend an hour each week testing each seal (touching, ringing, or whispering to it), or the Rites stored there become uncertain (treat as Compromised until restored).

\subsubsection*{Patron's Gift (Imbuement)}

Once per scene as an action, a Runekeeper of the Sealed Gate may touch a held item (weapon, shield, or tool) and speak a word of exclusion. For the rest of the scene, the item grants \textbf{+1 die} to \textit{Defend} or \textit{Resist} rolls, and the first time each scene the bearer would be moved against their will (shoved, pulled, or teleported), they may ignore it. After the scene, the user marks \textbf{+1 Obligation} to the Sealed Gate.

\subsubsection*{Rites of the Sealed Gate}

\paragraph{Rite of the Sealed Threshold} (Basic, 4 XP) \hfill \\
\emph{Scene; Touch; No.} \\
\textbf{Tags:} \texttt{[WARD]}, \texttt{[BIND]} \\
\textbf{Materials:} Chalk, iron dust, or sanctified cord. \\
\textbf{Effect:} Mark a doorway or passage as sealed. Unauthorized crossing tests Spirit+Resolve (DV 3) or suffers a Position penalty. Creates a 4-segment \textit{Ward Integrity} timer. \\
\textbf{Push It:} The barrier becomes a visible shimmer, imposing --1 die on attempts to cross it. Cost: Mark +1 Obligation (total +2) or Mark +1 Fatigue. \\
\textbf{Cost:} Mark +1 Obligation. \\
\textit{Requires:} Familiar (\textit{Invoke:} 1 Boon). \\
\textbf{Invoke:} 1 action. \\
\textbf{Duration:} Scene (Ward Integrity timer: 4 segments).

\paragraph{Rite of the Key's Rebuke} (Basic, 5 XP) \hfill \\
\emph{Instant; Near; No.} \\
\textbf{Tags:} \texttt{[FORCE]}, \texttt{[BANISH]} \\
\textbf{Materials:} A gesture of denial or a barrier symbol. \\
\textbf{Effect:} The target must test Body+Resolve (DV 3) or be pushed back from the threshold. On success, create a \textit{Rebuke Echo} token granting \textbf{+1 die} to the next boundary defense. \\
\textbf{Push It:} The rebuke lingers --- the target cannot cross this threshold for the rest of the scene. Cost: Mark +1 Obligation (total +2) or Mark +1 Fatigue. \\
\textbf{Cost:} Mark +1 Obligation. \\
\textit{Requires:} Familiar (\textit{Invoke:} 1 Boon). \\
\textbf{Invoke:} 1 action. \\
\textbf{Duration:} Instant.

\paragraph{Rite of the Circle of Denial} (Advanced, 8 XP) \hfill \\
\emph{Scene; Zone; No.} \\
\textbf{Tags:} \texttt{[WARD]}, \texttt{[AREA]}, \texttt{[BANISH]} \\
\textbf{Materials:} A salt ring, iron filings, or boundary stones. \\
\textbf{Effect:} Create a protective circle. Supernatural beings test against the caster's Spirit+Arcana (DV 4) to cross. Mortals suffer \textbf{--2 dice} to force entry. Creates a 6-segment \textit{Circle Integrity} timer. \\
\textbf{Push It:} The circle becomes a two-way barrier (no one can cross). Cost: Mark +1 Obligation (total +2) or Mark +1 Fatigue. \\
\textbf{Cost:} Mark +1 Obligation. \\
\textit{Requires:} Familiar + Codex (\textit{Invoke:} 1 Boon). \\
\textbf{Invoke:} 1 action. \\
\textbf{Duration:} Scene (Circle Integrity timer: 6 segments).

\paragraph{Rite of the Writ of Passage} (Advanced, 7 XP) \hfill \\
\emph{Scene; Zone; No.} \\
\textbf{Tags:} \texttt{[BOND]}, \texttt{[COMMAND]}, \texttt{[MOVEMENT]} \\
\textbf{Materials:} Signed authorization or spoken permissions. \\
\textbf{Effect:} Designate an authorized passage route. Allies gain \textbf{Position +1} when using the route. Unauthorized intruders suffer \textbf{--1 die} to movement or stealth. Creates an 8-segment \textit{Writ Authority} timer. \\
\textbf{Push It:} Extend authorization to additional allies; mark \textbf{+1 Obligation}. Cost: Mark +1 Obligation (total +2) or Mark +1 Fatigue. \\
\textbf{Cost:} Mark +1 Obligation. \\
\textit{Requires:} Familiar + Codex (\textit{Invoke:} 1 Boon). \\
\textbf{Invoke:} 1 action. \\
\textbf{Duration:} Scene (Writ Authority timer: 8 segments).

\paragraph{Rite of the Banishment Knot} (Master, 13 XP) \hfill \\
\emph{Instant; Near; No.} \\
\textbf{Tags:} \texttt{[BANISH]}, \texttt{[FORCE]}, \texttt{[FEAR]} \\
\textbf{Materials:} A knotted cord sealed with a gate-sigil. \\
\textbf{Effect:} The target supernatural entity tests against the caster's Spirit+Resolve (DV = entity Cap). On success, the entity is forced to immediately depart the scene. A failed attempt creates a 4-segment \textit{Banishment Backlash} timer. \\
\textbf{Push It:} The banishment lasts until the next moon phase. Cost: Mark +1 Obligation (total +3) or Mark +1 Fatigue. \\
\textbf{Cost:} Mark +2 Obligation. \\
\textit{Requires:} Familiar + Codex + Tier III (\textit{Invoke:} 2 Boons). \\
\textbf{Invoke:} 1 action. \\
\textbf{Duration:} Instant.

\paragraph{Rite of the Consecrated Barrier} (Master, 14 XP) \hfill \\
\emph{Extended; Zone; No.} \\
\textbf{Tags:} \texttt{[WARD]}, \texttt{[AREA]}, \texttt{[SANCTUARY]}, \texttt{[FATE]} \\
\textbf{Materials:} Relics from three traditions, boundary markers, a trespasser's acknowledgment. \\
\textbf{Effect:} Consecrate an area against all unauthorized passage. The tests required scale with the intrusion attempt's severity. A 10-segment \textit{Sacred Boundary} timer tracks the barrier's strength. \\
\textbf{Push It:} Make the barrier semi-permanent (lasts one season); start a \textit{Maintenance [6]} timer. Cost: Mark +1 Obligation (total +3) or Mark +1 Fatigue. \\
\textbf{Cost:} Mark +2 Obligation. \\
\textit{Requires:} Familiar + Codex + Tier III (\textit{Invoke:} 2 Boons). \\
\textbf{Invoke:} Extended action. \\
\textbf{Duration:} Session (Sacred Boundary timer: 10 segments).
...
\begin{tcolorbox}[colback=black!5,colframe=blue!70!black,title=\textbf{Threshold Monk (Sealed Gate)},breakable]
  \emph{You define what passes and what remains.}

  \vspace{2mm}
  \noindent\textbf{Prerequisites:} Spirit 3+, Wits 2+, Rite of the Sealed Threshold or Cantor of the Sealed Gate.

  \vspace{2mm}
  \noindent\textbf{Debt-Resistant Frame.} A threshold does not owe --- it defines. Monks of the Threshold Monk tradition gain \textbf{+1 die} to resist any magical debt or obligation that would force them to cross a boundary they do not wish to cross. If they succeed, the debtor finds the way inexplicably barred (a door will not open, a contract will not bind).
  ...
  \textbf{Iron Rebuke} (Basic, 5 XP) & When an enemy enters your Close range, make a free unarmed strike. On a hit, they cannot move closer this exchange. & 1 Fatigue & Body 3 \\
  \textbf{Seal the Threshold} (Basic, 4 XP) & Mark a doorway or passage as sealed. Unauthorized crossing tests Spirit+Resolve (DV 3) or suffers a Position penalty. Create a 4-segment Ward Integrity timer. & 1 Boon & Spirit 3 \\
  \textbf{Key's Rebuke} (Basic, 5 XP) & Force target to test Body+Resolve (DV 3) or be pushed back from a threshold you guard. & 1 Fatigue & Wits 2 \\
  \textbf{Circle of Denial} (Advanced, 8 XP) & Create a protective circle (salt or chalk). Supernatural beings test against your Spirit+Arcana (DV 4) to enter. Mortals suffer --2 dice to force entry. Create a 6-segment Circle Integrity timer. & 1 Boon & Tier II \\
  \textbf{Sanctuary Step} (Advanced, 9 XP) & When you would be moved against your will (shoved, pulled, teleported), you may ignore it. & Once per scene & Spirit 3 \\
  \textbf{Banishment Knot} (Advanced, 10 XP) & Force a supernatural entity to test Spirit+Resolve (DV = entity Cap) or depart the scene immediately. & 1 Fatigue + 1 Boon & Tier III \\
  \end{tblr}

  \vspace{2mm}
  \noindent\textbf{Master Technique (Epic Talent, 12 XP):}
  \textit{Threshold Incarnate.} Once per session, become a living ward for one scene. Allies within Near gain +1 Armor; any creature attempting to pass a threshold you guard must test Spirit+Resolve (DV 5) or be turned back. After the scene, you gain a permanent \textbf{Debt Scar} (a door-shaped mark on your palm) --- but you may ignore one debt or obligation that would force you to abandon your guard (the creditor finds themselves facing a wall where once there was a door).
\end{tcolorbox}