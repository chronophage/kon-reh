...
\textit{``The walls here got a memory like old parchment -- they remember every scream, every spell, every drop of blood spilled on the stones. And sometimes, when the air's just right, they whisper it back to you. The dungeon listens. The question is whether it learns.''}
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Custodians (Forgotten Caretakers)} -- The original stewards, now half‑mad or ghostly. They maintain the dungeon's rhythms (polishing thresholds, oiling traps) and may trade safe passage for a memory or a promise.  
...
Most expedition sites will feature two or three of these factions in uneasy balance. A dungeon with predatory \emph{Awakened Organs} and desperate \emph{Invaders} is a very different crawl than one haunted by melancholic \emph{Custodians} and curious \emph{Wild Symbiotes}.
...
\begin{quote}
\textit{"In the Dungeon, walls breathe, corridors shift, and memory is the only map worth keeping."}
\end{quote}
...
\paragraph{External Clocks} Threat: \textit{Heart Pressure Rises [6]}; Fracture: \textit{Floorplan Loops [4]}; Anchor: \textit{Safe Route Fails [4]}.
