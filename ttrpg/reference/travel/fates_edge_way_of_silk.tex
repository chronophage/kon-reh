...
The road is not a line on a map. It is a cord, and every traveler ties a knot. The Fhara tie knots in water -- they call them wells. The Sidhi tie knots in light -- they call them lighthouses. The Pereshi tie knots in fire -- they call them coals. The Kuvani tie knots in grass -- they call them remount stations. And we, the Tulkani, tie knots in memory -- we call them stories.

I am Sarnai, Knot-Tyer's Hand. My grandmother was Caravan Judge of the Nine Knots. She traveled the Way of Silk from the Brass Coast to the eastern isles, from the Violet Steppe to the Crimson Valley. She promised to write a guide for those who would follow -- a ledger of customs, strings, and tolls. She died before she could finish.
...
\begin{itemize}
  \item The \textit{Fate's Edge SRD or Player/GM Guides} for core rules (attributes, skills, actions, Position/Effect, Timers, Boons, Story Beats).
  \item A \textbf{Caravan/Tribe Sheet} (see Appendix) to track Standing, Feud, Exposure, Oath Ledger, and Notables.
  \item \textbf{Mount/Camel Tags} for desert, steppe, and highland travel.
  \item The \textbf{Season Wheel} to guide the campaign's rhythm.
\end{itemize}
...
Lighthouse courts and freedman's seals. A \textbf{Lighthouse Charter} grants safe harbour. Always honor the \textbf{Bread \& Salt} ritual before negotiation.
...
\textbf{What is the first thing you notice about a stranger?} Their water-skin. Full means not traveled far. Dry means traveled too far. No water-skin means either a well-keeper or a fool.
...
\begin{itemize}
  \item \textbf{Water First}: Offer water before any business $\rightarrow$ +1 Position on Petition or Broker in Fhara courts.
  \item \textbf{Three Cups, One Contract}: Complete the three-cup ritual (welcome, negotiation, contract) before a deal $\rightarrow$ +1 Effect on the final bargain.
  \item \textbf{Date Offering}: Accept a date from a well-keeper $\rightarrow$ gain a temporary String (Well-Spirit's Favor) usable once in that oasis.
  \item \textbf{Faux Pas (SB)}: Refusing a date lets the GM start \textit{Thirst Timer } [4].
\end{itemize}
...
\subsection*{Etiquette Hooks}
\begin{itemize}
  \item \textbf{Bread \& Salt}: Share a simple plate before negotiation $\rightarrow$ +1 Position in Sidhi homes or guild halls.
  \item \textbf{Name the Line}: Give your family or mentor's lineage when introduced $\rightarrow$ Audience: Cool $\rightarrow$ Warm with civic officials.
  \item \textbf{Guest-Right Token}: Present a stamped guest-right from a guild $\rightarrow$ DV --1 on the first Petition in that venue.
...
\subsection*{Etiquette Hooks}
\begin{itemize}
  \item \textbf{Fruit \& Tea}: Offer fruit and a warm cup on arrival $\rightarrow$ +1 Position in household negotiations.
  \item \textbf{Shoes at Threshold}: Remove shoes indoors $\rightarrow$ negate a Position --1 penalty from prior brusqueness.
  ...
  \item \textbf{Faux Pas (SB)}: Dust on the Mat -- barging in tracks Exposure +1 among neighbors.
\end{itemize}
...
\noindent
Sarnai says: \textit{``The Kuvani do not ride horses. They borrow them. A horse's name is its lineage. Its lineage is its story. I told a remount master a story about a wolf who forgot her pack. He gave me a gray mare. The mare knew the story already.''}
...
\textbf{What does no one talk about but everyone knows?} The ghost herd is real -- spectral horses that ride the salt pan on moonless nights. They are the souls of riders who died without honor. They ride forever, searching for a battle that will never come.
...
\subsection*{Etiquette Hooks}
\begin{itemize}
  \item \textbf{Guest Tent Rope}: Leave a rope coiled outside to signal neutrality $\rightarrow$ +1 Position on first Parley in camp.
...
\textbf{What is the first thing you notice about a stranger?} Their cord. Every Tulkani carries a knotted cord -- their life's ledger. Many knots mean traveled far. Few knots mean just begun. No cord means a ghost.
...
\subsection*{Etiquette Hooks}
\begin{itemize}
  \item \textbf{Story as Kinship}: Tell a true story (in character) to a Tulkani elder $\rightarrow$ gain a temporary String (Memory-Bead).
  \item \textbf{Circle Courtesy}: Do not step over the cord that marks the wagon circle $\rightarrow$ first social roll in the circle gains +1 Position.
  \item \textbf{Ninth Night Offering}: Leave a knotted cord at the central post $\rightarrow$ the Hollow's attention is delayed by one night.
...
\subsection*{Venue Tags}
\begin{itemize}
  \item \textbf{Wagon Circle}: neutral ground; no violence permitted; breaking the circle starts \textit{Exile Timer } [4].
...
\hline
\textbf{Oath Ledger} & favors owed/held (name + what; acts like Favor/Leverage) \\
\hline
...
\end{itemize}
At each transition: advance Politics (Mandate/Crisis) for regional powers; roll Weather/Heat [4--6] for the current theater; offer Oath Opportunities (escort, feud settlement, shrine warding).

\section*{Score Types}
Pick approach (Deceit ∙ Speed ∙ Shock ∙ Parley) $\rightarrow$ set Position/DV from venue/tags $\rightarrow$ roll.
...
On 1s: GM spends SB $\rightarrow$ sandstorm warning, hidden quicksand, rival caravan ambush, guide betrayal.  
...
11 star-temple ∙ 12 beacon tower ∙ 13 cliff monastery ∙ 14 salt works ∙ 15 oasis fleet ∙ 16 lord's pavilion ∙ 21 dune storehouse ∙ \dots (full table in \textit{Sands of Moon and Brass})
