...
\noindent\textbf{Trait}: \emph{Alarm} – On a Partial or Miss, advance the \emph{Reinforcements} clock by 1. The whistle they carry has a note that carries three streets.
...
\noindent\textbf{Traits}: \emph{Unwoven Mercy} – When the Daughter would take Harm, she may instead mark a segment on a \emph{Debt Clock} owned by the target (if any). If no debt exists, she may offer a bargain: “I will spare you, but you owe me a name.” The name is never yours to give.
...
\noindent\textbf{Traits}: \emph{Keystone Clause} – If the party is in an Aeler vault, the Warden may seal a door or trigger a collapse by spending 2 SB (advance the \emph{Cave‑In} clock by 2). The clause is always written in the margin; you never see it until the stone groans.
...
\noindent\textbf{Traits}: \emph{Payday} – At the start of each scene involving the Captain, the GM may tick the \emph{Arrears} clock. When it fills, the Captain’s company mutinies (treat as 1d6 hostile mercenaries). The mutiny is always over bread.
...
\begin{enumerate}[label=\arabic*.]
    ...
    \item Bribed official – may delay a legal clock once.
...
\epigraph{“Debt is a story we tell ourselves to make the world feel fair. The world is not fair.”}{—Dusana of the Raven Road, from a letter I found pressed in a ledger}
...
\noindent\textbf{Traits}: \emph{Legal Warfare} – The Auditor may spend 2 SB to impose a \emph{Seizure Clock} on a character’s Asset. When the clock fills, the Asset is confiscated. The notice is always polite; the bailiffs are not.

\noindent\textbf{Traits}: \emph{Immunity} – The Auditor cannot be directly attacked without triggering an immediate 4‑segment \emph{Arrest} clock. The law is a wall; the Auditor is the gate.
...
\noindent\textbf{Traits}: \emph{Grain Ledger} – The Agent may demand a tithe of grain (or its value in coin). Refusal advances the \emph{Embargo} clock. The tithe is never the same twice; it is calibrated to the weight of your silence.
