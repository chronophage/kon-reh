...
{ŁARGE\scshape\color{darkblue} The Way of the Warrior}\\[0.8cm]

{\normalsizeℹtshape A Fate's Edge Expansion for Combat, Courage, and the Edge of the Blade}\\[1.0cm]
...
\vspace{0.5cm}
ℎfill −−− \textit{Master Tarian Ironhand, Silkstrand, Year of the Broken Crown}
...
\begin{itemize}
    ℹtem \textbf{Body} −− strength, endurance, the ability to deal and receive harm.
    ℹtem \textbf{Wits} −− timing, perception, the ability to read an opponent's intent.
    ℹtem \textbf{Spirit} −− courage, resolve, the will to stand when standing is foolish.
    ℹtem \textbf{Presence} −− command, intimidation, the ability to shape a battlefield with a word.
\end{itemize}
...
\begin{talentbox}{Disciplined Body (3 XP)}
\textit{Requires: Melee 1+, Body 2+.} \\
\textbf{Passive + Active.} +1 die to unarmed combat attacks. Once per scene, convert 1 Harm to Fatigue, and once per scene improve Position by one step. (See \ref{talent:disciplined−body})
\end{talentbox}

\begin{talentbox}{Weapon Mastery (5 XP)}
\textit{Requires: Melee 2+.} \\
\textbf{Passive.} Choose a weapon type (swords, axes, spears, etc.); +2 dice when using it. Once per scene, +1 Effect with your signature weapon. (See \ref{talent:weapon−mastery})
\end{talentbox}

\begin{talentbox}{Second Wind (2 XP)}
\textbf{Active, once per scene.} Clear 1 Fatigue when you take a moment to catch your breath. (See \ref{talent:second−wind})
\end{talentbox}

\subsection{Advanced Talents (6−−8 XP)}

\begin{talentbox}{Weapon Mastery II −−− Flowing Strike (6 XP)}
\textit{Requires: Weapon Mastery, Melee 3+.} \\
When you successfully hit a target with your chosen weapon, you may immediately make a free repositioning move (up to Near distance) without provoking opportunity attacks. You may use this ability once per scene.
\end{talentbox}

\begin{talentbox}{Iron Resilience (6 XP)}
\textit{Requires: Disciplined Body, Body 3+.} \\
You have two additional Harm boxes (for a total of 5). Additionally, the first time you would be reduced to 0 Harm (Incapacitated) in a scene, you may instead remain standing with 1 Harm and mark 2 Fatigue.
\end{talentbox}
...
\begin{itemize}
    ℹtem \textit{Disciplined Body} (2 XP)
    ℹtem \textit{Shield Bearer} (4 XP): When wielding a shield, you gain +1 Armor and may use the \textit{Protect} action to interpose yourself between an ally and an attack, taking the harm yourself.
    ℹtem \textit{Disciplined Body II −−− Iron Resilience} (6 XP)
\end{itemize}
...
ℎfill −−− \textit{Rajah Anant, Dhaharan mahout}
...
\subsection{Core Talents}
\begin{itemize}
    ℹtem \textit{Weapon Mastery} (4 XP)
    ℹtem \textit{Flurry} (4 XP): Once per scene, you may make two attacks against the same target as a single action. Resolve each attack separately. After using Flurry, you suffer −−1 die on your next action.
    ℹtem \textit{Weapon Mastery II −−− Flowing Strike} (6 XP)
\end{itemize}
...
ℎfill −−− \textit{Mkara, Sekogo hunter}
...
\subsection{Core Talents}
\begin{itemize}
    ℹtem \textit{Weapon Mastery (bows or thrown weapons)} (4 XP)
    ℹtem \textit{Keen Senses} (2 XP): +1 die to Perception rolls.
    ℹtem \textit{Heightened Senses} (3 XP): You cannot be surprised in natural terrain.
    ℹtem \textit{Called Shot} (6 XP): \textit{Active, once per scene.} When you attack an \textbf{Unaware} target or a target you have \textbf{aimed at for a full round}, you may declare a Called Shot. On a hit, deal +1 Harm, ignore 1 Armor, and choose one: Blind (–2 dice to sight), Disarm (drop held item), Hamstring (half speed), or Vitals (+2 Harm instead of +1). On a miss, you are Exposed (Desperate Position and mark 1 Harm).
...
When you make a ranged attack from a hidden position, treat your Position as one step higher (Desperate $→$ Controlled, Controlled $→$ Dominant). Additionally, you may ignore one level of range penalty (Near $→$ Close, Far $→$ Near).
\end{talentbox}
...
ℎfill −−− \textit{Seneny Khenemet, Ashaan chariot captain}
...
\begin{itemize}
    ℹtem \textit{Weapon Mastery (chariot archery)} (4 XP): When firing from a chariot, you ignore the first range penalty.
    ℹtem \textit{Chariot Driver} (4 XP): You may drive a chariot and fire a bow in the same action (one roll for both).
    ℹtem \textit{The Unblinking Eye} (6 XP): Ashaani archers are trained to hold their draw under fire. You may delay your shot for up to one full exchange without losing accuracy; if you release after a delay, your attack gains +1 die.
\end{itemize}
...
ℎfill −−− \textit{Pharaoh's Guard inscription}
...
\begin{itemize}
    ℹtem \textit{Weapon Mastery (lance or saber)} (4 XP)
    ℹtem \textit{Mounted Charger} (4 XP): When you attack from horseback after moving at least Near distance, your attack gains +2 dice.
    ℹtem \textit{Chainmail Shirt} (2 XP): You wear a light chainmail shirt; +1 Armor, but −−1 die to Stealth.
\end{itemize}
...
ℎfill −−− \textit{Farari Souleymane, Oshiira war captain}
...
\begin{itemize}
    ℹtem \textit{Weapon Mastery (throwing knives)} (4 XP)
    ℹtem \textit{Keen Senses} (2 XP)
    ℹtem \textit{Poisoner} (4 XP): You can craft and apply poisons to your throwing knives. Crafting requires a Wits + Craft test (DV 3) and a downtime scene.
\end{itemize}
...
ℎfill −−− \textit{Avongara clan proverb}
...
\begin{itemize}
    ℹtem \textit{Weapon Mastery (lance or mace)} (4 XP)
    ℹtem \textit{Heavy Armor Training} (4 XP): You ignore the first −−1 die penalty for wearing heavy armor.
    ℹtem \textit{Cataphract Charge} (6 XP): When you charge with a kontos lance, your attack ignores 1 point of Armor and, on a hit, the target must test Resolve (DV 4) or be Shaken (−−1 die to all actions for one exchange).
\end{itemize}
...
ℎfill −−− \textit{Shahanshah inscription}
...
\begin{itemize}
    ℹtem \textit{Weapon Mastery (javelin or short sword)} (4 XP)
    ℹtem \textit{Light Cavalry} (4 XP): You may ride without saddle or bridle; your horse responds to knee pressure and spoken commands. When you make a Ride test, you gain +1 die.
    ℹtem \textit{Javelin Volley} (6 XP): You carry four javelins. As an action, you may throw two of them at the same target. Resolve each attack separately. After the volley, you are unarmed until you retrieve more javelins.
\end{itemize}
...
ℎfill −−− \textit{Sidhi cavalry captain}
...
\begin{itemize}
    ℹtem \textit{Weapon Mastery (composite bow or javelin)} (4 XP)
    ℹtem \textit{Mahout's Bond} (6 XP): You have bonded with a specific war elephant. While riding it, your melee reach extends to Near, and you may treat the elephant as having +1 Armor.
    ℹtem \textit{Elephant Charge} (8 XP): Your elephant may make a trample attack against all enemies in a Near zone. Each target must test Body + Athletics (DV 4) or take Harm 2 and be knocked prone. Using this ability costs 2 Fatigue for the elephant.
\end{itemize}
...
\begin{itemize}
    ℹtem \textit{Weapon Mastery (lance or saber)} (4 XP)
    ℹtem \textit{Desert Rider} (4 XP): You ignore the first environmental penalty from heat or sand. Your horse also ignores the first penalty.
    ℹtem \textit{Furusiyya Training} (6 XP): You have trained in the full art of the horse warrior: lance, saber, bow, and horsemanship. You gain +1 die to all Ride tests and may re‑roll one failed attack with a lance or saber per scene.
\end{itemize}
...
ℎfill −−− \textit{Fhara poet}

\clearpage
\chapter{Runic Warrior}
\epigraph{``Carve your vows into your flesh. Let your scars be your scripture.''}{\textit{−−− Master Tarian}}

A \textbf{Runic Warrior} is a Runekeeper who writes their covenant in flesh, stone, or steel rather than ink and paper. Their Codex is not a book−−−it is a scarred hand, an engraved gorget, a sentient blade that remembers every oath they swore.

This is not a new class. You still track Obligation, invoke Rites, and serve a Patron. But your tools are different, your costs are visible, and your covenant is written on your body.

\section{The Runic Codex}

Choose one form:

\subsection*{Flesh Codex (Cannot be stolen)}
Rites are tattooed, branded, or scarred onto your skin.
\begin{itemize}
    ℹtem \textbf{Advantage:} Cannot be confiscated. Always accessible. No action to retrieve.
    ℹtem \textbf{Risk:} Visible to everyone. Damage to your skin may damage the Codex. Some cultures fear flesh−writing.
\end{itemize}

\subsection*{Stone Codex (Durable, can be lost)}
Rites are carved, etched, or engraved onto a physical object−−−a gorget, a shield, a staff, a gauntlet.
\begin{itemize}
    ℹtem \textbf{Advantage:} Can serve as armour or weapon. Durable. May grant social advantage.
    ℹtem \textbf{Risk:} Can be stolen, confiscated, or broken.
\end{itemize}

\subsection*{Steel Codex (Sentient, rare)}
Rites are engraved on a sentient weapon or object that speaks and remembers.
\begin{itemize}
    ℹtem \textbf{Advantage:} Functions as both Codex and Thiasos. Counts as a witness for all Rites. Can offer counsel.
    ℹtem \textbf{Risk:} Has its own personality and opinions. Cannot be easily replaced. Attracts attention.
\end{itemize}

Your Codex sees every Rite you invoke. It counts as a \textbf{witness} for any Rite that requires one, improving Position or reducing DV.

\section{Obligation as Physical Scar}

When you mark Obligation, the cost is \textbf{visible}:
\begin{itemize}
    ℹtem \textbf{Minor (1−2 segments):} A small scar, a bruise that doesn't fade, an aching tooth.
    ℹtem \textbf{Moderate (3−4 segments):} A visible scar on the face or hands, a limp, a smouldering brand.
    ℹtem \textbf{Significant (5−6 segments):} A missing finger, a clouded eye, bark‑like skin.
    ℹtem \textbf{Critical (7+ segments):} A numb limb, a face that reflections avoid, a forgotten name.
\end{itemize}

A scar is not a punishment. It is a receipt. Touch a scar to recall the Rite that caused it; an NPC may recognise it as the mark of a specific Patron.

\section{The Thiasos as Ally}

Your Thiasos may be:

\subsection*{Living Thiasos (Creature)}
A warhorse, spirit−hound, wolf−ancestor, fungal familiar, or hollowed bird.
\begin{itemize}
    ℹtem \textbf{Cap:} 2−−5
    ℹtem \textbf{Harm:} Cap + 2
    ℹtem \textbf{Independent action:} Once per scene, on your turn
    ℹtem \textbf{Witness:} Always counts for Rites
    ℹtem \textbf{Shared Burden (talent):} Transfer Fatigue to your Thiasos (it becomes Winded)
\end{itemize}

\subsection*{Sentient Thiasos (Object) −−− Optional}
A weapon, shield, or tool that speaks and remembers. Cannot act independently.
\begin{itemize}
    ℹtem \textbf{Witness:} Always counts
    ℹtem \textbf{Communication:} Speaks aloud
    ℹtem \textbf{Codex:} Can serve as both Thiasos and Codex
    ℹtem \textbf{Cannot accept Shared Burden}
\end{itemize}

\section{Core Talents}

\subsection*{Foundation Talents (2−4 XP)}
\begin{itemize}
    ℹtem \textbf{Flesh Codex (2 XP)} −−− \textit{Body 3+}. Rites tattooed on body; cannot be stolen.
    ℹtem \textbf{Living Thiasos (3 XP)} −−− \textit{A living Thiasos}. Your Thiasos gains +1 Cap and may act independently once per scene.
    ℹtem \textbf{Scarred Resilience (3 XP)} −−− \textit{Body 2+}. Once per scene, ignore 1 Fatigue from physical exertion.
    ℹtem \textbf{Oath−Bound (4 XP)} −−− \textit{A sworn oath}. Once per session, reroll a failed Body roll when serving a sworn oath.
    ℹtem \textbf{Talking Weapon (2 XP)} −−− \textit{Sentient Thiasos}. Your Thiasos speaks aloud and counts as a witness.
\end{itemize}

\subsection*{Advanced Talents (6−8 XP)}
\begin{itemize}
    ℹtem \textbf{Engraved Defence (6 XP)} −−− \textit{Physical Codex}. Your Codex grants +1 Armour. You may mark 1 Obligation to reduce incoming Harm by 1.
    ℹtem \textbf{Shared Burden (6 XP)} −−− \textit{Living Thiasos}. Transfer 1 Fatigue to your Thiasos; it becomes Winded (−1 die) for the scene.
    ℹtem \textbf{Witnessed Scar (8 XP)} −−− \textit{Three permanent Obligation scars}. Once per session, touch a scar and gain +1 die on any roll tied to that event.
    ℹtem \textbf{Iron Resolve (7 XP)} −−− \textit{Body 3+, resilience talent}. When reduced to 0 Harm, remain standing with 1 Harm and mark 2 Fatigue.
\end{itemize}

\subsection*{Prestige Talents (10−12 XP)}
\begin{itemize}
    ℹtem \textbf{The Unbroken Vow (10 XP)} −−− \textit{Kept a sworn oath for one year}. When you swear an oath before a witness, gain +2 dice to all actions serving it.
    ℹtem \textbf{Avatar of the Codex (12 XP)} −−− \textit{Tier III+, physical Codex}. Once per session, become the embodiment of your Codex: Body +2, your Thiasos fights as a second combatant. Mark 2 Fatigue and 1 Obligation after.
    ℹtem \textbf{The Scar That Remembers (11 XP)} −−− \textit{Witnessed Scar, Tier III+}. Once per session, touch a scar and speak a sentence about the event that caused it. That sentence is considered witnessed truth in any court.
\end{itemize}

\section{Sample Rites}

\subsection*{Low Rites (4−5 XP)}
\begin{itemize}
    ℹtem \textbf{Rite of the Kindled Blade (Oath of Flame & Light) −−− 4 XP}. Your weapon ignites with radiant light. For one scene, melee attacks deal +1 Harm against undead and oath−breakers.
    ℹtem \textbf{Rite of the Blood Offering (Grimmir) −−− 4 XP}. Gain +1 die to Survival and Stealth in natural terrain for one scene.
    ℹtem \textbf{Rite of the Unlit Candle (Ikasha) −−− 4 XP}. Gain +1 die to a single Stealth or Subterfuge roll; on success, declare one minor detail ``forgotten'' by any observer.
    ℹtem \textbf{Rite of the Shared Bowl (Oath of Mercy & Grace) −−− 4 XP}. You and a touched ally each clear 1 Fatigue.
\end{itemize}

\subsection*{Standard Rites (7−9 XP)}
\begin{itemize}
    ℹtem \textbf{Rite of the Unbroken Vow (Oath of Flame & Light) −−− 7 XP}. Swear an oath before a flame and a witness; gain +1 Armour. Allies who hear the oath gain +1 die to actions serving it.
    ℹtem \textbf{Rite of the Sealed Threshold (The Sealed Gate) −−− 7 XP}. Mark a doorway as sealed. Unauthorised crossing tests Resolve (DV 3) or suffers a Position penalty.
    ℹtem \textbf{Rite of the Spinner's Debt (Inaea) −−− 9 XP}. Offer a willing creature a boon (clear 1 Fatigue or +1 die). They accept a Promise Timer [4] named ``Debt to the Web''.
\end{itemize}

\subsection*{High Rites (12−14 XP)}
\begin{itemize}
    ℹtem \textbf{Rite of the Dawnfire Covenant (Oath of Flame & Light) −−− 12 XP}. Consecrate a zone; allies gain +1 die to resist fear. Undead or oath−breaker enemies suffer −1 die.
    ℹtem \textbf{Rite of the Umbral Dream (Ikasha) −−− 14 XP}. Become intangible to mundane harm for one scene, or bypass a major obstacle as if a secret way had always been there.
    ℹtem \textbf{Rite of the Eternal Cycle (Carrion−King) −−− 14 XP}. Destroy one major asset, enemy, or obstacle, and create something new of equal value. Player and GM collaborate on the transformation.
\end{itemize}

\section{Runic Warrior Builds by Tradition}
...
⫕section{Core Talents (Existing)}
\begin{itemize}
    ℹtem \textit{Disciplined Body} (2 XP)
    ℹtem \textit{Shield Bearer} (4 XP)
    ℹtem \textit{Rites of Mykkiel or Inquisitor Prime}
\end{itemize}

⫕section{Signature Talent Chain −−− Oath of the Khopesh}
...
⫕section{Core Talents (Existing)}
\begin{itemize}
    ℹtem \textit{Weapon Mastery (lance)} (4 XP)
    ℹtem \textit{Mounted Charger} (4 XP)
    ℹtem \textit{Rites of the Traveler}
\end{itemize}

⫕section{Signature Talent Chain −−− Way of the Horse}
...
\begin{itemize}
    ℹtem \textit{Weapon Mastery (throwing knives)} (4 XP)
    ℹtem \textit{Poisoner} (4 XP)
    ℹtem \textit{Rites of the Carrion−King or Isoka}
\end{itemize}

⫕section{Signature Talent Chain −−− The Shed Skin}
...
⫕section{Core Talents (Existing)}
\begin{itemize}
    ℹtem \textit{Heavy Armor Training} (4 XP)
    ℹtem \textit{Cataphract Charge} (6 XP)
    ℹtem \textit{Rites of Mykkiel or The Witness}
\end{itemize}

⫕section{Signature Talent Chain −−− The Kontos}
...
⫕section{Core Talents (Existing)}
\begin{itemize}
    ℹtem \textit{Light Cavalry} (4 XP)
    ℹtem \textit{Harassing Skirmish} (8 XP)
    ℹtem \textit{Rites of Aveh or Ikasha}
\end{itemize}

⫕section{Signature Talent Chain −−− The Feigned Flight}
...
⫕section{Core Talents (Existing)}
\begin{itemize}
    ℹtem \textit{Mahout's Bond} (6 XP)
    ℹtem \textit{Elephant Charge} (8 XP)
    ℹtem \textit{Rites of Grimmir or Raéyn}
\end{itemize}

⫕section{Signature Talent Chain −−− The Howdah}
...
⫕section{Core Talents (Existing)}
\begin{itemize}
    ℹtem \textit{Furusiyya Training} (6 XP)
    ℹtem \textit{Lance Recover} (6 XP)
    ℹtem \textit{Rites of Aveh or the Traveler}
\end{itemize}

⫕section{Signature Talent Chain −−− The Falcon's Return}
...
\section{Sample Build −−− Tier I Dawn Warrior (36XP)}

\textbf{Concept:} A crusader who carries vows in steel and fire.
...
\begin{itemize}
    ℹtem Body 3 (9 XP)
    ℹtem Spirit 2 (6 XP)
    ℹtem Presence 2 (6 XP)
    ℹtem Wits 1 (3 XP) ℎfill \textit{Total: 24 XP}
\end{itemize}
...
\begin{itemize}
    ℹtem Melee 2 (4 XP)
    ℹtem Command 1 (2 XP)
    ℹtem Athletics 1 (2 XP) ℎfill \textit{Total: 8 XP}
\end{itemize}

\noindent
\textbf{Talents:}
\begin{itemize}
    ℹtem Flesh Codex (2 XP)
    ℹtem Oath−Bound (4 XP)
    ℹtem Scarred Resilience (3 XP) ℎfill \textit{Total: 9 XP}
\end{itemize}

\noindent
\textbf{Rites:}
\begin{itemize}
    ℹtem Rite of the Kindled Blade (4 XP)
\end{itemize}

\noindent
\textbf{Gear:} Engraved gauntlet (Codex), longsword, chainmail shirt.

\noindent
\textbf{Obligation Capacity:} Spirit 2 + Presence 2 = 4.
...
Monks do not wear armour. They do not carry shields. They walk the world with nothing but their bodies and their breath−−−and they are among the deadliest fighters alive. This chapter summarises the \textit{Complete Guide to Monks}, giving you the core tools to build an Unstruck Bell monk in minutes.

\section{The Unstruck Bell Philosophy}

A monk’s greatest strength is not the clenched fist but the open hand. Power lies in the pause before the strike. The world runs on breath, balance, and the careful stillness between action and consequence.

\begin{itemize}
    ℹtem \textbf{The Coin That Does Not Move:} Never resist what cannot be stopped. Yield, and the enemy’s own force becomes their downfall.
    ℹtem \textbf{The Witness Who Breathes:} Stillness is the greatest disguise. Be unremarkable.
    ℹtem \textbf{The Debt That Is Never Incurred:} When you do not strike, you create no debt. Victory is a chain. The open hand offers nothing, and so nothing can be taken from it.
\end{itemize}

\section{Meditation Mini−Game}

To find the still point, you need stillness, time, and intent. The four breaths:

\begin{enumerate}
    ℹtem \textbf{The Entering Breath:} Inhale the world’s noise, threat, opportunity.
    ℹtem \textbf{The Holding Breath:} The pause where decisions are made.
    ℹtem \textbf{The Releasing Breath:} The exhale that becomes action.
    ℹtem \textbf{The Empty Breath:} The space after the exhale. Act from here to become invisible to predictions.
\end{enumerate}

\subsection*{Meditation Procedure}
\begin{enumerate}
    ℹtem \textbf{Settle the Body (Wits + Medicine, DV 2):} On a hit, gain +1 die to the Meditation roll.
    ℹtem \textbf{Settle the Breath (Spirit + Meditation skill, DV 3):} On a hit, enter a light meditative state.
    ℹtem \textbf{The Still Point (Spirit + Resolve, DV 3 for clarity, DV 4 for healing, DV 5 for transcendence):} Achieve your intent.
\end{enumerate}

\subsection*{Meditation Benefits}
\begin{itemize}
    ℹtem \textbf{Healing:} Clear 1 Fatigue and remove one minor Condition (Fear, Shaken, Guilty) with a test.
    ℹtem \textbf{Clarity:} Gain +1 die to your next Wits−based roll.
    ℹtem \textbf{Transcendent:} Once per session, reroll any single failed roll; the GM gains 1 Story Beat.
\end{itemize}

\section{Core Monk Talents}
...
Once per scene, you may make three unarmed attacks against a single target as one action. Roll each separately. After using Flurry of Blows, you suffer –1 to defense DV for one exchange.
\end{talentbox}

\begin{talentbox}{Still Point (2 XP)}
When you do not move during your turn, gain +1 die to your next defence roll. This benefit lasts until you move or take an aggressive action.
\end{talentbox}

\begin{talentbox}{Monk’s Breath (2 XP)}
Once per session, you may clear 1 Fatigue by meditating for one minute uninterrupted. No roll required.
\end{talentbox}

\section{Monastic Traditions}

Choose a tradition that defines your martial and philosophical lineage.

\subsection{Way of the Unstruck Bell (Ikasha – Shadow)}

You read the gaps in perception, become forgettable, and move through shadows.

⫕section*{Basic Technique – Shadow Meld (6 XP)}
\textit{Prerequisite: Stealth 2+.} Once per session, step into any shadow larger than yourself and emerge from another shadow within Near range. Mark 1 Fatigue.

⫕section*{Advanced Technique – The Unobserved (8 XP)}
\textit{Prerequisite: Tier II, Stealth 3+.} When you are Hidden, social Story Beats cannot frame you for a crime you did not commit. Witnesses suffer –2 dice to recall your face.

\subsection{Way of the Iron Vow (Mykkiel / Oath of Flame – Law)}

Your body is a temple; your fist is a gavel. Swear binding oaths that grant power in exchange for restriction.

⫕section*{Basic Technique – Sworn Defender (6 XP)}
\textit{Prerequisite: Body 3+.} When you take the Defend action, an adjacent ally also gains +1 Armour until your next turn.

⫕section*{Advanced Technique – Unyielding Vow (8 XP)}
\textit{Prerequisite: Tier II, Spirit 3+.} Swear a vow before a witness. While actively working toward it, ignore the first point of Fatigue each scene from exhaustion, fear, or despair.

\subsection{Way of the Endless Cup (Thrysos – Drunken Master)}

Stagger and stumble to become impossible to anticipate. Every spill is a trip, every belch a battle cry.

⫕section*{Basic Technique – Stagger Step (4 XP)}
\textit{Prerequisite: Body 3+.} When an enemy misses you with a melee attack, move one range band without provoking opportunity attacks. Mark 1 Fatigue (once per round).

⫕section*{Advanced Technique – Drunken Fist (6 XP)}
\textit{Prerequisite: Body 3+, Spirit 2+.} Your unarmed strikes gain +1 die and ignore the first point of Armour when you are at Fatigue 2 or more.

\section{Cultural Variants}

\subsection{Ashaan Stick Dancer (Tahtib)}

\begin{talentbox}{Tahtib – Dance of Sticks (6 XP)}
When you wield a long stick or staff, you gain +1 die to Parry and may make a free trip attempt after a successful parry. Enemies who watch you fight suffer –1 die to attack you.
\end{talentbox}

\subsection{Dhaharan Kalaripayattu}

\begin{talentbox}{Kalaripayattu – Body as Serpent (6 XP)}
Escape grapples as a free action (once per round). On a critical hit with an unarmed strike, impose a Condition (Blinded, Muted, Lamed) for one exchange.
\end{talentbox}

\subsection{Sidhi Knife Dancer}

\begin{talentbox}{Knife Dance – Twin Blades (6 XP)}
When you wield two khanjar knives, make two attacks as a single action against two different targets within Close range. Once per scene.
\end{talentbox}
...
\section{Quick Build: Unstruck Bell Monk (Tier I, 36XP)}

\textbf{Concept:} A silent shadow who strikes from darkness and vanishes.
...
\begin{itemize}
    ...
    ℹtem Wits 3 (9 XP)
    ℹtem Spirit 2 (6 XP)
    ℹtem Presence 1 (3 XP) ℎfill \textit{Total: 27 XP}
\end{itemize}
...
\begin{itemize}
    ℹtem Stealth 2 (4 XP)
    ℹtem Melee 1 (2 XP)
    ℹtem Meditation 1 (2 XP) ℎfill \textit{Total: 8 XP}
\end{itemize}

\noindent
\textbf{Talents:}
\begin{itemize}
    ℹtem Unarmed Combatant (4 XP)
    ℹtem Disciplined Body (2 XP)
    ℹtem Still Point (2 XP)
    ℹtem Monk’s Breath (2 XP)
\end{itemize}

\noindent
\textbf{Tradition:} Way of the Unstruck Bell (add Shadow Meld later).

\noindent
\textbf{Flavour:} Your knuckles are calloused, your eyes are calm, and your shadow sometimes moves a heartbeat after you do.

\noindent
\textbf{The Breath Scar:} Every monk who walks the Unstruck path accepts a permanent Breath Scar−−−you can never again be surprised, but neither can you ever truly rest.
...
\subsection{Core Talents}
\begin{itemize}
    ℹtem \textit{Stealth} (existing)
    ℹtem \textit{Weapon Mastery (hidden blade or dagger)} (4 XP)
    ℹtem \textit{Poisoner} (4 XP)
\end{itemize}
...
When you attack from a hidden position or from a crowd, your target suffers −−1 to defense DV (they do not see the threat until it is too late). Additionally, if you attack with a poisoned blade, the poison's DV is reduced by 1 (to a minimum of 1).
\end{talentbox}
...
\begin{itemize}
    ℹtem \textbf{High Ground:} Gain +1 die to ranged attacks and +1 Position when defending against melee.
    ℹtem \textbf{Chokepoints:} A doorway or bridge turns a horde into a queue. Tanks can hold a chokepoint while strikers attack from behind.
    ℹtem \textbf{Cover:} A low wall or a wagon provides Partial Cover (+1 Armor), Total Cover (cannot be targeted). Destroying cover is a valid tactic.
    ℹtem \textbf{Difficult Terrain:} Mud, rubble, or shallow water halves movement. Ranged attackers thrive here; melee fighters suffer.
\end{itemize}
...
\begin{itemize}
    ℹtem \textbf{Teamwork:} Use the \textit{Aid} action to give an ally +1 die. Two characters aiding a striker can turn a miss into a crit.
    ℹtem \textbf{Interrupts:} Talents like \textit{Wound−Drinker} or \textit{The Replacement} allow actions outside your turn. These are gold.
    ℹtem \textbf{Use Boons:} Boons can be spent to improve Position or re−roll one failure. Do not hoard them.
\end{itemize}
...
\begin{itemize}
    ℹtem \textbf{Intimidation:} After a display of violence (a kill, a shattered weapon), make a Presence + Command test. On success, remaining enemies may flee or surrender.
    ℹtem \textbf{Deception:} A false flag (dressing as the enemy) can let you walk through a camp. Requires a Deception test and a plausible story.
    ℹtem \textbf{Parley:} Even in combat, a shouted offer of mercy can stop a fight. The GM may allow a Presence + Sway test as a free action.
    ℹtem \textbf{Environmental Manipulation:} Cut the rope holding the chandelier. Set fire to the grain store. Collapse the tunnel. Creativity trumps stats.
\end{itemize}
...
\begin{itemize}
    ℹtem \textbf{ACCURATE:} +1 die to attack rolls.
    ℹtem \textbf{Armor Piercing:} Ignore 1 point of Armor.
    ℹtem \textbf{Brutal:} On a critical hit, add +1 Harm.
    ℹtem \textbf{Defensive:} +1 die to Parry actions.
    ℹtem \textbf{Heavy:} Two−handed; cannot be used with a shield.
    ℹtem \textbf{Light:} May be dual−wielded.
    ℹtem \textbf{Reach:} Can attack from Near range without closing.
    ℹtem \textbf{Thrown:} May be used as a ranged weapon (Near).
\end{itemize}
...
\vspace{0.5cm}
ℎfill −−− \textit{Master Tarian Ironhand}
...
\appendix
\chapter{The Black River at Dawn −−− A Runic Warrior's Tale}

\noindent
Roran did not sleep the night after the trial.
...
Inside was not a portrait. It was a braid. A single, thin braid of auburn hair, tied with a scrap of red thread.
...
She stood on the far bank. Young. Younger than him. Younger than Eleanor. The age she had been when the birthing fever took her. Her hair fell loose, auburn and unbound. Her face held no sorrow−−−only a quiet certainty. A red blanket draped over her shoulders like a cloak.

``Mother,'' he said.

She smiled.

``You have carried the weight long enough. The locket was never meant to be a burden. It was meant to be a bridge.''
...
``Lay it at the bank. Tie it with the thread. Let the water carry it where it needs to go.''

``Where?''

``To her. To Bryn. She kept it for forty years. She never asked for thanks. But she deserves to know it reached you.''

He felt the old name strike him like a blow.

``That name forsook me. I am Roran now. That is the name I chose when I left the ledger behind.''

Her eyes softened. Not with disappointment. With understanding.

``Then let the river carry what you are ready to release. Not the name you were given. The weight you have carried because of it.''
...
``You have to let go,'' his mother said.
