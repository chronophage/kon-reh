...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Ring a bell} to create a \textbf{Warded [4]} clock in one zone: spirits suffer Position –1 and must test Resolve (DV 3) to enter.
\end{itemize}
\downside: If you ring a bell without first checking the ward‑pattern (i.e., without invoking remembrance), you may tick the \textbf{Fog Attention clock} (see below).
...
Every time the party fails a roll involving the fog, a spirit, or the Direwood \textit{while forgetting what must be remembered}, the GM may tick the \textbf{Fog Attention [6]} clock. When it fills:
\begin{itemize}[leftmargin=*]
...
\end{itemize}
Reducing the clock requires a ritual at a bell‑line \textit{while speaking a fragment of the heroine’s name} (spend 1 Boon per segment, or a service to the Aeler framed as an act of remembrance).
...
\end{itemize}
When the clock fills, they must choose:
\begin{itemize}[leftmargin=*]
  \item \textbf{Serve:} Go to the deep vaults for one month (off‑screen). Clear the clock and gain +2 XP, but return with a \textbf{Shadow} (a minor Condition: –1 die to resist fear, as you’ve seen something that still watches you – a shadow of the forgotten).
...
\section{Adventure Seeds (Revised for Name Remembrance)}
...
\begin{enumerate}[label=\arabic*.]
  ...
  \item \textbf{Salt‑pan fire:} The Aeler’s pan‑works are burning. Help put it out (Body + Athletics) to gain a temporary Ward‑Salt use. Fail and the Aeler Patience clock ticks +1.
  ...
  \item \textbf{The ninth cup:} A tavern keeper offers a ninth cup free. If a PC accepts, the Fog Attention clock ticks +1, \textit{and that night they dream of the heroine’s face – if they speak her name in the dream, they wake with +1 Boon}.
\end{enumerate}
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Curse:} The wielder is marked by the warden’s vestige. Once per session, the GM may tick the Fog Attention clock by 1 at no cost \textit{– unless the wielder speaks the heroine’s name aloud when activating this curse, in which case the vestige recoils and gains no Fog Attention}.
\end{itemize}
