...
\item \textit{The Crimson Masquerade} (High) -- Assume the appearance of any person you have touched (skin-to-skin) for one scene. Your voice matches theirs perfectly. Costs 2 Fatigue and marks a \textit{Debt Clock} [4] with Livaea.
\end{itemize}
...
\item \textit{The Debt Note} (Standard) -- Sing a curse on a rival. They suffer -1 die on their next heist. Costs 2 Fatigue and marks a \textit{Debt Clock} [4] that Malachai will collect. The collection is always painful and often ironic.
...
\item \textbf{The Grey Eminence} (Unknown; rumoured to be a Veiled Sister or a Covenant judge)
\end{enumerate}

Each Creditor has a \textbf{Favour Clock} [6]. When you do a significant service for them, tick the clock. When it fills, they owe you a major favour (a heist's worth of help). When you owe them a debt, they may call it in at any time.
...
\item \textit{Muscle} (a favour from a bruiser, a crew for a job)
...
\item \textbf{+2 to +3: Trusted.} You may request a Favour from any Creditor once per arc; you may attend the Velvet Moot.
\end{itemize}
...
\item \textbf{Learn Rumours} (gain 1d3 Strings of intelligence about future events)
\end{itemize}
...
The Court tracks two clocks that measure active attention from authorities and rivals:
\begin{itemize}
...
\item Start Burn [4] clock to recover/replace
...
\chapter{Court-Approved Heists (The Heist Clock)}

When the players accept a job from the Court, the GM sets up a \textbf{Heist Clock} [6] (simple) to [12] (complex) to track overall progress toward the objective. Additionally, the GM may establish \textbf{Threat Clocks} to track escalating danger during the heist.
...
\midrule
\textbf{Tension to rise steadily} & Use 2-3 Threat Clocks (Alarm, Patrol, Rival Interference). Tick them on \textit{Partials and Misses}. & Using only a single Heist Clock – players need to see \textit{multiple} pressures. \\
\textbf{Lockpicking to feel dramatic} & Make the safe a \textit{Progress Clock [6]}. Each Mechanics roll ticks \textit{1-2 segments on a Partial} (success on full tick). & Rolling once for "success/fail" – binary feels flat. \\
\textbf{Setup rolls to matter} & Grant \textit{Kit Tokens} (one-use +1d or Position boost) on \textit{strong hits}. On partials, start a complication clock (e.g., Guard Alert [3]). & Letting setup rolls have no mechanical effect. \\
...
\textbf{Partials to feel like progress} & Offer a \textit{menu}: reduce next attempt’s DV by 1, improve Position by 1 step, tick a Progress Clock, gain a Clue, or \textit{succeed at a cost} (GM gains 1 SB). & Saying "nothing happens" – that’s a Miss (no effect, GM gains SB). \\
\textbf{Momentum after a Miss} & \textbf{Spend the SB immediately}: new guard appears, door locks/jams, or tick a Threat Clock \textit{twice}. & Banking SB – strike while the iron is hot (escalation feels punitive, not reactive). \\
\textbf{Teamwork to shine} & Remind players of \textit{Strings}:\\
& • \textit{"You and Kael share a String – invoke it now: you both gain 1 Boon or reduce DV by 1."}\\
...
\section{Heist Clock}
The Heist Clock advances when players:
\begin{itemize}
...
\end{itemize}
The clock ticks backward when:
\begin{itemize}
\item A guard raises an alarm
\item A rival thief interferes
\item The party is forced to leave evidence
\end{itemize}
When the clock fills, the heist is complete. The players gain the loot, plus 1 Favour with the Creditor who hired them. The GM may also award a \textit{Court Favour} (a one-use String that can be traded for help, information, or a safe house).
If the clock empties (reaches 0), the heist fails. The party must escape, may lose a member, and gains a \textit{Debt} owed to the Creditor (they will call it in later).

\section{Threat Clocks}
The GM sets up Threat Clocks to track specific dangers:
\begin{itemize}
...
\end{itemize}
When a Threat Clock fills, the corresponding complication occurs (e.g., Alarm filling means guards are fully alerted and responding).
...
\item \textbf{Addicted to Adoration} -- If you go more than a session without a notable social triumph, you suffer -1 die to all social rolls until you perform a grand gesture (a public seduction, a scandalous revelation, a purchased favour).
\item \textbf{The Velvet Price} -- Livaea expects her followers to keep the oldest profession thriving. Refusing to trade in intimacy (even as a broker, not a direct participant) may cause her to withdraw her favour.
\end{itemize}
...
\begin{enumerate}
\item The Grey Eminence is actually a Veiled Sister who defected. She knows the true name of a bound spirit that guards the Sunken Quarter.
...
\item The Ninth Key legend is real. One fragment is held by the Grey Eminence. Another is in the Archivolt's vault.
...
\noindent\textbf{Agenda Clock [6]}
Track the rival crew's progress toward their goal. Tick the clock when:
\begin{itemize}
...
\end{itemize}
When the Agenda Clock fills, the rival crew achieves their goal (which may complicate the PCs' plans).
...
\item \textbf{Miss:} War [8] clock starts with 1 segment filled (rivals begin contesting control)
\end{itemize}
...
\begin{description}
  ...
  \item[Rumour Mill] Before a scene set in the city, you may state a plausible rumour about a person, place, or upcoming event. The GM must incorporate it as a truth (though possibly incomplete or misleading). Examples: \textit{“The Watch Captain visits the Silk House every Restday.”} or \textit{“The Dye Syndicate is about to raise prices.”}
...
\begin{itemize}
  ...
  \item A rival brothel gains +1 Favour with the Watch.
  \item The GM may advance a \textbf{Rival Clock} (e.g., \textit{Watch Raid} or \textit{Syndicate Takeover}).
\end{itemize}
...
\begin{enumerate}
  ...
  \item \textbf{Livaea’s Price} – The goddess demands a favour: sponsor a festival, deliver a cursed mirror, etc. Refuse and the House’s Free Use is unavailable for one session.
