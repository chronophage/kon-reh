...
\begin{itemize}
\item [THRESHOLD] [DREAD] [MARK]
\item Immune to fear effects. Cannot be compelled, charmed, or intimidated by magic that targets emotion.
\item Suffers -1 Position on Stealth rolls near the child. Shadows grow thin. Witnesses remember.
\item Cannot be harmed by any creature that casts a shadow. Convert all incoming Harm to Fatigue (the shadow absorbs the wound).
\item Clock: \textit{Return the Shadow} [8]. Ticks each session the child remains shadowless. When full, the original bargain finds a new debtor — possibly one of the characters.
\end{itemize}
