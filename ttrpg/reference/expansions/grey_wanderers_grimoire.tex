...
A Quick Working is a single-action roll that relies on the witch's knowledge of thresholds, elements, and will. Its effects are limited, subtle, and always carry the risk of obligation. The witch names a threshold, chooses a Layer (Echo, Veil, or Flow), and rolls Attribute + Skill (typically Wits + Lore or Spirit + Insight). On success, a minor effect manifests: a door sticks, a candle gutters, a pursuer slips. Partial results bring complications; failures invoke Promise Timers.
...
\begin{itemize}
    ...
    \item \textbf{The Promise Timer :} Every binding creates a weight, tracked as a timer of 4 to 6 segments. When it fills, the price comes due. A witch's ledger is written in flesh and time, not ink.
