...
\spell{Fate's Needle}{\abilitytag{Fate}, \abilitytag{Strike}, \dangerous{\abilitytag{Divination}}}{5}{Roll 3d10, keep the highest; that many successes auto-succeed on your next non-magical action.}{Major Backlash -- caster marks Harm 1 and begins a \textit{Fate Debt} timer [4].}
...
Most Cantors fear the \textit{bloom}. They see the Corruption Clock as a countdown to obsolescence, a timer ticking toward madness or monstrosity. They are wrong. The timer is not a bomb; it is a chrysalis.
...
\begin{itemize}
    ...
    \item \textbf{The Promise Clock:} Every binding creates a debt, tracked as a timer of 4 to 6 segments. When it fills, the price comes due. A witch's ledger is written in flesh and time, not ink.
...
This is a \textbf{Veil-edit}, bound to a Named anchor. It has a timer (Recursion [6]) that ticks when the victim reinforces the story by hiding their affliction or refusing witness. It has a counter---the bell, the name, the threshold. And it has a Story Beat menu: minor echoes, warped thresholds, layer-slips, and the propagation of the Ninth.
...
\begin{enumerate}
    ...
    \item \textbf{The Heart (Obishaal pure).} Bare stone altar. Speaking the name \textit{Kaelen} fills the Name Unspoken timer. The Master's spirit rises. If \textit{Master's Breath} is 1 or less, the Spector appears immediately. The party must speak the name while evading it.
\end{enumerate}
