...
\newcommand{\threeclockrule}{%
    ...
    Maintain at most 3 active clocks per scene: \\
    \begin{itemize}
        \item Merge Hunger/Consumption/Evolution into \textbf{Anomaly [6]}
        \item GM selects 3 of: Air, Light, Noise, Attention per scene
        \item Long-term clocks (Mask Integrity, etc.) tracked between sessions
        \item Faction clocks advance at session end only
    \end{itemize}
}
...
Represent Aeler law through: Strings system for contractual obligations; -1 DV in negotiations with proper documentation; ``Audit [4]'' clocks for contract violations.
...
\end{atTheTable}
\textit{Quotas as Clocks:} \textbf{Boat-Timber Quota [6]}, \textbf{Lanternwood Allotment [4]}—fill clock to unlock export; overfill triggers Canopy Censure
...
\begin{itemize}
    ...
    \item \textbf{Moss-Tally.} Reveal one hidden quota clock in market/lumberyard scene
\end{itemize}
...
\begin{itemize}
    ...
    \item When clock fills, suffer -1 die to all rolls until next dawn
...
When a Beastfolk, Dragonfolk, or Other PC moves openly in human lands, the GM creates an ``Otherness'' clock for that community (4--6 segments, as appropriate).
\begin{itemize}
  \item Each time the PC acts in a conspicuous way, mark 1 segment.
  \item When the clock fills, introduce a consequence: hunters, scholars, inquisitors, curious nobles, street rumors, or religious panic.
\end{itemize}
...
\end{itemize}
When the clock fills, you owe the Wild---or a Patron of the Wild---a serious price to reconcile who you have become.
