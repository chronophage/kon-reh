...
\end{itemize}
\textit{Fronts (example):} \textbf{Bark Blight [6]} $\rightarrow$ \textbf{Canopy Hunger [4]} $\rightarrow$ \textbf{Famine Tariff [4]}
...
Place-name, season-mark, kinship-hand, roof-tree sign. Absent any two $\rightarrow$ assume misreadings. When reading/pleading in older registers, increase DV by +1 unless holding Context String (gloss-tree, witness, song). Cashing Context String reduces DV by -1 and creates Shared Frame (Position +1 on first exchange)
\end{atTheTable}
...
\textbf{Harm Translation:} One scale larger $\rightarrow$ Effect +1; smaller $\rightarrow$ Effect --1 but may Call a Nook (convert into cover/Position once/scene instead of straight Harm).
...
\textbf{Levers:} Cite or be Cited = DV --1 (Plan/Research); Errata Slip downgrades Sting$\rightarrow$Public Correction [4]; Threshold Architecture = Traverse DV --1 (once/session).\\
