...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Leave the Light.} Replace the shade you take: plant, HEAL, or pay. (Use \emph{Light-Due Strings} below.)
\end{itemize}
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Quotas as Timers:} \emph{Boat-Timber Quota [6]}, \emph{Lanternwood Allotment [4]}. Fill the timer to unlock export; overfill and trigger \emph{Canopy Censure}.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Moss-Tally.} Reveal one hidden quota timer in a market or lumberyard scene.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Violets \& Stone:} Dock courts host River Days; Lethai-al ferries grant \emph{canoe-lane priority} to crews who HEAL ladders and steps.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Timers.} \textbf{Ridge--Root Stalemate [4]} (project stalls until a mixed design is signed); \textbf{Sluice vs. Salmon [4]} (silt, tariffs, tempers).
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Isolation Habits.} Markets on the margin (hedge fairs) rather than deep exchange; joint patrols that HEAL hedges together but camp apart; songs traded by echo, not chorus.
  \item \textbf{Strings.} light-due receipt; ferry knot; windbreak right; border-song.
  \item \textbf{Timers.} \textbf{Furlong Breach [4]} (speech, grazing, or felling in the strip); \textbf{Banner at the Hedge [3]} (Meadow banners crowd the edge; tempers tighten).
...
\begin{center}
  ...
  \item \textbf{Marks $\rightarrow$ Curses.} A mark without its living story curdles: a ``Hunter's Night'' becomes sleeplessness; a ``Mercy Knot'' becomes compulsion. The Lethai-ar curate and retire marks like hazardous heirlooms.
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Leave the Light.} Replace what you take: plant, HEAL, or pay \emph{seed-tithe}.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Oath-Magnet.} Words stick; a careless promise rewrites your plan (\emph{start a Task Timer you named aloud}).
...
\begin{itemize}
  ...
  \item \textbf{Step on Stone:} Where stones are laid, use them; every crushed sprout is a debt
  \item \textbf{Water First:} Pour a first cup to the river or cistern before you drink
  \item \textbf{Leave the Light:} Replace the shade you take: plant, HEAL, or pay
\end{itemize}
...
\begin{itemize}
  \item When a Lethai attempts to use talents from both paths in the same scene, mark 1 segment on a Curse Timer [4]
  \item When the timer fills, suffer -1 die to all rolls until the next dawn
...
\begin{center}
  ...
  \begin{itemize}
  \item \textbf{Keystone Audit:} Mark the one stone, beam, or rope that bears a route. Remove or HEAL as needed; leave a repair year ledger if you break it.
  ...
  \item Spending a Bell Token in a retreat converts one lethal consequence to a named price or timer.
  ...
  \item \textbf{Moves:} Name the Stone (spot the load point), HEAL or Break (choose repair-year or collapse), Toll-Binder (chalk a temporary tariff that even foes hesitate to cross)
  \end{itemize}
  ...
  \item \textbf{Mask Moot:} Declare roles, then seal parley under silk; shunts a lethal standoff into posture, concessions, and timers.
  ...
  \textbf{At the table (mechanical hooks):}
  \begin{itemize}
  \item When Mask-Right is established, treat first parley as Controlled$\rightarrow$Dominant (Position +1).
