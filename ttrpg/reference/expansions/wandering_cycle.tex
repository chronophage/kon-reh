...
\title{\Huge\textbf{The Wandering Cycle}\\ \large Tales of the Threshold, from the Eastern Roads}
...
\item \textbf{Clock: Borrowed Ground [6]} -- ticks each time the land is used without acknowledgment. When full, the spirit demands a name, a memory, or a story. Refusal causes crops to fail for a season.
\end{itemize}
...
\item Offering a gift (any possession of personal value) allows the character to "cut" one burden: a grudge, a geas, a relationship, or a memory. The GM removes that narrative thread from play.
\item The gift is consumed. The character cannot reclaim it.
\item \textbf{Clock: The Fox's Return [8]} -- ticks each time the character takes on a new burden. When full, the foxes return to collect "interest" --- a memory, a skill point, or a cherished relationship.
\end{itemize}
...
\begin{itemize}
\item Once per year (GM's discretion), a character who understands the timer's mechanism may declare that a time-sensitive agreement is invalid due to "evaporation error." The agreement is voided; the other party suffers --2 dice to all actions for a day (confusion).
...
\item \textbf{Clock: The Dripping Reckoning [4]} -- ticks each time the timer is used outside of the solstice. When full, the water spirit inside the timer awakens and demands a sacrifice (a memory, an oath, or a finger).
\end{itemize}
...
\item \textbf{Clock: The Road's Reckoning [6]} -- ticks each time the character accepts a gift or favor while carrying the message. When full, the message opens itself, and the character forgets the message's contents. The GM gains 3 SB.
\end{itemize}
