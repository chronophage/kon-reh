...
\begin{mechanicbox}
The Northern Crusade is not a single war but a \textit{process}. GMs can track it with a \textbf{Temple Pressure [6]} clock in a region. Each tick represents a new chapel, a synod court, a school, a bell‑tower. When the clock fills, the region becomes Temple‑controlled (venue tags shift to Temple District or Contested Ground). Player actions can tick the clock backward by restoring a shrine, exposing a corrupt synod judge, or leading a successful raid against a Temple granary.
\end{mechanicbox}
