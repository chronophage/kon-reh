...
\begin{itemize}
\item Vice Timer system for character flaws
...
\subsubsection{Vice Timer (4 segments per vice)}
...
\subsubsection{Pressure Timer (6-8 segments)}
...
\subsubsection{Reputation Timer (8 segments)}
...
\begin{itemize}
  ...
  \item \textbf{Recommended Patrons:}
    \begin{itemize}
      ...
      \item \textbf{Malachai (Curses)} -- corruption through intimate nightlife scenes.
...
\begin{itemize}
  ...
  \item \textbf{Recommended Patrons:}
    \begin{itemize}
      \item \textbf{Malachai} -- one-off curses for payback jobs.
...
\begin{tcolorbox}[colback=black!2,colframe=black!40!white,title={Noir Table Stakes},breakable]
...
\begin{itemize}
\item Use pre-generated characters or build using 20-32 XP with noir-themed backgrounds
...
\item Identify highest rank for Case Timer size (2-5:4, 6-10:6, J/Q/K:8, A:10)
...
\begin{itemize}
\item Case Timer based on highest card rank
...
\item Demonstrate Vice Timer management through temptation scenario
...
\begin{itemize}
\item Single Case Timer with simplified complications
...
\item Reputation/Pressure timers creating ongoing consequences
...
\item Vice Timer advancement opportunities for relevant characters
\end{itemize}
...
\item May trigger Pressure Timer advancement or Reputation changes
\end{itemize}

\textbf{Moral Choice:}
\begin{itemize}
\item Ethical dilemmas, compromise decisions, personal cost evaluations
\item May trigger Vice Timer advancement or Reputation Timer changes
...
\begin{itemize}
\item \framebox{Draw case cards and establish Case Timer }
...
\begin{itemize}
\item Use timer advancement and Vice temptations rather than adversarial GMing
...
\item More recovery options for timer advancement
\end{itemize}
...
\item Permanent consequences for filled timers and broken characters
\end{itemize}

\textbf{New Players:}
\begin{itemize}
\item Streamlined systems with fewer timers
...
\begin{itemize}
\item \textbf{1 Boon:} Prevent Vice Timer from incrementing this scene
\item \textbf{2 Boons:} Clear 1 segment from Vice Timer 
\item \textbf{4 Segments Full:} Draw new Vice card, apply fallout, reset timer
\end{itemize}
...
\begin{itemize}
\item Vice Timer system directly serves noir themes of moral ambiguity and self-destruction
...
\begin{itemize}
\item Introduce systems gradually across sessions (Investigation Points $\rightarrow$ Vice Timer $\rightarrow$ Evidence System)
\item Start with basic case management before adding secondary timers
...
\item Partial timer fills create regional reputation effects
...
\item Vice Timer integrates with existing Boon economy
\item Investigation procedures use standard resolution with thematic modifiers
\item Timer interactions follow logical cause-and-effect relationships
\end{itemize}
...
\begin{itemize}
\item New mechanics (Vice Timer , Investigation Points) feel natural to noir genre
...
\item Timer names and effects directly relate to noir concepts
...
\item Streamlined timers for new players maintain core tension
...
\begin{enumerate}
    ...
    \item Identify the highest rank as your main Challenge Timer (2-5: 4 segments, 6-10: 6 segments, J/Q/K: 8 segments, A: 10 segments)
...
\begin{enumerate}
    \item Draw 3 cards (Spade, Heart, Club)
    \item Identify highest rank for Challenge Timer (2-5:4, 6-10:6, J/Q/K:8, A:10)
...
\begin{itemize}
    \item \textbf{4-Segment Timer :} Visual tracker that increments during play
...
\begin{itemize}
    ...
    \item \textbf{Effect:} Prevent Vice Timer from incrementing
\end{itemize}

\textbf{Mitigation (Active):}
\begin{itemize}
    \item \textbf{Cost:} 2 Boons
    \item \textbf{Effect:} Clear 1 segment from Vice Timer 
\end{itemize}
...
\begin{enumerate}
    ...
    \item Reset timer based on new card (Jack=1, Queen=2, King=3, Ace=4)
\end{enumerate}
...
\begin{itemize}
    \item Only one Vice Timer increments per scene (character's choice)
...
\begin{itemize}
    ...
    \item \textbf{Vice Timer Integration:} If investigating involves a romantic interest (Heart Q), players with Emotional Vice may face temptation. If family secrets involve corruption, Professional Vice players may be tempted.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Vice Timer Integration:} Players with Professional Vice may be tempted by the councilman's offer. Those with Emotional Vice may develop feelings for the assistant.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Vice Timer Integration:} Players with Psychological Vice may become paranoid about the danger. Those with Physical Vice may turn to substances under pressure.
\end{itemize}
