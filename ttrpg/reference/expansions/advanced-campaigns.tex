...
\subsection{Overview}
Session Zero establishes the creative DNA of a Fate's Edge campaign. This procedure ensures players and GM share expectations, themes, relationships, and campaign focus from the start.

\subsection{Step 1: Character Intentions}
Each player defines:
\begin{itemize}
  \item \textbf{Primary Goal}: What they hope to achieve in the first arc.
  \item \textbf{Secondary Goal}: Long-term or personal desire.
  \item \textbf{Connection}: Link to at least one other PC.
\end{itemize}

\subsection{Step 2: GM Campaign Seeds}
The GM presents 3--5 potential campaign seeds:
\begin{itemize}
  \item A regional crisis
  \item An emerging supernatural threat
  \item A political upheaval
  \item A legendary figure’s return
  \item Something deeply personal to the PCs
\end{itemize}

\subsection{Step 3: Collaborative Crown Spread}
The table performs the Crown Spread together:
\begin{enumerate}
  \item Choose the card that excites the group most.
  \item Rewrite interpretations to reflect group goals.
  \item Form 2--4 \textbf{Initial Stakes} that connect to PCs.
\end{enumerate}

\subsection{Step 4: Faction and Relationship Mapping}
\begin{itemize}
  \item Define which factions are relevant to PC goals.
  \item Establish starting attitudes (use the -3 to +3 scale).
  \item Identify at least one \textbf{shared antagonist}.
\end{itemize}

\subsection{Step 5: Tone and Theme Alignment}
Choose 3--5 campaign themes:
\begin{itemize}
  \item Redemption, tragedy, legacy, mystery, revolution, faith, corruption
\end{itemize}

\subsection{Step 6: Safety and Table Contracts}
A brief but explicit agreement on:
\begin{itemize}
  \item Narrative expectations
  \item Lines and veils
  \item Desired playstyle (episodic, character-driven, political, mythic)
\end{itemize}
...
\subsection{Purpose}
Campaigns naturally fluctuate between high-intensity arcs and slower, contemplative sessions. The Campaign Momentum Tracker provides a simple, table-visible metric for tracking these shifts and using them to guide pacing, difficulty, and narrative tension.

\subsection{The Momentum Dial}

\begin{center}
\textbf{-3 \quad -2 \quad -1 \quad 0 \quad +1 \quad +2 \quad +3}
\end{center}

\noindent
Momentum reflects the \textbf{current energy of the campaign world}:
\begin{description}
  \item[-3 (Stagnant)] The world is quiet. Threats retreat, mysteries deepen, NPCs withdraw.
  \item[-1 to -2 (Low)] The pace slows. Investigation, downtime, reflection.
  \item[0 (Balanced)] Normal storytelling flow. Most scenes operate without modifiers.
  \item[+1 to +2 (Surging)] Threats accelerate, factions become active, opportunities spike.
  \item[+3 (Overclocked)] Crisis mode. Cascading consequences and major turning points.
\end{description}

\subsection{Triggers}

Momentum shifts only on meaningful developments:

\subsubsection*{Positive Shifts (+1)}
\begin{itemize}
  \item Major PC victory or solved arc
  \item Significant faction upheaval
  \item New campaign information revealed
  \item A clock reaches a crisis threshold
\end{itemize}

\subsubsection*{Negative Shifts (-1)}
\begin{itemize}
  \item PCs suffer major loss
  \item A faction collapses or withdraws support
  \item A mystery goes cold
  \item A major NPC abandons or betrays the party
\end{itemize}

\subsection{Mechanical Effects}

\begin{description}
  \item[Difficulty Modifiers:] At +2 or +3, key rolls increase DV by +1. At -2 or -3, reduce DV by 1 where narratively appropriate.
  \item[Clock Pressure:] High momentum increases advancement rate of hostile clocks.
  \item[NPC Attitudes:] Factions act boldly at high momentum and conservatively at low momentum.
  \item[Player Benefits:] Low momentum grants a minor narrative advantage: reduced Fatigue, easier investigation, or bonus positioning.
\end{description}

\subsection{Using Momentum as a GM Tool}

Momentum is a dashboard indicator, not a story script:
\begin{itemize}
  \item Raise momentum when the campaign needs pressure or acceleration.
  \item Drop momentum when players need space, reflection, or agency resets.
  \item Never change momentum for trivial events.
\end{itemize}
