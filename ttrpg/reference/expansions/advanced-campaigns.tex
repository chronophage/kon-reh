...
\fancyhead[R]{\thepage}
\fancyfoot[C]{Page \thepage\ of \texttt{[MISSING REF: LastPage]}}
...
\subsubsection{Timer Evolution System}
...
\subsubsection{Creating New Timers}
...
\subsubsection{Timer Interactions}

Advanced campaigns benefit from timer relationships:

\begin{description}
  \item[Supporting Timers] One timer's progress helps another (Plague Spread $\rightarrow$ Resource Scarcity)
  \item[Opposing Timers] One timer's progress hinders another (Public Support ↓ Crime Rate ↑)
  \item[Cascade Timers] One timer's resolution triggers another (War Ends $\rightarrow$ Reconstruction Begins)
  \item[Hidden Timers] Progress tied to player ignorance (Ancient Awakening while players focus elsewhere)
\end{description}
...
This worksheet becomes part of the Campaign Dashboard (see Section~\texttt{[MISSING REF: sec:campaign-dashboard]}).
...
Use answers to set initial \textbf{Campaign Themes} and seed the Resonance System (see Section~\texttt{[MISSING REF: sec:campaign-resonance]}).
...
\begin{enumerate}
    ...
    \item Seeds for the first set of \textbf{Campaign Timers}
\end{enumerate}

These can be formalized into the Campaign Dashboard (Section~\texttt{[MISSING REF: sec:campaign-dashboard]}) and referenced at the start of each arc.
...
\begin{description}
  ...
  \item[$+1$ Rising] Players are driving events, timers are moving, stakes feel alive.
  \item[$+2$ Surging] Big swings, hard choices, multiple timers near crisis.
  \item[$+3$ Overtimered] Everything is on fire. Climax-level intensity nearly every scene.
\end{description}

Keep the dial on a small index card or a corner of your Campaign Dashboard (see Section~\texttt{[MISSING REF: sec:campaign-dashboard]}). Adjust it only when something \emph{meaningful} happens; this is a coarse-grain tool, not a per-roll modifier.
...
\begin{itemize}
  \item \textbf{Decisive Victories}: Players resolve a major timer, defeat a key threat, or secure a powerful asset.
...
\begin{itemize}
  ...
  \item \textbf{Plateaus}: Multiple sessions where timers barely move and stakes do not escalate.
...
\begin{itemize}
  ...
  \begin{itemize}
    \item Factions take bolder moves on their turns (see Section~\texttt{[MISSING REF: sec:faction-turns]}).
...
\begin{enumerate}
  \item \textbf{What changed in the world?} (Timers, factions, big events)
...
\begin{itemize}[leftmargin=1.2em]
  ...
  \item \textbf{Balanced (-1 to +1):} Default rhythm; normal DV and timer motion.
...
\begin{itemize}[leftmargin=1.2em]
  ...
  \item \textbf{Apply:} High Momentum $\rightarrow$ escalate timers; Low Momentum $\rightarrow$ ease difficulty.
\end{itemize}
...
\begin{itemize}
  \item[$\square$] Review current campaign timers and their interactions
...
\begin{itemize}
  ...
  \item[$\square$] Advance relevant campaign timers
...
\subsubsection{1. Modify a Timer Outcome (1 PAP)}
After a timer advances due to player failure or faction action:
\begin{itemize}
    \item Reduce its advancement by 1 tick, \emph{or}
    \item Move advancement to a different related timer (GM discretion).
\end{itemize}
...
\begin{enumerate}
    ...
    \item Connect Region to Campaign Themes and Timers
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Position Shifts:} Start certain actions at Controlled, Controlled, or Desperate.
    ...
    \item \textbf{Timer Pressure:} Fast timers during storms, slow timers during festival seasons.
\end{itemize}
...
\begin{itemize}
    ...
    \item Create 1--2 \textbf{Regional Timers}.
...
\textbf{Regional Timers:} \underline{\phantom{XXXXXX}}
...
\subsubsection{World Timer Resolution}
...
\begin{itemize}
    ...
    \item Legacy timers or unresolved echoes.
\end{itemize}
...
\begin{itemize}
    \item Momentum $\rightarrow$ shifts public sentiment.
    \item Agency Points $\rightarrow$ negotiate, blackmail, or expose truths.
    \item Faction Attitudes $\rightarrow$ political DV modifiers.
    \item Regional Pressures $\rightarrow$ spark intrigue events.
\end{itemize}
...
\begin{itemize}
    ...
    \item Localized timers (4-segment max).
...
\begin{itemize}
    ...
    \item 6-segment timers dominate structure.
\end{itemize}
...
\begin{itemize}
    ...
    \item Long-form 8-segment timers.
\end{itemize}
...
\begin{itemize}
    ...
    \item A Crown Anchor shifts type (Suit $\rightarrow$ Mythic).
\end{itemize}
...
\begin{description}
    \item[Escalation] Threat advances; a timer ticks.
...
\begin{itemize}
    ...
    \item \textbf{Current Timer State}
...
\begin{quote}
\textbf{Momentum +1 $\rightarrow$} Faction goodwill, lower DV, allies appear \\
\textbf{Momentum +2 $\rightarrow$} Regional stability, access to rare resources \\
\textbf{Momentum -1 $\rightarrow$} Timers accelerate, NPCs cautious or fearful \\
\textbf{Momentum -2 $\rightarrow$} Regional panic, Patron demands, world-scale echoes
\end{quote}
...
Timer Impact & \underline{\phantom{XXXXXXXX}} \\
\hline
\end{longtable}
\end{center}
...
\begin{enumerate}
    ...
    \item Advance 1 timer or shift Momentum by 1.
...
\begin{itemize}
  ...
  \item Advance a meaningful timer.
...
\begin{enumerate}
    ...
    \item \textbf{Travel Timers} (progress, danger, resources)
...
\begin{itemize}
    \item Progress Timer (to reach destination)
    \item Danger Timer (environment, creatures)
    \item Resource Timer (food, water, gear)
    \item Discovery Timer (secrets, sites, lore)
\end{itemize}
...
% ============================================================
% 9. REGION $\rightarrow$ REGION TRANSITIONS
% ============================================================
...
\begin{itemize}
    ...
    \item \textbf{One Threat} (4--6 segment timer)
...
When overloaded:
\begin{itemize}
    \item Combine two small timers into one bigger timer.
    \item Or resolve a timer early using the highest-tension outcome.
\end{itemize}
...
\begin{description}
    \item[Conflict Mode:] Threats advance; timers tick twice as fast.
...
\textbf{Threat Timer :} \underline{\phantom{XXXXXXXXXXXX}} \\
