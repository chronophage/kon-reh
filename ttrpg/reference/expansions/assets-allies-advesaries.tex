...
\item \textbf{Make Holdings Matter:} Assets have tags, strings, upkeep, and project timers that change scenes. Patronage brings Position, seals, and coin---and obligations, audits, and scandals.
\item \textbf{Companions with Teeth:} Followers act on-screen and off-screen with clear risks, timers, and costs. Social fabric (Loyalty, Morale, Bonds) generates scenes and consequences.
\item \textbf{Organizations as Levers:} Cohorts and institutions move through timers, strings, and orders without excessive bookkeeping.
\item \textbf{Use the Core:} Leverages core Fate's Edge mechanics: Position/\dv, \sbtext, Timers, Favor/Leverage/Heat, Reputation, Strings. No new dice math.
...
\item \textbf{Pick an \asset:} Choose a type (safehouse, workshop, etc.). Mark Integrity \timer{6} and Resources \timer{6}. Attach 1-2 Strings (permits, seals). Choose 1-2 Project Timers \timer{4}-\timer{8}.
\item \textbf{Name a \patron (optional):} Set Patron \tier and Obligations.
\item \textbf{Create 1-2 \followers:} Name them, pick a Playbook (Combat Ally, Specialist, etc.). Mark Loyalty \timer{6} and Morale \timer{6}. Attach Strings they control.
\item \textbf{Pick an \organisation (if any):} Mark Cohesion \timer{6}-\timer{8}, Heat \timer{6}, and Bandwidth (1-3 orders/cycle).
...
\textbf{Tracks:}
\begin{itemize}[noitemsep]
\item \textbf{Integrity \timer{6}:} condition/standing; fill $\rightarrow$ shutdown or seizure
\item \textbf{Exposure \timer{8}:} attention/risk; fill $\rightarrow$ audit/attack/crisis
...
\item \tag{Hazardous}: On 1s, GM may start Accident \timer{4}
...
\item \textbf{Partial:} Choose: pay upkeep (no yield) OR generate yield (status $\rightarrow$ Strained)
\item \textbf{Miss:} Crisis. Status $\rightarrow$ Strained/Broke + trigger Event + tick Exposure +1
\end{itemize}

\textbf{\asset Orders (Between Sessions):} Choose 2 orders per solvent asset:
\begin{itemize}[noitemsep]
\item \textbf{Operate:} Generate Yield + risk Exposure
\item \textbf{Improve:} Advance Project Timer 
...
\item \textbf{Sanction \timer{4}:} warning $\rightarrow$ censure $\rightarrow$ revocation $\rightarrow$ hunt
\end{itemize}
...
\textbf{Tracks:}
\begin{itemize}[noitemsep]
\item \textbf{Spirit \timer{6}:} loyalty/morale (4-6: Dominant; 2-3: Controlled; 0-1: Desperate/quit)
\item \textbf{Harm \timer{3}:} minor / serious / broken
\end{itemize}
...
\textbf{Tracks:}
\begin{itemize}[noitemsep]
\item \textbf{Integrity \timer{8}:} health (5+: Bandwidth 3; 2-4: Bandwidth 2, +1 DV; 0-1: Bandwidth 1, +2 DV)
\item \textbf{Exposure \timer{6}:} visibility/risk
\end{itemize}
...
Trigger 1-2 between sessions when Exposure $\geq$ 4 or on missed Cycle roll:
\begin{itemize}[noitemsep]
\item \textbf{Inspection \timer{4}:} Official scrutiny
\item \textbf{Accident \timer{4}:} Mishap with consequences
...
\textbf{Cycle Roll:} Partial $\rightarrow$ Choose yield (clue) $\rightarrow$ Status $\rightarrow$ Strained. \\
\textbf{Delegate:} Specialist to archives $\rightarrow$ Partial $\rightarrow$ gets info but ticks Spirit -1. \\
...
\begin{center}
    ...
    \begin{enumerate}
      ...
      \item Choose 1--2 \textbf{Project Timers [4--8]} to improve or expand the asset.
    ...
    \textbf{Type:} safehouse / barge company / workshop / archive / shrine / foundry line / office / permit / theater / farm / mine / caravan yard / canal lock lease\\
    ...
    \begin{itemize}
      ...
      \item \textbf{Resources [6]} drop from upkeep, shocks, strikes; refill via Orders, scenes, or Projects. If Resources = 0, future Upkeep requires \textbf{Favor} or a concession timer.
    \end{itemize}
    ...
    \begin{itemize}
      ...
      \item \textbf{Tier 3 --- Protector:} start public scenes \textbf{Dominant} in their venues; \textbf{Endorsement} creates a 4-timer \emph{Bandwagon} on targets.
    \end{itemize}
    ...
    \begin{itemize}
      ...
      \item \textbf{Patron Tiers:} 0 Contact \textbullet{} 1 Sponsor (+Pos once/scene; 1 stipend) \textbullet{} 2 Patron (cancel 1 Social SB; DV --1 to Petition) \textbullet{} 3 Protector (start Dominant; Bandwagon timer).
    ...
    \begin{itemize}
      \item \textbf{Make holdings matter:} Assets have tags, strings, upkeep, and project timers that change scenes.
      \item \textbf{Patrons with teeth:} Patronage brings Position, seals, and coin---and obligations, audits, and scandals.
      \item \textbf{Use the core:} Position/DV, SB, timers, Favor/Leverage/Heat, Reputation, Strings. No new dice math.
      \item \textbf{Low bookkeeping:} Track names, tags, and a few timers; orders and events move the world.
    ...
    \item \textbf{Strings as Leverage:} Patron-granted strings can be used to improve asset projects. Spending a patron string gives +2 segments on a relevant project timer.
    ...
    \item Advance \textbf{Project Timers} from successful orders
    \end{enumerate}
    ...
    \begin{itemize}
    \item \textbf{Asset Crisis $\rightarrow$ Patron Problems:} Asset Integrity failure ticks Patron Sanction +1
    \item \textbf{Patron Scandal $\rightarrow$ Follower Doubts:} Patron Sanction at 3+ gives -2 Spirit to all followers
    \item \textbf{Follower Betrayal $\rightarrow$ Organization Schism:} Follower at 0 Spirit may trigger Organization Integrity -2
    \item \textbf{Organization Collapse $\rightarrow$ Asset Vulnerability:} Organization Integrity failure removes asset security tags
    \end{itemize}
    ...
    \hline
    All Exposure ≤ 3 & Any Exposure = 4-5 & Any Exposure $\geq$ 6 \\
    All Integrity $\geq$ 4 & Any Integrity = 2-3 & Any Integrity ≤ 1 \\
    Spirit all $\geq$ 3 & Any Spirit = 1-2 & Any Spirit = 0 \\
    Sanction = 0 & Sanction = 1-2 & Sanction $\geq$ 3 \\
    \end{longtable}
    ...
    \begin{itemize}
    \item \textbf{Too Many Timers?} Combine related timers (e.g., asset Exposure and patron Sanction can share a unified ``Trouble'' timer)
...
\subsubsection{Villain Timers}

Rather than hit points, villains are tracked through narrative timers:

\begin{itemize}
  \item \textbf{Influence Timer (6-8 segments):} How much control they have over the world
  \item \textbf{Momentum Timer (6 segments):} How close they are to achieving their goal
  \item \textbf{Moral Timer (4-6 segments):} How far they've strayed from their original ideals
  \item \textbf{Alliance Timer (4 segments):} Who still stands with them
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Tactical Actions:} Reactions to player actions (1 per timer segment filled)
...
\textbf{Mechanics:}
\begin{itemize}
  \item Multiple timers representing different facets
  \item Defeat requires changing the system, not killing a leader
  \item Player actions affect multiple timers simultaneously
\end{itemize}
...
\textbf{Mechanics:}
\begin{itemize}
  \item Shared timers with the PCs (Moral Timers, Relationship Timers)
...
\textbf{Mechanics:}
\begin{itemize}
  \item No ``defeat'' timer---only alternative paths
...
\textbf{Framework:}
\begin{itemize}
  \item Has their own \textbf{Legacy Project Timer } (8-12 segments)
...
\begin{itemize}
  ...
  \item Search for the Crown's Secret (Advance Timer , risk Personal Cost)
\end{itemize}
...
\begin{itemize}
  ...
  \item When their timer will fill without intervention
\end{itemize}
...
\begin{itemize}
  \item \textbf{Favor Timer (4-6 segments):} How well-liked the NPC is by the players
  \item \textbf{Loyalty Timer (6 segments):} Their commitment to the party or cause
  \item \textbf{Knowledge Timer (4-8 segments):} What they know and are willing to share
  \item \textbf{Influence Timer (4-6 segments):} Their power and connections in the world
\end{itemize}
...
\textbf{Mechanics:}
\begin{itemize}
  \item Has their own timers: Loyalty, Skill, and Morale
  \item Gains experience through exposure to the party's journey
  \item Can gain \textbf{Follower} status with the party through Loyalty Timer advancement
  \item Can choose to leave if Loyalty Timer drops too low
\end{itemize}

\textbf{Example: Reese the Outcast}
\begin{itemize}
  \item \textbf{Role:} Scavenger from the edge of the city
  \item \textbf{Loyalty Timer :} 3/6 (helped the party, but fears retribution)
  \item \textbf{Knowledge Timer :} 4/6 (knows the undercity)
...
\textbf{Mechanics:}
\begin{itemize}
  \item Tracks: Rivalry Timer (6 segments), Trust Timer (6 segments)
  \item Rivalry Timer : Advances when interests conflict; fills at 6
  \item Trust Timer : Advances when the rival trusts the party; fills at 6
  \item When Rivalry Timer fills, the rival must choose: compete, cooperate, or withdraw
  \item When Trust Timer fills, the rival becomes an ally
\end{itemize}
...
\textbf{Mechanics:}
\begin{itemize}
  \item \textbf{Patron Timer :} Tracks the patron's trust in the party (6 segments)
  \item \textbf{Secret Timer :} Tracks how much the party knows about the patron (4 segments)
  \item \textbf{Favor Bank:} Spent to activate hidden benefits
\end{itemize}

\textbf{Example: The Veiled Benefactor}
\begin{itemize}
  \item \textbf{Patron Timer :} 3/6 (has provided safe passage and resources)
  \item \textbf{Secret Timer :} 2/4 (party suspects they're a former royal)
...
\textbf{Framework:}
\begin{itemize}
  \item \textbf{Legacy Timer (8-12 segments):} Tracks the character's influence after they leave the story
  ...
  \item \textbf{Fading Timer :} Tracks how long their influence lasts
\end{itemize}

\textbf{Example: Master Dara, the Archivist}
\begin{itemize}
  \item \textbf{Legacy Timer :} 7/10 (her knowledge is slowly being lost)
...
\begin{itemize}
  ...
  \item \textbf{World Timer :} Tracks how the world changes under their influence
...
\begin{itemize}
  ...
  \item \textbf{World Timer :} 4/8 (has reshaped trade laws but faces opposition)
...
\begin{itemize}
  \item \textbf{Favor Timer [6]:} 4/6 (helped the party, but they must earn her trust)
  \item \textbf{Loyalty Timer [6]:} 3/6 (willing to guide them through the archives)
  \item \textbf{Knowledge Timer [8]:} 5/8 (can share city history and secrets)
  \item \textbf{Influence Timer [4]:} 2/4 (respected by scholars, distrusted by authorities)
\end{itemize}
...
\textbf{Strategic Actions:}
\begin{itemize}
  \item Share a fragment of dangerous knowledge (Loyalty Timer +1)
  \item Warn of impending danger (Favor Timer +1)
  \item Withhold information (Loyalty Timer -1)
\end{itemize}
...
\subsubsection{NPC Evolution Timer }

Track how the NPC changes through the campaign:

\begin{itemize}
  \item \textbf{Level 1:} Background Character (no special mechanics)
  \item \textbf{Level 2:} Notable Character (1 timer)
  \item \textbf{Level 3:} Impactful Character (2 timers, can evolve)
  \item \textbf{Level 4:} Central Figure (3+ timers, can change world)
