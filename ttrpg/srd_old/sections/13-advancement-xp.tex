...
\begin{description}[leftmargin=1.5em, style=nextline]
  ...
  \item[Off‑Screen Assets (Minor/Standard/Major):] Minor (4 XP, 1 day), Standard (8 XP, 1 week), Major (12 XP, 1 month).
  \item[Talents/Minor Assets:] As listed (e.g., Spellcraft 6 XP; Patron's Symbol 4 XP).
\end{description}
...
\begin{itemize}
  \item \textbf{Efficient (Higher XP, Less Time)} -- Cost: Upkeep XP = $\max(1, \lceil \frac{\text{Acquisition XP}}{3} \rceil)$. Minimal effort.
...
When you buy or upgrade an ability during downtime, you may skip the normal training time. The GM creates a \textbf{Haste clock [4]}. If the clock fills (e.g., from missed rolls or complications), the new ability or asset carries flaws or narrative hitches. (See Training and Development Time in the full rules.)
...
\paragraph{High‑Tier Play and Boons}
As characters advance, the role of \textbf{Boons} shifts. At early tiers, Boons primarily buy \emph{reliability} -- negating Fatigue, stabilizing risky actions, or nudging Position. At higher tiers, Boons increasingly buy \emph{authority}. A single Boon may force a truth, seal a pact, redirect consequences, or resolve a phase of a conflict outright.

High‑tier characters typically have \emph{fewer Boons to rely on}, but each spend carries greater narrative weight. This is intentional. Power at high tier comes from preparation, Bonds, oaths, and hard choices -- not from inflating Difficulty Values. The world does not become harder to roll against simply because characters are stronger; instead, success and failure both reshape the story in larger ways.
