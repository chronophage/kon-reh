#!/usr/bin/env python3
"""
add_copyright.py – Insert \input{copyright} into .tex files if missing.
"""
...
def find_tex_files(root: Path, recursive: bool = False, max_depth: int = -1) -> List[Path]:
    """
    Find .tex files under root.
    If recursive is False, only scan root.
    If max_depth >= 0, limit recursion depth (relative to root).
    """
    if not recursive:
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        if max_depth >= 0 and depth > max_depth:
            # Prune further recursion by deleting dirnames
            dirnames.clear()
            continue
        # Add .tex files
        for f in filenames:
            if f.endswith(".tex"):
                tex_files.append(Path(dirpath) / f)
    return tex_files
...
def main():
    ...
    parser.add_argument(
        ...
        help="Maximum recursion depth (only used with --recursive). -1 means unlimited."
    )
    ...
    # Gather .tex files
    tex_files = find_tex_files(root, recursive=args.recursive, max_depth=args.depth)
